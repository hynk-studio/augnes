...
const smokePath = "scripts/smoke-salience-governor-browser-validation-v0-1.mjs";
...
const packageScriptName = "smoke:salience-governor-browser-validation-v0-1";
...
const validationKind = "salience_governor_browser_validation";
const validationVersion = "salience_governor_browser_validation.v0.1";
const recommendationStatus =
  "ready_for_bounded_external_source_intake_contract_v0_1";
const nextRecommendedSlice = "bounded_external_source_intake_contract_v0_1";
const boundedExternalSourceIntakeContractDownstreamPointer = [
...
const rebuiltImplementationFixture = buildSalienceGovernorImplementation({
  salience_governor_contract: contractFixture,
  source_contract_ref: `${contractFixture.contract_version}:${contractFixturePath}`,
});
const topKOverrideResult = buildTopKOverrideResult();
...
assertValidatedPriorityView(fixture.validated_priority_view);
assertAuthorityBoundary(fixture.authority_boundary);
assertGeneratedPriorityView();
assertTopKOverride();
assertDocsPointers();
...
assert.deepEqual(
  fixture,
  rebuiltFixture,
  "rebuilt Salience Governor browser validation fixture must match committed fixture",
);
  ...
  JSON.stringify(
    {
      ...
      generated_priority_view_boundary_matches_contract:
        fixture.validated_priority_view
          .generated_priority_view_authority_boundary_matches_contract,
      top_level_implementation_boundary_is_separate:
        fixture.validated_priority_view
          .top_level_implementation_boundary_is_separate,
      all_scores_in_range: fixture.validated_priority_view.all_scores_in_range,
      default_top_k_applied: fixture.validated_priority_view.default_top_k_applied,
...
function buildValidationFixture() {
  const validation = {
    ...
    source_implementation_ref:
      `${implementationFixture.implementation_version}:${implementationFixturePath}#719`,
    ...
    validated_priority_view: {
      generated_priority_view_follows_contract: true,
      generated_priority_view_authority_boundary_matches_contract: true,
      generated_priority_view_validation_matches_contract: true,
      top_level_implementation_boundary_is_separate: true,
      all_scores_in_range: true,
      default_top_k_applied: true,
      candidate_preview_count_within_top_k: true,
      action_hints_are_hint_only: true,
      action_hints_do_not_mutate_now: true,
...
function buildTopKOverrideResult() {
  const [first, second] =
    contractFixture.sample_salience_priority_view.candidate_priority_preview;
  return buildSalienceGovernorImplementation({
    ...
    candidate_priority_preview: [
      {
        ...clone(first),
        candidate_ref_id: "candidate_ref_z_tie",
        salience_score_preview: 0.91,
      },
      {
        ...clone(second),
        candidate_ref_id: "candidate_ref_a_tie",
        salience_score_preview: 0.91,
      },
      {
        ...clone(first),
        candidate_ref_id: "candidate_ref_lower",
        salience_score_preview: 0.12,
      },
    ],
  });
}
...
function assertStaticBoundary() {
  ...
  for (const changedFile of changedFiles) {
    assert.ok(
      ...
      `unexpected changed file in Salience Governor browser validation slice: ${changedFile}`,
    );
...
function assertImplementationRebuild() {
  assert.deepEqual(
    ...
    "#719 implementation fixture must match rebuilt deterministic output",
  );
}
...
function assertValidatedPriorityView(value) {
  for (const [key, expected] of Object.entries({
    ...
    assert.equal(value[key], expected, `${key} must be ${expected}`);
  }
}
...
function assertGeneratedPriorityView() {
  const priorityView = implementationFixture.generated_salience_priority_view;
  ...
  assert.equal(Object.hasOwn(priorityView.authority_boundary, "implementation_added_now"), false);
  ...
  for (const [key, flag] of Object.entries(implementationFixture.authority_boundary)) {
    if (
  ...
  assert.equal(implementationFixture.validation.generated_priority_view_follows_contract, true);
  assert.equal(implementationFixture.validation.generated_priority_view_boundary_matches_contract, true);
  assert.equal(implementationFixture.validation.generated_priority_view_validation_matches_contract, true);
  assert.equal(implementationFixture.validation.top_level_implementation_boundary_is_separate, true);
  assert.equal(implementationFixture.validation.all_scores_in_range, true);
  assert.equal(implementationFixture.validation.action_hints_are_hint_only, true);
  assert.equal(implementationFixture.validation.action_hints_do_not_mutate_now, true);
  assert.equal(implementationFixture.validation.salience_score_not_authority, true);
  assert.equal(implementationFixture.validation.salience_score_not_promotion_readiness, true);
  assert.equal(implementationFixture.validation.salience_score_not_durable_approval, true);
  assert.equal(implementationFixture.validation.salience_score_not_evidence_strength, true);
  assert.equal(implementationFixture.validation.non_authority_policy_preserved, true);
  assert.equal(implementationFixture.priority_view_summary.top_k, contractFixture.priority_view_contract.default_top_k);
  assert.equal(
  ...
  assert.equal(implementationFixture.action_hint_summary.all_hints_hint_only, true);
  assert.equal(implementationFixture.action_hint_summary.all_hints_no_mutation_now, true);
  assert.equal(implementationFixture.priority_view_summary.priority_view_is_display_only, true);
  assert.equal(
    implementationFixture.priority_view_summary.priority_view_does_not_delete_or_hide_records,
    true,
  );
  assert.equal(implementationFixture.inhibition_component_summary.suppression_is_display_hint_only, true);
  assert.equal(implementationFixture.inhibition_component_summary.reactivation_is_display_hint_only, true);
}

function assertTopKOverride() {
  const candidates =
    topKOverrideResult.generated_salience_priority_view.candidate_priority_preview;
  assert.equal(topKOverrideResult.priority_view_summary.top_k, 1);
  assert.equal(candidates.length, 1);
  assert.equal(candidates[0].candidate_ref_id, "candidate_ref_a_tie");
  assert.equal(candidates[0].salience_score_preview, 0.91);
}
...
function assertDocsPointers() {
  for (const requiredText of [
    ...
    "validates generated display/reuse priority view from #718 contract",
    "validates generated priority view contract authority boundary",
    "validates top-level implementation boundary separation",
    "validates salience component summary",
    "validates inhibition component summary",
    "validates action hint summary",
    "validates priority view summary",
    "validates salience score preview as display/reuse priority only",
    "validates synthetic top_k override behavior",
  ...
  for (const doc of [substrateDoc, surfaceDoc, gateDoc]) {
    assert.match(doc, /Salience Governor browser validation v0\.1/i);
    ...
    assert.match(doc, /Bounded External Source Intake contract v0\.1|bounded_external_source_intake_contract_v0_1/i);
  }
}
...
function assertImplementationSmokeDownstreamPointer() {
  for (const requiredText of [
    ...
    assert.ok(
      ...
      `#719 Salience Governor implementation smoke must allow browser validation downstream pointer: ${requiredText}`,
    );
  }
}
