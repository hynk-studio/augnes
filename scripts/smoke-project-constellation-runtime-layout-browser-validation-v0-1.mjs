...
const {
  buildProjectConstellationRuntimeLayoutImplementationFixture,
  buildProjectConstellationRuntimeLayoutPreviewBundle,
  validateProjectConstellationRuntimeLayoutPreviewBundle,
  createProjectConstellationRuntimeLayoutFingerprint,
} = await import(
  "../lib/research-candidate-review/project-constellation-runtime-layout.ts"
);
  ...
  JSON.stringify(
    {
      ...
      durable_perspective_state_read_now:
        fixture.authority_boundary.durable_perspective_state_read_now,
...
function assertBuilderFile() {
  for (const requiredText of [
    ...
    "salience_summary",
    "reference_summary",
    "invalid_layout_preview_override_rejected",
    "invalid_node_override_rejected",
    "invalid_edge_override_rejected",
    "invalid_authority_boundary_override_rejected",
    "invalid_refs_override_rejected",
    "layout_preview_missing_layout_id",
    "node_missing_node_ref",
    "edge_missing_edge_ref",
...
function assertPerspectiveGeometryDigestContractDownstreamPointer() {
  ...
  for (const requiredText of [
    ...
    assert.ok(
      digestSmoke.includes(requiredText),
      `#738 browser validation smoke must allow Perspective Geometry Digest contract downstream pointer: ${requiredText}`,
    );
  }
}
...
function assertNoForbiddenRuntimePatterns() {
  ...
  for (const filePath of readChangedFiles()) {
    ...
    if (
      ...
      filePath === perspectiveGeometryDigestSmokePath ||
      filePath === perspectiveGeometryDigestImplementationSmokePath ||
      filePath === perspectiveGeometryDigestBrowserValidationSmokePath ||
...
function assertImplementationFixture(value) {
  ...
  for (const flag of [
    ...
    "contract_layout_principles_preserved",
    "contract_stability_policy_preserved",
    "contract_source_balance_policy_preserved",
    "contract_candidate_overlay_policy_preserved",
    "contract_snapshot_policy_preserved",
    "contract_salience_policy_preserved",
    "contract_node_families_preserved",
    "contract_edge_families_preserved",
  ]) {
    assert.equal(value.implemented_contract[flag], true, `${flag} must be true`);
  }
  ...
  assert.equal(
    ...
    createProjectConstellationRuntimeLayoutFingerprint(value),
  );
}

function assertBuiltLayoutPreviewBundle(bundle) {
  assert.equal(bundle.preview_version, previewVersion);
  assert.equal(bundle.source_contract_ref, sourceContractRef);
  assert.equal(bundle.operator_context_ref, contractFixture.sample_project_constellation_layout_preview.operator_context_ref);
  assert.deepEqual(bundle.layout_input_preview, contractFixture.sample_project_constellation_layout_preview.layout_input_preview);
  assert.deepEqual(bundle.layout_preview, contractFixture.sample_project_constellation_layout_preview.layout_preview);
  assert.deepEqual(bundle.authority_boundary, contractFixture.sample_project_constellation_layout_preview.authority_boundary);
  assert.deepEqual(bundle.validation_policy, contractFixture.validation_policy);
  assert.deepEqual(bundle.stability_policy, contractFixture.stability_policy);
  assert.deepEqual(bundle.source_balance_policy, contractFixture.source_balance_policy);
  assert.deepEqual(bundle.candidate_overlay_policy, contractFixture.candidate_overlay_policy);
  ...
  const nodeKinds = bundle.layout_preview.nodes.map((node) => node.node_kind).sort();
  const edgeKinds = bundle.layout_preview.edges.map((edge) => edge.edge_kind).sort();
  assert.deepEqual(nodeKinds, contractFixture.node_families.map((family) => family.node_kind).sort());
  assert.deepEqual(edgeKinds, contractFixture.edge_families.map((family) => family.edge_kind).sort());
  assert.equal(bundle.layout_principle_summary.layout_is_interface_not_truth, true);
  assert.equal(bundle.node_family_summary.all_node_families_preserved, true);
  assert.equal(bundle.edge_family_summary.all_edge_families_preserved, true);
  assert.equal(bundle.stability_summary.layout_persistence_not_implemented, true);
  assert.equal(bundle.source_balance_summary.source_balance_not_promotion_authority, true);
  assert.equal(bundle.candidate_overlay_summary.candidate_overlay_not_durable_graph, true);
  assert.equal(bundle.snapshot_summary.snapshot_not_independent_source_of_truth, true);
...
function assertValidation(validation) {
  ...
  for (const flag of [
    ...
    "layout_input_fields_preserved",
    "layout_output_fields_preserved",
    "layout_principles_preserved",
    "node_families_preserved",
    "edge_families_preserved",
    ...
    assert.equal(validation[flag], true, `${flag} must be true`);
  }
}

function assertInvalidLayoutPreviewOverrideCoverage() {
  const invalidLayout = clone(contractFixture.sample_project_constellation_layout_preview.layout_preview);
  invalidLayout.layout_id = "";
  invalidLayout.nodes = [];
  invalidLayout.edges = [];
  invalidLayout.all_coordinates_not_truth = false;
  invalidLayout.runtime_write_now = true;
  invalidLayout.layout_persistence_now = true;
  invalidLayout.graph_mutation_now = true;
  delete invalidLayout.source_balance_summary;
  invalidLayout.candidate_overlay_and_durable_graph_distinct = false;
  invalidLayout.perspective_snapshot_source_of_truth_enabled = true;
  const invalid = buildProjectConstellationRuntimeLayoutImplementationFixture({
    project_constellation_runtime_layout_contract: contractFixture,
    source_contract_ref: sourceContractRef,
    layout_preview: invalidLayout,
  });
  assertInvalidFailureCodes(invalid, [
    "layout_preview_missing_layout_id",
    "layout_preview_missing_nodes",
    "layout_preview_missing_edges",
    "layout_preview_coordinates_as_truth",
    "layout_preview_runtime_write_enabled",
    "layout_preview_layout_persistence_enabled",
    "layout_preview_graph_mutation_enabled",
    "layout_preview_source_balance_missing",
    "layout_preview_candidate_overlay_not_distinct",
    "layout_preview_perspective_snapshot_source_of_truth_enabled",
  ]);
}

function assertInvalidNodeOverrideCoverage() {
  const invalidLayout = clone(contractFixture.sample_project_constellation_layout_preview.layout_preview);
  invalidLayout.nodes = [
    {
      node_ref: "",
      node_kind: "unknown_node_family",
      label_summary: "invalid node",
      source_refs: [],
      state_boundary: "candidate",
      runtime_write_now: true,
      public_safe: false,
      coordinate_truth_enabled: true,
    },
    {
      ...invalidLayout.nodes.find((node) => node.node_kind === "durable_perspective_node"),
      candidate_only: true,
    },
    {
      ...invalidLayout.nodes.find((node) => node.node_kind === "candidate_overlay_node"),
      candidate_only: false,
      visually_distinct_from_durable: false,
    },
    {
      ...invalidLayout.nodes.find((node) => node.node_kind === "tension_marker_node"),
      resolution_implied_by_layout: true,
    },
    {
      ...invalidLayout.nodes.find((node) => node.node_kind === "knowledge_gap_marker_node"),
      closure_implied_by_layout: true,
    },
    {
      ...invalidLayout.nodes.find((node) => node.node_kind === "evidence_anchor_node"),
  ...
  assertInvalidFailureCodes(invalid, [
    "node_missing_node_ref",
    "node_unknown_family_kind",
    "node_missing_source_refs",
    "node_missing_position_hint",
    "node_coordinate_truth_enabled",
    "node_runtime_write_enabled",
    "node_not_public_safe",
    "durable_node_marked_candidate_only",
    "candidate_overlay_node_not_candidate_only",
    "candidate_overlay_node_not_visually_distinct",
    "tension_marker_resolution_implied",
    "knowledge_gap_marker_closure_implied",
    "evidence_anchor_proof_write_enabled",
  ]);
}

function assertInvalidEdgeOverrideCoverage() {
  const invalidLayout = clone(contractFixture.sample_project_constellation_layout_preview.layout_preview);
  invalidLayout.edges = [
    {
      edge_ref: "",
      edge_kind: "unknown_edge_family",
      from_node_ref: "node_ref:public:from",
      to_node_ref: "node_ref:public:to",
      source_refs: [],
      relation_summary: "invalid edge",
      runtime_write_now: true,
      public_safe: false,
    },
    {
      ...invalidLayout.edges.find((edge) => edge.edge_kind === "candidate_overlay_link"),
      durable_graph_mutation_now: true,
    },
    {
      ...invalidLayout.edges.find((edge) => edge.edge_kind === "bridge_hint"),
      navigation_hint_only: false,
    },
    {
      ...invalidLayout.edges.find((edge) => edge.edge_kind === "tension_line"),
      resolution_implied: true,
    },
    {
      ...invalidLayout.edges.find((edge) => edge.edge_kind === "knowledge_gap_line"),
      closure_implied: true,
    },
    {
      ...invalidLayout.edges.find((edge) => edge.edge_kind === "supports_ref"),
      proof_write_now: true,
    },
  ];
  const invalid = buildProjectConstellationRuntimeLayoutImplementationFixture({
  ...
  assertInvalidFailureCodes(invalid, [
    "edge_missing_edge_ref",
    "edge_unknown_family_kind",
    "edge_missing_source_refs",
    "edge_runtime_write_enabled",
    "edge_not_public_safe",
    "candidate_overlay_edge_mutates_durable_graph",
    "bridge_edge_not_navigation_hint",
    "tension_edge_resolution_implied",
    "knowledge_gap_edge_closure_implied",
    "evidence_edge_proof_write_enabled",
  ]);
}
...
function assertInvalidAuthorityBoundaryOverrideCoverage() {
  const invalid = buildProjectConstellationRuntimeLayoutImplementationFixture({
    ...
    authority_boundary_overrides: {
      runtime_layout_implemented_now: true,
      seeded_layout_runtime_now: true,
      force_directed_layout_runtime_now: true,
      temporal_smoothing_runtime_now: true,
      layout_persistence_now: true,
      layout_coordinate_write_now: true,
      ...
      crawler_now: true,
      layout_coordinate_authority: true,
      manual_anchor_authority: true,
      cluster_position_authority: true,
  ...
  assertInvalidFailureCodes(invalid, [
    "runtime_layout_enabled",
    "seeded_layout_runtime_enabled",
    "force_directed_layout_runtime_enabled",
    "temporal_smoothing_runtime_enabled",
    "layout_persistence_enabled",
    "layout_coordinate_write_enabled",
    "graph_db_enabled",
    ...
    "crawler_enabled",
    "layout_coordinate_authority_enabled",
    "manual_anchor_authority_enabled",
    "cluster_position_authority_enabled",
...
function assertInvalidRefsOverrideCoverage() {
  const invalidInput = clone(contractFixture.sample_project_constellation_layout_preview.layout_input_preview);
  invalidInput.source_refs = [];
  invalidInput.layout_scope_ref = "layout_scope_ref:private:unstable_http://localhost";
  invalidInput.raw_private_source_body = "raw_private_source_body";
  invalidInput.provider_thread_ref = "thread_abc123";
  const invalidLayout = clone(contractFixture.sample_project_constellation_layout_preview.layout_preview);
  invalidLayout.layout_id = "";
  invalidLayout.evidence_rays = [
    {
      evidence_ray_ref: "evidence_ray_ref:public:invalid",
      from_evidence_node_ref: "node_ref:public:evidence_anchor",
      to_claim_node_ref: "node_ref:public:claim",
      source_refs: ["source_ref:public:project_constellation_runtime_layout_contract"],
      evidence_ray_is_ref_not_proof: true,
      proof_write_now: true,
    },
  ];
  invalidLayout.nodes = invalidLayout.nodes.map((node) =>
    node.node_kind === "source_reference_node"
      ? {
          ...node,
          raw_source_body_allowed: true,
          raw_source_body: "raw_private_source_body",
        }
      : node,
  );
  const invalid = buildProjectConstellationRuntimeLayoutImplementationFixture({
    project_constellation_runtime_layout_contract: contractFixture,
    source_contract_ref: sourceContractRef,
    layout_input_preview: invalidInput,
    layout_preview: invalidLayout,
  });
  assertInvalidFailureCodes(invalid, [
    "layout_id_missing",
    ...
    "evidence_ray_missing_evidence_ref",
    "evidence_ray_proof_write_enabled",
    "source_reference_node_raw_body_enabled",
  ]);
}

function assertInvalidFailureCodes(invalidFixture, expectedCodes) {
  const validation = invalidFixture.validated_implementation;
  assert.equal(validation.passed, false, "invalid override must fail validation");
  for (const code of expectedCodes) {
    assert.ok(
      validation.failure_codes.includes(code),
      `invalid override must fail with ${code}; got ${validation.failure_codes.join(", ")}`,
    );
  }
}
...
function assertPortableMergeBaseFallback() {
  assert.ok(mergeBaseRef(), "mergeBaseRef must resolve a portable merge base");
  assert.match(smokeSource, /origin\/main/);
  assert.match(smokeSource, /local main/);
  assert.match(smokeSource, /HEAD\^/);
}
...
function mergeBaseRef() {
  ...
  for (const candidate of [
    ...
    try {
      cachedMergeBaseRef = readGitOutput(candidate);
      return cachedMergeBaseRef;
    } catch {
  ...
  try {
    cachedMergeBaseRef = readGitOutput(["rev-parse", "HEAD^"]);
    return cachedMergeBaseRef;
  } catch (error) {
    throw new Error(
      `Unable to resolve merge base from origin/main, local main, or HEAD^: ${error}`,
    );
  }
}
