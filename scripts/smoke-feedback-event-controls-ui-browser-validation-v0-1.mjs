...
const listRouteBrowserValidationSmokeSource = existsSync(
  listRouteBrowserValidationSmokePath,
)
  ? readFileSync(listRouteBrowserValidationSmokePath, "utf8")
  : "";
...
function assertStaticBoundary() {
  ...
  const activeRequiredChangedFiles = downstreamAggregationReadModelBrowserValidationSliceActive()
...
function assertListRouteBrowserValidationDownstreamPointer() {
  ...
  for (const requiredText of [
    ...
    assert.ok(
      ...
      `#702 UI browser validation smoke must allow list route browser validation text: ${requiredText}`,
    );
  }
...
function assertListUiContractDownstreamPointer() {
  ...
  for (const requiredText of [
    ...
    assert.ok(
      ...
      `#702 UI browser validation smoke must allow list UI contract text: ${requiredText}`,
    );
  }
  ...
  for (const requiredText of [
    ...
    assert.ok(
      readFileSync(listUiImplementationSmokePath, "utf8").includes(requiredText) ||
        listUiContractSmokeSource.includes(requiredText),
      `#702 UI browser validation smoke must allow list UI implementation text: ${requiredText}`,
    );
  }
