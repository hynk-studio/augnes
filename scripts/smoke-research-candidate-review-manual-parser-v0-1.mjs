...
function assertDocsPointers() {
  ...
  assert.match(
    ...
    expectedNextStep,
    "surface doc next step must point to parser output Cockpit/Perspective static preview panel",
  );
  assert.match(
...
function assertExistingSmokeAlignment() {
  for (const [label, source] of [
    ...
    assert.match(
      source,
      /parser output Cockpit\/Perspective static preview panel/i,
      `${label} must expect the parser output Cockpit/Perspective static preview panel next step`,
    );
  }
}
