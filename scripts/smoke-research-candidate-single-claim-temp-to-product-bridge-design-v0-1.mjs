...
function assertNoRouteUiSchemaDependencyExpansion() {
  ...
  for (const line of addedPackageLines) {
    assert.ok(
      ...
      `package.json must only add temp-to-product bridge design, disabled bridge skeleton, dry-run harness, authority bundle, disabled adapter skeleton, disabled adapter contract-test, or disabled adapter dry-run invocation harness scripts, not dependencies: ${line}`,
    );
  }
}
