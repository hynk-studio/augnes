...
function assertNoRouteUiSchemaDependencyExpansion() {
  ...
  for (const line of addedPackageLines) {
    assert.ok(
      ...
      `package.json must only add product write gate-design, temp-to-product bridge design, disabled bridge skeleton, dry-run harness, authority bundle, or disabled adapter skeleton scripts, not dependencies: ${line}`,
    );
  }
}
