...
const files = {
  previewType: "types/perspective-unit-scoped-write-preview.ts",
  previewHelper: "lib/workplane/perspective-unit-scoped-write-preview.ts",
  previewPanel:
    "components/workplane/perspective-unit-scoped-write-preview-panel.tsx",
  writeType: "types/perspective-unit-write.ts",
  writeHelper: "lib/workplane/perspective-unit-write.ts",
  route: "app/api/workplane/perspective-units/route.ts",
  reviewType: "types/perspective-unit-record-review.ts",
  reviewHelper: "lib/workplane/perspective-unit-record-review.ts",
  reviewForWeb:
  ...
  perspectiveRelayUpdateSmoke:
    "scripts/smoke-perspective-relay-update-decision-write-contract-v0-1.mjs",
  smoke: "scripts/smoke-perspective-unit-scoped-write-v0-1.mjs",
...
assertPackageScript({
  packageJsonText,
  scriptName: "smoke:perspective-unit-scoped-write-v0-1",
  expectedCommand:
    "tsx --tsconfig tsconfig.json scripts/smoke-perspective-unit-scoped-write-v0-1.mjs",
});
...
assertContainsAll(
  [
    ...
    .join("\n"),
  [
    "perspective_unit_scoped_write_preview.v0.1",
    "perspective_unit_record.v0.1",
    "perspective_unit_receipt.v0.1",
    "perspective_unit_store.v0.1",
    "perspective_unit_record_review.v0.1",
  ],
  { label: "perspective unit scoped write files" },
);
...
const biasWrite = await import(
  "../lib/workplane/perspective-unit-write.ts"
);
const { buildPerspectiveUnitRecordReviewV01 } = await import(
...
const route = await import(
  "../app/api/workplane/perspective-units/route.ts"
);
...
const emptyPreview = buildPerspectiveUnitScopedWritePreviewV01();
assert.equal(
...
const nonReadyContractPreview = buildPerspectiveUnitScopedWritePreviewV01({
  perspective_relay_update_write_contract_preview: {
    ...clone(readyContract),
    contract_preview_status: "future_write_contract_candidates_available",
  },
  perspective_relay_update_decision_record_review: relayRecordReview,
  requested_operator_ref: "operator:perspective-unit-reviewer",
  requested_idempotency_key: "idempotency:perspective-unit",
  review_confirmation_ref: "review:perspective-unit",
  source_refs: ["source:perspective-unit"],
});
assert.notEqual(
...
const noPerspectiveUnitContract = clone(readyContract);
noPerspectiveUnitContract.selected_candidate_refs_by_target.perspective_unit = [];
noPerspectiveUnitContract.proposed_future_write_contract.perspective_unit_update_contract.selected_candidate_refs = [];
noPerspectiveUnitContract.input_summary.selected_perspective_unit_candidate_count = 0;
const noPerspectiveUnitPreview = buildPerspectiveUnitScopedWritePreviewV01({
  perspective_relay_update_write_contract_preview: noPerspectiveUnitContract,
  perspective_relay_update_decision_record_review: relayRecordReview,
  requested_operator_ref: "operator:perspective-unit-reviewer",
  requested_idempotency_key: "idempotency:perspective-unit",
  review_confirmation_ref: "review:perspective-unit",
...
assert.notEqual(
  noPerspectiveUnitPreview.scoped_write_preview_status,
  "ready_for_future_perspective_unit_record_write",
);
assert.equal(noPerspectiveUnitPreview.input_summary.selected_perspective_unit_candidate_count, 0);
assert(noPerspectiveUnitPreview.input_summary.non_writable_next_work_bias_candidate_count > 0);

const notInContract = clone(readyContract);
notInContract.selected_candidate_refs_by_target.perspective_unit = [
  "candidate:perspective-not-in-contract",
];
const notInContractPreview = buildPerspectiveUnitScopedWritePreviewV01({
...
assert(
  notInContractPreview.refusal_reasons.includes(
    "selected_perspective_unit_refs_not_in_contract",
  ),
);

const notInRecord = clone(readyContract);
notInRecord.selected_candidate_refs_by_target.perspective_unit = [
  "candidate:perspective-not-in-record",
];
notInRecord.proposed_future_write_contract.perspective_unit_update_contract.selected_candidate_refs =
  ["candidate:perspective-not-in-record"];
const notInRecordPreview = buildPerspectiveUnitScopedWritePreviewV01({
...
assert(
  notInRecordPreview.refusal_reasons.includes(
    "selected_perspective_unit_refs_not_in_decision_record",
  ),
);

const readyUnitPreview = buildPerspectiveUnitScopedWritePreviewV01({
...
assert.equal(
  readyUnitPreview.scoped_write_preview_status,
  "ready_for_future_perspective_unit_record_write",
);
assert.equal(readyUnitPreview.write_readiness.write_ready, true);
assert.equal(
  readyUnitPreview.recommended_next_action,
  "write_perspective_unit_record",
);
assert.equal(readyUnitPreview.perspective_unit_entries.length, 1);
assert.equal(readyUnitPreview.authority_boundary.can_write_next_work_bias, false);
assert.equal(readyUnitPreview.authority_boundary.can_write_perspective_unit, false);
assert.equal(readyUnitPreview.related_next_work_bias_record_refs.length, 0);
assert.equal(
  readyUnitPreview.authority_boundary.can_update_current_working_perspective,
  false,
);
assert.equal(readyUnitPreview.authority_boundary.can_update_continuity_relay, false);
const readyUnitPreviewWithBiasContext = buildPerspectiveUnitScopedWritePreviewV01({
  perspective_relay_update_write_contract_preview: readyContract,
  perspective_relay_update_decision_record_review: relayRecordReview,
  perspective_relay_update_candidate_bridge_preview: bridgePreview,
...
assert.equal(
  readyUnitPreviewWithBiasContext.scoped_write_preview_status,
  "ready_for_future_perspective_unit_record_write",
);
assert.deepEqual(readyUnitPreviewWithBiasContext.related_next_work_bias_record_refs, [
  "perspective-next-work-bias:advisory",
]);

const unitApproval = {
  operator_decision: "approve_for_perspective_unit_record",
  approved_by: "operator:perspective-unit-reviewer",
  operator_ref: "operator:perspective-unit-reviewer",
  approved_at: "2026-07-05T00:00:02.000Z",
  approval_statement: "approved bounded local perspective unit record",
  checklist_confirmations: ["check:reviewed-perspective-unit-entries"],
};
const validInput = {
  scoped_write_preview: readyUnitPreview,
  operator_approval: unitApproval,
  idempotency_key: "idempotency:perspective-unit",
};

let db = new Database(":memory:");
let refused = biasWrite.writePerspectiveUnitRecordV01(
  {
    scoped_write_preview: noPerspectiveUnitPreview,
    operator_approval: unitApproval,
...
assert.equal(biasWrite.perspectiveUnitWriteSchemaExistsV01(db), false);
db.close();
...
for (const [label, mutatedInput, expectedReason] of [
  ...
  [
    ...
    () => ({
      ...clone(validInput),
      requested_side_effects: { next_work_bias_written: true },
    }),
  ...
  [
    ...
    () => {
      ...
      input.scoped_write_preview.would_write_perspective_unit_record_preview.perspective_unit_entries[0].lifecycle_directive =
        "reinforce";
      input.scoped_write_preview.would_write_perspective_unit_record_preview.perspective_unit_entries[0].bucket =
        "perspective_unit_weaken_or_warn_candidates";
      return input;
    },
    "perspective_unit_entry_lifecycle_directive_bucket_mismatch",
  ],
  [
    "forged next work bias authority",
    () => {
      const input = clone(validInput);
      input.scoped_write_preview.authority_boundary.can_write_next_work_bias = true;
      return input;
    },
  ...
  const result = biasWrite.writePerspectiveUnitRecordV01(mutatedInput(), {
    db,
  });
  assert.equal(result.status, "refused", label);
  assert(result.receipt.refusal_reasons.includes(expectedReason), label);
  assert.equal(biasWrite.perspectiveUnitWriteSchemaExistsV01(db), false, label);
  db.close();
}

db = new Database(":memory:");
assert.equal(
  biasWrite.readPerspectiveUnitRecordByIdV01("record:none", { db }).status,
  "schema_missing",
);
assert.equal(biasWrite.perspectiveUnitWriteSchemaExistsV01(db), false);
const written = biasWrite.writePerspectiveUnitRecordV01(validInput, {
  db,
});
assert.equal(written.status, "written");
assert.equal(written.record?.record_version, "perspective_unit_record.v0.1");
assert.equal(written.receipt.receipt_version, "perspective_unit_receipt.v0.1");
assert.equal(written.no_side_effects.perspective_unit_record_written, true);
assert.equal(written.no_side_effects.perspective_unit_receipt_written, true);
assert.equal(written.no_side_effects.perspective_unit_persisted, true);
assert.equal(written.no_side_effects.perspective_unit_written, true);
assert.equal(written.no_side_effects.next_work_bias_written, false);
for (const field of forbiddenNoSideEffectFields) {
  assert.equal(written.no_side_effects[field], false, field);
}
const replay = biasWrite.writePerspectiveUnitRecordV01(validInput, {
  db,
});
assert.equal(replay.status, "idempotent_existing");
const conflict = biasWrite.writePerspectiveUnitRecordV01(
  {
    ...clone(validInput),
    operator_approval: {
      ...unitApproval,
...
assert.equal(
  biasWrite.readPerspectiveUnitRecordByIdV01(written.record.record_id, {
    db,
  }).status,
  "read",
);
assert.equal(
  biasWrite.readPerspectiveUnitRecordByIdempotencyKeyV01(
    written.record.idempotency_key,
    { db },
  ).status,
  "read",
);
assert.equal(biasWrite.listPerspectiveUnitRecordsV01({ db }).records.length, 1);

const outOfScopeRecord = {
  ...written.record,
  record_id: "perspective-unit:out-of-scope",
...
assert.equal(
  biasWrite.readPerspectiveUnitRecordByIdV01(outOfScopeRecord.record_id, {
    db,
  }).status,
  "not_found",
);
...
biasWrite.ensurePerspectiveUnitWriteSchemaV01(replayScopeDb);
const outOfScopeReplayRecord = {
  ...written.record,
  record_id: "perspective-unit:out-of-scope-replay",
  scope: "project:not-augnes",
};
...
const outOfScopeReplay = biasWrite.writePerspectiveUnitRecordV01(
  validInput,
  { db: replayScopeDb },
);
...
const noRecordReview = buildPerspectiveUnitRecordReviewV01();
assert.equal(noRecordReview.review_status, "no_records");
const recordReview = buildPerspectiveUnitRecordReviewV01({
  store_result: written,
});
assert.equal(recordReview.review_status, "records_available");
assert.equal(recordReview.perspective_unit_material_summary.perspective_unit_entry_count, 1);
assert.equal(recordReview.receipt_no_side_effects_summary.perspective_unit_written_count > 0, true);
assert.equal(recordReview.receipt_no_side_effects_summary.next_work_bias_written_count, 0);
const malformedReview = buildPerspectiveUnitRecordReviewV01({
  records: [
    {
      record_version: "perspective_unit_record.v0.1",
      record_id: "malformed",
...
assert(
  malformedReview.record_summaries[0].problem_reasons.includes(
    "perspective_unit_record_malformed",
  ),
);
const rawMaterialReview = buildPerspectiveUnitRecordReviewV01({
  records: [
    {
      ...clone(written.record),
      record_id: "perspective-unit:raw-material",
...
const corruptReview = buildPerspectiveUnitRecordReviewV01({
...
assert(
  corruptReview.blocked_reasons.includes(
    "perspective_unit_receipt_no_side_effects_invalid",
  ),
);
...
const overviewBeforeWrite = buildWorkbenchDogfoodLoopSpineOverviewV01({
  ...
  perspective_relay_update_decision_record_review: relayRecordReview,
  perspective_relay_update_write_contract_preview: readyContract,
  perspective_unit_scoped_write_preview: readyUnitPreview,
  perspective_unit_record_review: noRecordReview,
});
for (const stepId of [
  "perspective_unit_scoped_write",
  "perspective_unit_record",
...
const overviewAfterWrite = buildWorkbenchDogfoodLoopSpineOverviewV01({
  ...
  perspective_relay_update_write_contract_preview: readyContract,
  perspective_unit_scoped_write_preview: readyUnitPreview,
  perspective_unit_record_review: recordReview,
});
assert.equal(
...
assertChangedFilesWithin({
  label: "perspective unit scoped write v0.1",
  allowedChangedFiles: Object.values(files),
});

console.log("PASS smoke:perspective-unit-scoped-write-v0-1");

async function assertRouteBoundaries({ route, validInput }) {
  const tempRoot = ".tmp/perspective-units";
  rmSync(tempRoot, { recursive: true, force: true });
  mkdirSync(tempRoot, { recursive: true });
  const invalidPath = `${tempRoot}/invalid-write.sqlite`;
  let response = await route.GET(
    new Request(
      "http://localhost/api/workplane/perspective-units?db_path=/tmp/private.sqlite",
    ),
  );
  assert.equal(response.status, 400);
  response = await route.GET(
    new Request(
      "http://localhost/api/workplane/perspective-units?db_path=.tmp/perspective-units/missing.sqlite",
    ),
  );
  assert.equal(response.status, 404);
  response = await route.POST(
    new Request("http://localhost/api/workplane/perspective-units", {
      method: "POST",
  ...
  routeInput.idempotency_key = "idempotency:route-perspective-unit";
  routeInput.scoped_write_preview.would_write_perspective_unit_record_preview.requested_idempotency_key =
    "idempotency:route-perspective-unit";
  response = await route.POST(
    new Request("http://internal.local/api/workplane/perspective-units", {
      method: "POST",
      headers: {
  ...
  assert.equal(body.perspective_unit_record_written, true);
  assert.equal(body.perspective_unit_written, true);
  assert.equal(body.next_work_bias_written, false);
  assert.equal(body.no_side_effects.perspective_unit_record_written, true);
  assert.equal(body.no_side_effects.perspective_unit_written, true);
  assert.equal(body.no_side_effects.next_work_bias_written, false);
  for (const field of forbiddenNoSideEffectFields) {
  ...
  response = await route.GET(
    new Request(
      `http://localhost/api/workplane/perspective-units?db_path=${validPath}&record_id=${encodeURIComponent(recordId)}`,
    ),
  );
  assert.equal(response.status, 200);
  response = await route.GET(
    new Request(
      `http://localhost/api/workplane/perspective-units?db_path=${validPath}&idempotency_key=${encodeURIComponent(routeInput.idempotency_key)}`,
    ),
  );
  assert.equal(response.status, 200);
  rmSync(tempRoot, { recursive: true, force: true });
