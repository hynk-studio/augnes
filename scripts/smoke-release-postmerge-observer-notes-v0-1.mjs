...
const releaseCandidateFreezeManifestDocsPath =
  "docs/RELEASE_CANDIDATE_FREEZE_MANIFEST_V0_1.md";
const docsPath = "docs/RELEASE_POSTMERGE_OBSERVER_NOTES_V0_1.md";
const helperPath = "lib/release-readiness/release-postmerge-observer-notes.ts";
const fixturePath = "fixtures/release-postmerge-observer-notes.sample.v0.1.json";
const packagePath = "package.json";
const indexPath = "docs/00_INDEX_LATEST.md";

const observerVersion = "release_postmerge_observer_notes.v0.1";
const inputVersion = "release_postmerge_observer_input.v0.1";
const resultVersion = "release_postmerge_observer_result.v0.1";
const noteVersion = "release_postmerge_observer_note.v0.1";
const fixtureVersion = "release_postmerge_observer_notes.sample.v0.1";
const scope = "project:augnes";
const packageScriptName = "smoke:release-postmerge-observer-notes-v0-1";
const packageScriptValue = "node scripts/smoke-release-postmerge-observer-notes-v0-1.mjs";
...
const allowedFixturePlaceholders = [
  "raw postmerge observer payload blocked by fixture",
  "raw conversation blocked by postmerge observer fixture",
  "hidden reasoning blocked by postmerge observer fixture",
  "telemetry dump blocked by postmerge observer fixture",
...
const releaseCandidateFreezeManifestDocs = read(releaseCandidateFreezeManifestDocsPath);
...
assert.ok(
  roadmap.includes("release_postmerge_observer_notes_v0_1"),
  "roadmap must contain release_postmerge_observer_notes_v0_1",
);
assert.ok(
  releaseCandidateFreezeManifestDocs.includes(
    "Release Candidate Freeze Manifest is review-only.",
  ),
  "Release Candidate Freeze Manifest docs must preserve review-only wording",
);

assert.equal(fixture.fixture_version, fixtureVersion);
assert.equal(fixture.observer_version, observerVersion);
assert.equal(fixture.input_version, inputVersion);
assert.equal(fixture.result_version, resultVersion);
assert.equal(fixture.note_version, noteVersion);
assert.equal(fixture.scope, scope);

assert.equal(
  packageJson.scripts?.[packageScriptName],
  packageScriptValue,
  "package.json must register the release postmerge observer notes smoke",
);
...
for (const value of [...decisions, ...noteKinds, ...severities, ...statuses]) {
...
assert.ok(
  helperSource.includes('collectForbiddenAuthorityObjectFailures(input, "note")'),
  "note validator must recursively scan forbidden authority fields",
);
...
assert.ok(
  helperSource.includes("duplicateNoteIdFailureCodes"),
  "input validator must include duplicate note_id validation",
);
assert.ok(
  helperSource.includes("duplicate_note_id"),
  "duplicate note_id validation must expose a stable failure code",
);
assert.ok(
...
assert.ok(
  helperSource.includes("buildReleasePostmergeObserverNotesV01"),
  "helper must export builder",
);
assert.ok(
  helperSource.includes("validateReleasePostmergeObserverInputV01"),
...
const helper = await importReleasePostmergeObserverHelper();

assert.deepEqual(
  helper.buildReleasePostmergeObserverNotesV01(fixture.expected_valid_input),
  fixture.expected_result,
  "valid release postmerge observer notes output must match expected_result",
);
assert.equal(fixture.expected_result.status, "built");
assert.equal(fixture.expected_result.decision, "observer_notes_candidate_only");
assertResultAuthorityClosed(fixture.expected_result);

const repeated = helper.buildReleasePostmergeObserverNotesV01(fixture.expected_valid_input);
assert.equal(
  repeated.observer_fingerprint,
  fixture.expected_result.observer_fingerprint,
  "repeated build must keep the same fingerprint",
);

assert.deepEqual(
  helper.buildReleasePostmergeObserverNotesV01(fixture.expected_empty_input),
  fixture.expected_empty_result,
  "empty release postmerge observer notes output must match expected_empty_result",
);
...
for (const [caseName, input] of Object.entries(fixture.decision_coverage_inputs)) {
  const actual = helper.buildReleasePostmergeObserverNotesV01(input);
...
const observedNoteKinds = new Set();
const observedSeverities = new Set();
for (const input of [
  fixture.expected_valid_input,
  ...Object.values(fixture.decision_coverage_inputs),
]) {
  for (const note of input.input_notes) {
    observedNoteKinds.add(note.note_kind);
    observedSeverities.add(note.severity);
  }
}
for (const noteKind of noteKinds) {
  assert.ok(observedNoteKinds.has(noteKind), `fixture must cover note kind ${noteKind}`);
}
...
assertDecisionCase(
  "missing_freeze_manifest_note_blocked",
  "blocked",
  "release-postmerge-observer-note:missing:freeze_manifest",
  "mandatory_observer_note_missing",
);
assertDecisionCase(
  "missing_merge_boundary_note_blocked",
  "blocked",
  "release-postmerge-observer-note:missing:merge_boundary",
  "mandatory_observer_note_missing",
);
assertDecisionCase(
  "missing_release_boundary_note_blocked",
  "blocked",
  "release-postmerge-observer-note:missing:release_boundary",
  "mandatory_observer_note_missing",
);
assertDecisionCase(
  "missing_product_write_boundary_note_blocked",
  "blocked",
  "release-postmerge-observer-note:missing:product_write_boundary",
  "mandatory_observer_note_missing",
);
assert.equal(
  fixture.expected_decision_results.excluded_blocking_note_blocked.decision,
  "blocked",
);
assert.ok(
  fixture.expected_decision_results.excluded_blocking_note_blocked.reason_codes.includes(
    "blocking_note_present",
  ),
  "excluded blocking note must record blocking_note_present",
);
assert.equal(
  fixture.expected_decision_results.excluded_warning_note_needs_operator_review
    .decision,
  "needs_operator_review",
);
assert.equal(
  fixture.expected_decision_results.unknown_note_kind_rejected.decision,
  "rejected",
);
assert.equal(
...
for (const [caseName, input] of Object.entries(fixture.invalid_inputs)) {
  ...
  assert.equal(actual.notes.length, 0, `${caseName} must not include output notes`);
  assertResultAuthorityClosed(actual);
...
assert.equal(
  fixture.expected_rejection_results.public_safe_false_note.status,
  "blocked_invalid_input",
);
assert.equal(
...
assert.equal(
  fixture.expected_rejection_results.unknown_note_level_authority_true.status,
  "blocked_invalid_input",
);
for (const caseName of [
  "unknown_top_level_actual_postmerge_observed_true",
  "unknown_top_level_merge_executed_true",
...
for (const caseName of [
  ...
  assert.equal(
    fixture.expected_rejection_results[caseName].status,
    "blocked_invalid_input",
    `${caseName} must reject duplicate note IDs before observer build`,
  );
  assert.equal(
    fixture.expected_rejection_results[caseName].decision,
    "rejected",
    `${caseName} must not produce observer_notes_candidate_only`,
  );
}

const publicSafeHyphenInput = structuredClone(fixture.expected_valid_input);
publicSafeHyphenInput.observer_id = "release-postmerge-observer-notes:hyphen-safe";
publicSafeHyphenInput.input_notes[0].bounded_summary =
  "Public-safe risk-reduction and task-level release postmerge observer notes summary.";
const publicSafeHyphenResult =
  helper.buildReleasePostmergeObserverNotesV01(publicSafeHyphenInput);
assert.equal(
  ...
  JSON.stringify(
    {
      ...
      verified: {
        valid_result: fixture.expected_result.observer_fingerprint,
        empty_result: fixture.expected_empty_result.observer_fingerprint,
        decisions: [...observedDecisions].sort(),
        note_kinds: [...observedNoteKinds].sort(),
...
function assertDecisionCase(caseName, decision, missingRef, reasonCode) {
  ...
  assert.ok(
    result.missing_note_refs.includes(missingRef),
...
function assertResultAuthorityClosed(result) {
  assert.equal(
    result.actual_postmerge_observed,
    false,
    "actual_postmerge_observed must be false",
  );
  assert.equal(result.merge_executed, false, "merge_executed must be false");
  ...
  for (const note of result.notes ?? []) {
    assert.equal(note.public_safe, true, "result note must be public safe");
    assertAuthorityBoundary(note.authority_boundary);
  }
}

function assertAuthorityBoundary(authorityBoundary) {
  assert.equal(
    authorityBoundary.release_postmerge_observer_notes_now,
    true,
    "release_postmerge_observer_notes_now must be true",
  );
...
function assertIndexDoesNotImplyForbiddenAuthority() {
  const forbiddenIndexImplications = [
    "actual postmerge observation was added",
    "merge execution was added",
    "Git execution was added",
    "GitHub calls were added",
    "repository file writes were added",
...
async function importReleasePostmergeObserverHelper() {
