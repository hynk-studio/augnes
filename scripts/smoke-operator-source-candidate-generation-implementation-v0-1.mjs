...
const implementationKind = "operator_source_candidate_generation_implementation";
...
assert.deepEqual(
  ...
  "rebuilt Operator Source Candidate Generation implementation fixture must match committed fixture",
);
assertImplementationFixture(fixture);
assertGeneratedCandidateGenerationPreviewBundle(
  fixture.generated_candidate_generation_preview_bundle,
);
assertCandidatePreviewFamilySummary(fixture.candidate_preview_family_summary);
assertGeneratedCandidateSummary(fixture.generated_candidate_summary);
assertSourceReferenceSummary(fixture.source_reference_summary);
assertProvenanceSummary(fixture.provenance_summary);
assertReviewSummary(fixture.review_summary);
  ...
  JSON.stringify(
    {
      ...
      generated_candidate_count:
        fixture.generated_candidate_summary.generated_candidate_count,
      generated_candidates_preview_only:
        fixture.validation.generated_candidates_preview_only,
      ...
      candidate_record_write_not_implemented:
        fixture.validation.candidate_record_write_not_implemented,
...
function assertStaticBoundary() {
  ...
  for (const changedFile of changedFiles) {
    assert.ok(
      ...
      `unexpected changed file in Operator Source Candidate Generation implementation slice: ${changedFile}`,
    );
...
function assertNoForbiddenRuntimePatterns() {
  ...
  for (const filePath of changedSourceFiles) {
    ...
    for (const { label, regex } of [
      ...
      { label: "candidate runtime generation", regex: /\brunCandidateGeneration\b|\bcreateCandidateRecord\b|\bwriteCandidateRecord\b/i },
...
function assertImplementationFixture(implementation) {
  ...
  assert.equal(
    implementation.source_bounded_external_source_intake_validation_ref,
    contractFixture.source_bounded_external_source_intake_validation_ref,
  );
  for (const requiredField of [
    ...
    assert.ok(Object.hasOwn(implementation, requiredField), `implementation must include ${requiredField}`);
  }
...
function assertGeneratedCandidateGenerationPreviewBundle(bundle) {
  ...
  assert.equal(
    ...
    sample.source_intake_bundle_ref,
  );
  ...
  assert.ok(bundle.generated_candidate_previews.length >= 6);
  ...
  assert.equal(bundle.authority_boundary.contract_only, true);
  assert.equal(
    bundle.authority_boundary.operator_source_candidate_generation_contract_defined_now,
    true,
  );
...
function assertCandidatePreviewFamilySummary(summary) {
...
function assertGeneratedCandidateSummary(summary) {
  const familyKinds = contractFixture.candidate_preview_families.map(
    (family) => family.family_kind,
  );
  assert.ok(summary.generated_candidate_count >= familyKinds.length);
  assert.ok(summary.generated_candidate_preview_ids.length >= familyKinds.length);
  ...
  assert.equal(summary.all_generated_candidate_source_refs_resolve_to_bundle_refs, true);
...
function assertSourceReferenceSummary(summary) {
  assert.equal(
    summary.source_ref_count,
    fixture.generated_candidate_generation_preview_bundle.source_refs.length,
  );
...
function assertProvenanceSummary(summary) {
  assert.deepEqual(summary, contractFixture.provenance_policy);
}

function assertReviewSummary(summary) {
  assert.deepEqual(summary, contractFixture.review_policy);
  assert.equal(summary.human_review_required_later, true);
  assert.equal(summary.promotion_requires_later_contract, true);
  assert.equal(summary.proof_evidence_requires_later_gate, true);
  assert.equal(summary.work_creation_requires_later_contract, true);
  assert.equal(summary.product_write_requires_later_contract, true);
}

function assertPrivacySummary(summary) {
  assert.deepEqual(summary, contractFixture.privacy_policy);
  assert.equal(summary.no_secrets_in_fixture, true);
  assert.equal(summary.no_raw_oauth_tokens, true);
  assert.equal(summary.no_private_urls_in_fixture, true);
  assert.equal(summary.no_provider_ids_as_canonical_labels, true);
  assert.equal(summary.no_thread_run_session_ids_as_canonical_labels, true);
}

function assertNonAuthoritySummary(summary) {
  assert.deepEqual(summary, contractFixture.non_authority_policy);
  assert.equal(summary.not_source_of_truth, true);
  assert.equal(summary.not_proof_or_evidence, true);
  assert.equal(summary.not_perspective_state, true);
  assert.equal(summary.not_work_status, true);
  assert.equal(summary.not_promotion_basis, true);
  assert.equal(summary.not_retrieval_rag_result, true);
  assert.equal(summary.not_salience_authority, true);
  assert.equal(summary.not_product_write, true);
}

function assertAuthorityBoundary(boundary) {
  assert.equal(boundary.implementation_added_now, true);
  assert.equal(boundary.contract_followed_now, true);
  assert.equal(boundary.fixture_backed_only, true);
...
function assertValidation(validation) {
  ...
  assert.equal(validation.generated_bundle_follows_contract, true);
  assert.equal(validation.generated_bundle_boundary_matches_contract, true);
  assert.equal(validation.generated_bundle_validation_matches_contract, true);
  assert.equal(validation.top_level_implementation_boundary_is_separate, true);
  assert.equal(validation.candidate_preview_families_preserved, true);
  assert.equal(validation.generated_candidates_preview_only, true);
  assert.equal(validation.generated_candidates_have_source_refs, true);
  assert.equal(validation.generated_candidates_source_refs_resolve_to_bundle_refs, true);
  assert.equal(validation.generated_candidates_have_operator_context, true);
  assert.equal(validation.generated_candidates_review_required_later, true);
  assert.equal(validation.generated_candidates_durable_write_now_false, true);
  assert.equal(validation.generated_candidates_not_proof_or_evidence, true);
  assert.equal(validation.generated_candidates_not_source_of_truth, true);
  assert.equal(validation.generated_candidates_not_perspective_state, true);
  assert.equal(validation.generated_candidates_not_work_status, true);
  assert.equal(validation.generated_candidates_not_product_write, true);
  assert.equal(validation.source_refs_reference_only, true);
  assert.equal(validation.source_fetch_not_implemented, true);
  assert.equal(validation.provider_extraction_not_implemented, true);
  assert.equal(validation.retrieval_rag_not_implemented, true);
  assert.equal(validation.candidate_generation_not_runtime, true);
  assert.equal(validation.candidate_record_write_not_implemented, true);
  assert.equal(validation.candidate_mutation_not_implemented, true);
  assert.equal(validation.privacy_policy_preserved, true);
  assert.equal(validation.non_authority_policy_preserved, true);
  assert.equal(validation.authority_boundary_preserved, true);
...
function assertInvalidGeneratedCandidatePreviewOverride() {
  ...
  const invalidPreviews = [
    { ...basePreview, candidate_preview_id: "invalid_unknown_family", family_kind: "unknown_candidate_preview" },
    { ...basePreview, candidate_preview_id: "invalid_missing_source_refs", source_refs: [] },
    { ...basePreview, candidate_preview_id: "invalid_unknown_source_ref", source_refs: ["source_ref_missing"] },
    { ...basePreview, candidate_preview_id: "invalid_missing_operator_context", operator_context_ref: "" },
    { ...basePreview, candidate_preview_id: "invalid_candidate_only", candidate_only: false },
    { ...basePreview, candidate_preview_id: "invalid_preview_only", preview_only: false },
    { ...basePreview, candidate_preview_id: "invalid_review_required", review_required_later: false },
    { ...basePreview, candidate_preview_id: "invalid_durable_write", durable_write_now: true },
    { ...basePreview, candidate_preview_id: "invalid_proof_or_evidence", not_proof_or_evidence: false },
    { ...basePreview, candidate_preview_id: "invalid_source_of_truth", not_source_of_truth: false },
    { ...basePreview, candidate_preview_id: "invalid_perspective_state", not_perspective_state: false },
    { ...basePreview, candidate_preview_id: "invalid_work_status", not_work_status: false },
    { ...basePreview, candidate_preview_id: "invalid_product_write", not_product_write: false },
  ];
  const invalidResult = buildOperatorSourceCandidateGenerationImplementation({
    operator_source_candidate_generation_contract: contractFixture,
  ...
  for (const expectedCode of [
    ...
    assert.ok(
      invalidResult.validation.failure_codes.includes(expectedCode),
      `invalid override must include ${expectedCode}`,
    );
  }
  assert.equal(invalidResult.generated_candidate_summary.all_generated_candidates_have_source_refs, false);
  assert.equal(invalidResult.generated_candidate_summary.all_generated_candidate_source_refs_resolve_to_bundle_refs, false);
  assert.equal(invalidResult.generated_candidate_summary.all_generated_candidates_have_operator_context, false);
  assert.equal(invalidResult.generated_candidate_summary.all_generated_candidates_preview_only, false);
  assert.equal(invalidResult.generated_candidate_summary.all_generated_candidates_candidate_only, false);
  assert.equal(invalidResult.generated_candidate_summary.all_generated_candidates_review_required_later, false);
  assert.equal(invalidResult.generated_candidate_summary.all_generated_candidates_durable_write_now_false, false);
  assert.equal(invalidResult.generated_candidate_summary.all_generated_candidates_not_proof_or_evidence, false);
  assert.equal(invalidResult.generated_candidate_summary.all_generated_candidates_not_source_of_truth, false);
  assert.equal(invalidResult.generated_candidate_summary.all_generated_candidates_not_perspective_state, false);
  assert.equal(invalidResult.generated_candidate_summary.all_generated_candidates_not_work_status, false);
  assert.equal(invalidResult.generated_candidate_summary.all_generated_candidates_not_product_write, false);
  assertAuthorityBoundary(invalidResult.authority_boundary);
  assert.deepEqual(
    invalidResult.generated_candidate_generation_preview_bundle.authority_boundary,
    contractFixture.authority_boundary,
...
function assertDocsPointers() {
  for (const requiredText of [
    ...
    "generated candidate generation preview bundle from #724 contract",
    "candidate preview family summary",
    "generated candidate summary",
    "source reference summary",
    "provenance/review/privacy/non-authority summaries",
