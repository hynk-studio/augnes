...
assert.deepEqual(
  ...
  [],
  "demo readiness PR must not add or change app/api files",
);
assert.deepEqual(
  changedFiles.filter((file) => file.startsWith("lib/")),
  [],
  "demo readiness PR must not change lib runtime files",
);
...
assert.deepEqual(logoAssetsChanged, [], "demo readiness PR must not add logo assets");
...
console.log(
  JSON.stringify(
    {
      smoke: "cockpit-demo-readiness",
      package_script_present: true,
      six_tab_order_preserved: true,
      ...
      graphic_logo_artwork_added: false,
      logo_asset_added_or_imported: false,
      final_polish_css_present: true,
