...
if (
  ...
  void main().catch((error) => {
    console.error(
      "operational_reentry_v04_provider_compatibility_probe_failed",
    );
...
async function main(): Promise<void> {
  ...
  if (capability.status !== "available") {
    failV01(`operational_reentry_v04_probe_gateway_${capability.status}`);
  }
  ...
  if (!route) failV01("operational_reentry_v04_probe_route_unavailable");
      ...
      JSON.stringify({
        status:
          result.report.outcome === "accepted_all_shapes"
            ? "operational_reentry_v04_provider_compatibility_probe_accepted_all_shapes"
            : "operational_reentry_v04_provider_compatibility_probe_terminal",
...
function parseArgumentsV01(args: string[]): {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (
      value ===
      "--confirm-operational-reentry-v04-provider-compatibility-probe"
    ) {
      confirmed = true;
      ...
      failV01("operational_reentry_v04_probe_argument_invalid");
    }
  }
  if (!confirmed) {
    failV01("operational_reentry_v04_probe_explicit_confirmation_required");
  }
  if (!authorizationFile) {
    failV01("operational_reentry_v04_probe_authorization_file_required");
  }
  return {
...
function readAuthorizationRepositoryIdentityV01(value: unknown): {
  ...
  if (
    ...
    failV01("operational_reentry_v04_probe_authorization_repository_identity_invalid");
  }
  return {
...
export function preflightOperationalReentryV04ProviderCompatibilityProbeRepositoryV01(
  ...
  if (
    ...
    failV01("operational_reentry_v04_probe_repository_identity_mismatch");
  }
  if (
    realpathSync(gitV01(repositoryRoot, ["rev-parse", "--show-toplevel"])) !==
    repositoryRoot
  ) {
    failV01("operational_reentry_v04_probe_repository_root_mismatch");
  }
  ...
  if (
    ...
    failV01("operational_reentry_v04_probe_dirty_or_mismatched_head");
  }
  ...
  try {
    ...
    failV01("operational_reentry_v04_probe_origin_main_unavailable");
  }
  if (exactOriginMain !== authorizationIdentity.exact_merged_source_head) {
    failV01("operational_reentry_v04_probe_source_head_not_exact_origin_main");
  }
  if (
...
function boundedErrorCodeV01(error: unknown): string {
  ...
  return "operational_reentry_v04_probe_runtime_failed";
}
