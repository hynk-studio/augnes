...
const PLAN_PATH = path.join(ARTIFACT_DIR, "dry-run-transaction-plan.json");
...
const FALLBACK_PACKAGE_ADDED_LINES = [
  '    "smoke:research-candidate-single-claim-temp-to-product-disabled-bridge-dry-run-transaction-plan-v0-1": "node scripts/smoke-research-candidate-single-claim-temp-to-product-disabled-bridge-dry-run-transaction-plan-v0-1.mjs",',
  '    "plan:research-candidate-single-claim-temp-to-product-disabled-bridge-dry-run-transaction-plan-v0-1": "node scripts/run-research-candidate-single-claim-temp-to-product-disabled-bridge-dry-run-transaction-plan-v0-1.mjs",',
];
...
async function main() {
  ...
  const report = {
    ...
    artifact_paths: {
      report: REPORT_PATH,
      dry_run_transaction_plan: PLAN_PATH,
    },
    input_paths: {
    ...
    final_status:
      plan.dry_run_transaction_plan_status === READY_PLAN_STATUS ? "pass" : "fail",
  };
    ...
    JSON.stringify(
      {
        ...
        dry_run_transaction_plan_status:
          report.dry_run_transaction_plan_status,
...
function buildDryRunTransactionPlan({
  contractTestsReport,
  ...
  const planCore = {
    ...
    dry_run_transaction_plan_version: PLAN_VERSION,
    ...
    dry_run_idempotency_envelope: buildIdempotencyEnvelope(
      tempToProductBridgeDesign,
    ),
...
function validateSourceEvidence({
  ...
  if (contractTestsReport.next_recommended_slice !== THIS_SLICE) {
...
function explicitForbiddenSurfaces() {
  return Object.fromEntries(PLAN_FORBIDDEN_SURFACE_KEYS.map((key) => [key, false]));
}
...
function validateStaticRepoBoundary() {
  ...
  if (missingExpectedFiles.length > 0) {
    failures.push("static_expected_dry_run_transaction_plan_files_missing");
    messages.push(
...
function resolveStaticBoundaryDelta() {
  const envBaseRef =
    process.env.AUGNES_DRY_RUN_TRANSACTION_PLAN_BASE_REF?.trim();
  if (envBaseRef) {
    const envDelta = resolveDeltaFromBaseRef(
      envBaseRef,
      "env:AUGNES_DRY_RUN_TRANSACTION_PLAN_BASE_REF",
    );
    if (envDelta) return deltaOrAllowlistFallback(envDelta);
    return emptyDelta(envBaseRef, "env:AUGNES_DRY_RUN_TRANSACTION_PLAN_BASE_REF");
  }
