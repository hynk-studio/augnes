...
const disabledProductWriteAdapterHarnessDocsPath =
  "docs/DISABLED_PRODUCT_WRITE_ADAPTER_REENTRY_HARNESS_V0_1.md";
const disabledProductWriteAdapterHarnessHelperPath =
  "lib/product-write/disabled-product-write-adapter-reentry-harness.ts";
const disabledProductWriteAdapterHarnessFixturePath =
...
const releaseCandidateFreezeManifestHelperPath =
  "lib/release-readiness/release-candidate-freeze-manifest.ts";
const releaseCandidateFreezeManifestFixturePath =
...
const disabledProductWriteAdapterHarnessChangedFiles = [
  disabledProductWriteAdapterHarnessDocsPath,
...
const releaseCandidateFreezeManifestChangedFiles = [
  "docs/AUGNES_INTEGRATED_DEVELOPMENT_ROADMAP_V0_2_1_FULL.md",
  releaseCandidateFreezeManifestDocsPath,
...
function disabledProductWriteAdapterHarnessSliceActive(changedFiles) {
  return [
    disabledProductWriteAdapterHarnessDocsPath,
    disabledProductWriteAdapterHarnessHelperPath,
    disabledProductWriteAdapterHarnessFixturePath,
...
function releaseCandidateFreezeManifestSliceActive(changedFiles) {
  return [
    releaseCandidateFreezeManifestDocsPath,
    releaseCandidateFreezeManifestHelperPath,
    releaseCandidateFreezeManifestFixturePath,
...
function assertReleaseCandidateFreezeManifestChangedFiles(changedFiles) {
  for (const expectedFile of releaseCandidateFreezeManifestChangedFiles) {
    assert.ok(changedFiles.includes(expectedFile), `changed files must include ${expectedFile}`);
  }
  for (const changedFile of changedFiles) {
    assert.ok(
      releaseCandidateFreezeManifestChangedFiles.includes(changedFile),
      `unexpected changed file in release candidate freeze manifest slice: ${changedFile}`,
    );
    assert.doesNotMatch(changedFile, /^app\/api\//, "must not change app/api routes");
    assert.doesNotMatch(
      changedFile,
...
function assertDisabledProductWriteAdapterHarnessChangedFiles(changedFiles) {
  for (const expectedFile of disabledProductWriteAdapterHarnessChangedFiles) {
    assert.ok(changedFiles.includes(expectedFile), `changed files must include ${expectedFile}`);
  }
  for (const changedFile of changedFiles) {
    assert.ok(
      disabledProductWriteAdapterHarnessChangedFiles.includes(changedFile),
      `unexpected changed file in disabled product write adapter harness slice: ${changedFile}`,
    );
    assert.doesNotMatch(changedFile, /^app\/api\//, "must not change app/api routes");
    assert.doesNotMatch(
      changedFile,
...
function assertFeedbackEventStoreListUiImplementationChangedFiles(changedFiles) {
  ...
  for (const changedFile of changedFiles) {
    ...
    if (changedFile.startsWith("components/")) {
      assert.ok(
        [listUiImplementationComponentPath, componentPath].includes(changedFile),
