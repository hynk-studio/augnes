...
export function formatStrategyCompositionComparisonMarkdownV01(comparison: StrategyCompositionComparisonV01): string {
  const lines = [
    ...
    "## Four variants",
    "",
    "| Variant | Case | Components | Bindings | Relations |",
    "| --- | --- | ---: | ---: | ---: |",
    ...comparison.variant_summaries.map((item) => `| ${item.variant_kind} | ${item.case_ref.case_id} | ${item.component_count} | ${item.role_binding_count} | ${item.relation_count} |`),
    "",
    "Component content, sources, task-family identity, cutoff, and other construction material are exact across unbound, bound, and ordered; only binding/order structure changes.",
    "",
    "## Observed outcome vectors",
    "",
    "| Variant | Required passed | Failed | Hard gate | Corrections | Interventions | Tool calls | Latency ms | Completeness |",
    "| --- | ---: | ---: | --- | ---: | ---: | ---: | ---: | --- |",
    ...comparison.outcome_observations.map((item) => `| ${item.subject_kind} | ${shown(item.outcome.verification.required_passed)} | ${shown(item.outcome.verification.failed)} | ${shown(item.outcome.verification.hard_gate_failure)} | ${shown(item.outcome.review_burden.correction_count)} | ${shown(item.outcome.review_burden.intervention_count)} | ${shown(item.outcome.cost_operability.tool_call_count)} | ${shown(item.outcome.cost_operability.latency_ms)} | ${item.completeness} |`),
    "",
