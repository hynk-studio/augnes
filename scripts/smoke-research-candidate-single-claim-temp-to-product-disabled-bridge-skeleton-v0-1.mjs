...
function assertNoRouteUiSchemaDependencyExpansion() {
  ...
  for (const line of addedPackageLines) {
    assert.ok(
      ...
      `package.json must only add disabled bridge skeleton, contract-test, dry-run, authority bundle, disabled adapter skeleton, disabled adapter contract-test, disabled adapter dry-run invocation harness, no-op report, or preflight envelope contract-test scripts, not dependencies: ${line}`,
    );
  }
}
