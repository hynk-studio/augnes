...
function assertCockpitSurface() {
  assertContainsAll(cockpitFile, [
    ...
    "PERSPECTIVE_INGEST_MISSING_INPUT_ERROR",
    "manual:pasted_text",
    "missing_input_text",
...
function assertBoundaryDocs() {
  assertContainsAll(boundaryDoc, [
    ...
    "credential-shaped",
    "Empty and whitespace-only manual preview input fails closed",
    "missing_input_text",
    "Pasted text is required.",
    "rejected empty input is not echoed",
    "does not present the previous graph as the current preview",
