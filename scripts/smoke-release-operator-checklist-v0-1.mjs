...
const releaseNotesDocsPath = "docs/RELEASE_NOTES_PUBLIC_SAFE_SUMMARY_V0_1.md";
const docsPath = "docs/RELEASE_OPERATOR_CHECKLIST_V0_1.md";
const helperPath = "lib/release-readiness/release-operator-checklist.ts";
const fixturePath = "fixtures/release-operator-checklist.sample.v0.1.json";
const packagePath = "package.json";
const indexPath = "docs/00_INDEX_LATEST.md";

const checklistVersion = "release_operator_checklist.v0.1";
const inputVersion = "release_operator_checklist_input.v0.1";
const resultVersion = "release_operator_checklist_result.v0.1";
const itemVersion = "release_operator_checklist_item.v0.1";
const fixtureVersion = "release_operator_checklist.sample.v0.1";
const scope = "project:augnes";
const packageScriptName = "smoke:release-operator-checklist-v0-1";
const packageScriptValue = "node scripts/smoke-release-operator-checklist-v0-1.mjs";
...
const allowedFixturePlaceholders = [
  "raw checklist payload blocked by fixture",
  "raw conversation blocked by release checklist fixture",
  "hidden reasoning blocked by release checklist fixture",
  "telemetry dump blocked by release checklist fixture",
...
const releaseNotesDocs = read(releaseNotesDocsPath);
...
assert.ok(
  roadmap.includes("release_operator_checklist_v0_1"),
  "roadmap must contain release_operator_checklist_v0_1",
);
assert.ok(
  releaseNotesDocs.includes("Release Notes Public Safe Summary is review-only."),
  "Release Notes Public Safe Summary docs must preserve review-only wording",
);

assert.equal(fixture.fixture_version, fixtureVersion);
assert.equal(fixture.checklist_version, checklistVersion);
...
assert.equal(
  ...
  packageScriptValue,
  "package.json must register the release operator checklist smoke",
);
...
assert.ok(
  helperSource.includes("buildReleaseOperatorChecklistV01"),
  "helper must export builder",
);
assert.ok(
  helperSource.includes("validateReleaseOperatorChecklistInputV01"),
...
const helper = await importReleaseOperatorChecklistHelper();

assert.deepEqual(
  helper.buildReleaseOperatorChecklistV01(fixture.expected_valid_input),
  fixture.expected_result,
  "valid release operator checklist output must match expected_result",
);
assert.equal(fixture.expected_result.status, "built");
assert.equal(fixture.expected_result.decision, "checklist_candidate_only");
assertResultAuthorityClosed(fixture.expected_result);

const repeated = helper.buildReleaseOperatorChecklistV01(fixture.expected_valid_input);
assert.equal(
  repeated.checklist_fingerprint,
  fixture.expected_result.checklist_fingerprint,
  "repeated build must keep the same fingerprint",
);

assert.deepEqual(
  helper.buildReleaseOperatorChecklistV01(fixture.expected_empty_input),
  fixture.expected_empty_result,
  "empty release operator checklist output must match expected_empty_result",
);
...
for (const [caseName, input] of Object.entries(fixture.decision_coverage_inputs)) {
  const actual = helper.buildReleaseOperatorChecklistV01(input);
...
assertDecisionCase(
  "missing_release_notes_summary_item_blocked",
  "blocked",
  "release-operator-checklist-item:missing:release_notes_summary",
  "mandatory_checklist_item_missing",
);
assertDecisionCase(
  "missing_release_boundary_item_blocked",
  "blocked",
  "release-operator-checklist-item:missing:release_boundary",
  "mandatory_checklist_item_missing",
);
assertDecisionCase(
  "missing_product_write_boundary_item_blocked",
  "blocked",
  "release-operator-checklist-item:missing:product_write_boundary",
  "mandatory_checklist_item_missing",
);
assert.equal(
  fixture.expected_decision_results.unchecked_blocking_item_blocked.decision,
  "blocked",
);
assert.ok(
  fixture.expected_decision_results.unchecked_blocking_item_blocked.reason_codes.includes(
    "blocking_item_present",
  ),
  "unchecked blocking item must record blocking_item_present",
);
assert.equal(
  fixture.expected_decision_results.unchecked_warning_item_needs_operator_review
    .decision,
  "needs_operator_review",
);
assert.equal(
...
publicSafeHyphenInput.checklist_id = "release-operator-checklist:hyphen-safe";
publicSafeHyphenInput.input_items[0].bounded_summary =
  "Public-safe risk-reduction and task-level release operator checklist summary.";
const publicSafeHyphenResult =
  helper.buildReleaseOperatorChecklistV01(publicSafeHyphenInput);
assert.equal(
  ...
  JSON.stringify(
    {
      ...
      verified: {
        valid_result: fixture.expected_result.checklist_fingerprint,
        empty_result: fixture.expected_empty_result.checklist_fingerprint,
...
function assertAuthorityBoundary(authorityBoundary) {
  assert.equal(
    authorityBoundary.release_operator_checklist_now,
    true,
    "release_operator_checklist_now must be true",
  );
...
async function importReleaseOperatorChecklistHelper() {
