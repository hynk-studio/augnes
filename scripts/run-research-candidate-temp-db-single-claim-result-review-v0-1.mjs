...
const REVIEW_VERSION = "manual_note_temp_db_single_claim_result_review.v0.1";
const ARTIFACT_DIR = "/tmp/augnes-temp-db-single-claim-result-review-v0-1";
const REPORT_PATH = path.join(ARTIFACT_DIR, "report.json");
const REVIEW_PATH = path.join(ARTIFACT_DIR, "result-review.json");
const COMMITTED_HARNESS_FIXTURE_PATH =
  "fixtures/research-candidate-review.manual-note-temp-db-single-claim-write-prototype-harness.sample.v0.1.json";
const TMP_HARNESS_REPORT_PATH =
  "/tmp/augnes-single-claim-write-prototype-v0-1/report.json";
const BROWSER_VALIDATION_REPORT_PATH =
...
async function main() {
  ...
  const committedHarnessReport = await readJson(COMMITTED_HARNESS_FIXTURE_PATH);
  const tmpHarnessReport = await readOptionalJson(TMP_HARNESS_REPORT_PATH);
  const browserValidationReport = await readOptionalJson(
    BROWSER_VALIDATION_REPORT_PATH,
  );
  const review = buildResultReview({
    committedHarnessReport,
    tmpHarnessReport,
    browserValidationReport,
  });
  const report = {
    report_kind: "manual_note_temp_db_single_claim_result_review_report",
    report_version: REVIEW_VERSION,
    artifact_dir: ARTIFACT_DIR,
    artifact_paths: {
      report: REPORT_PATH,
      result_review: REVIEW_PATH,
    },
    input_paths: {
      committed_harness_fixture: COMMITTED_HARNESS_FIXTURE_PATH,
      tmp_harness_report: TMP_HARNESS_REPORT_PATH,
      browser_validation_report: BROWSER_VALIDATION_REPORT_PATH,
    },
    optional_inputs: {
      tmp_harness_report_present: Boolean(tmpHarnessReport),
      tmp_harness_report_final_status: tmpHarnessReport?.final_status ?? null,
      browser_validation_report_present: Boolean(browserValidationReport),
      browser_validation_report_final_status:
        browserValidationReport?.final_status ?? null,
    },
    result_review: review,
    preserved_boundaries: review.product_write_boundary,
    ...
    JSON.stringify(
      {
        ...
        source_result_status: review.source_harness.result_status,
...
function buildResultReview({
  ...
  const committedReport = asRecord(committedHarnessReport);
  const tmpReport = asRecord(tmpHarnessReport);
  ...
  const reviewCore = {
    ...
    evidence_checks: {
      ...
      browser_external_requests_zero_when_report_available: checkNotBlocked(
        evidenceCheckResults,
        "browser_external_requests_zero",
      ),
      browser_forbidden_requests_zero_when_report_available: checkNotBlocked(
        evidenceCheckResults,
  ...
  const fingerprint = createFingerprint(reviewCore);
  const review = {
    ...reviewCore,
...
function validateReport(review) {
  return (
    ["temp_result_review_passed", "temp_result_review_needs_attention"].includes(
      review.review_status,
    ) &&
    review.evidence_check_results.every((result) => result.status !== "block") &&
    review.temp_record_summary.exactly_one_each === true &&
    review.source_harness.result_status === "temp_db_write_passed" &&
    boundaryIntact(review.product_write_boundary)
  );
}
...
function buildAttentionNote(review) {
...
function boundaryIntact(boundary) {
  if (!boundary || boundary.result_review_only !== true) return false;
  return Object.entries(boundary).every(([key, value]) =>
    key === "result_review_only" ? value === true : value === false,
  );
}
