...
function assertChangedFileBoundary() {
  ...
  for (const file of files) {
    ...
    assert(
      ...
      `Phase 2B must not add API route files outside the Delta Projection read route: ${file}`,
    );
    assert(
      !/^app\/.*route\.(ts|tsx|js|jsx)$/.test(file) ||
        allowedRouteFiles.has(file),
      `Phase 2B must not add route files outside the Delta Projection read route: ${file}`,
    );
