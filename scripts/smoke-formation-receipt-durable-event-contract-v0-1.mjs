...
const typePath = "types/formation-receipt-durable-event-contract.ts";
const fixturePath =
...
assertSelectedContextPolicy(fixture);
assertExcludedContextPolicy(fixture);
assertUnresolvedTensionPolicy(fixture);
assertDecisionLinkPolicy(fixture);
...
assert.deepEqual(
  fixture,
  rebuiltFixture,
  "rebuilt Formation Receipt durable event contract fixture must match committed fixture",
);
  ...
  JSON.stringify(
    {
      ...
      source_feedback_event_aggregation_validation_ref:
        fixture.source_feedback_event_aggregation_validation_ref,
      durable_event_write_implemented_now:
        fixture.authority_boundary.durable_event_write_implemented_now,
      ...
      proof_or_evidence_record: fixture.authority_boundary.proof_or_evidence_record,
      perspective_promotion: fixture.authority_boundary.perspective_promotion,
...
function buildContractFixture() {
  ...
  const sourceValidationRef = `${sourceValidationFixture.validation_version}:${sourceValidationFixturePath}#711`;
  const sourceRefs = [
    {
      ref_id: "source_ref_feedback_aggregation_validation_711",
      ref_kind: "source_ref",
      ref: sourceValidationRef,
      public_safe: true,
    },
    {
      ref_id: "source_ref_geometry_digest_sample",
      ref_kind: "geometry_digest_ref",
      ref: "perspective_geometry_digest.v0.1:fixtures/research-candidate-review.perspective-geometry-digest.sample.v0.1.json",
      public_safe: true,
    },
    {
      ref_id: "source_ref_ai_context_packet_sample",
      ref_kind: "ai_context_packet_ref",
      ref: "research_candidate_ai_context_packet.v0.1:fixtures/research-candidate-review.ai-context-packet.sample.v0.1.json",
      public_safe: true,
    },
  ];
  const selectedContextRefs = [
    {
      context_ref_id: "selected_context_ref_feedback_summary",
      context_kind: "feedback_aggregation_read_model_summary",
      selection_status: "selected",
      source_refs: ["source_ref_feedback_aggregation_validation_711"],
      reason:
        "Selected as provenance for what feedback aggregation validated before Formation Receipt contract definition.",
    },
    {
      context_ref_id: "selected_context_ref_geometry_digest",
      context_kind: "geometry_digest_reference",
      selection_status: "selected",
      source_refs: ["source_ref_geometry_digest_sample"],
      reason:
        "Selected as digest provenance reference only; it is not proof or source of truth.",
    },
  ];
  const excludedContextRefs = [
    {
      ...
      source_refs: ["source_ref_feedback_aggregation_validation_711"],
      reason:
        "Excluded because this contract slice must not call providers or treat provider output as receipt input.",
    },
    {
      ...
      source_refs: ["source_ref_feedback_aggregation_validation_711"],
      reason:
        "Excluded because product-write remains parked by #686 and this receipt is audit/provenance only.",
    },
  ];
  const unresolvedTensionIds = [
    "unresolved_tension_feedback_selection_vs_promotion_authority",
    "unresolved_tension_digest_reference_vs_source_of_truth",
  ];
  const fixture = {
    contract_kind: contractKind,
    contract_version: contractVersion,
    source_feedback_event_aggregation_validation_ref: sourceValidationRef,
    receipt_event_scope: {
    ...
    receipt_event_inputs: {
      source_refs: sourceRefs,
      candidate_ids: [
        "candidate_ref_selected_context_summary",
        "candidate_ref_excluded_provider_output",
      ],
      ...
      geometry_digest_ref: sourceRefs[1],
      ai_context_packet_ref: sourceRefs[2],
      feedback_aggregation_read_model_ref: sourceRefs[0],
      operator_decision_ref: {
        ref_id: "operator_decision_ref_contract_only",
        ref_kind: "operator_decision_ref",
        ref: "operator_decision_ref:contract_only_selection_context",
        public_safe: true,
      },
      related_pr_ref: {
        ref_id: "related_pr_ref_711",
        ref_kind: "related_pr_ref",
        ref: "github_pr:711",
        public_safe: true,
      },
      result_link: {
        ref_id: "result_link_ref_static_contract",
        ref_kind: "result_link_ref",
        ref: "result_link:formation_receipt_contract_static_fixture",
        public_safe: true,
      },
    },
    ...
    sample_receipt_event: {
      formation_receipt_event_id:
        "formation_receipt_event_contract_sample_v0_1_001",
      event_version: eventVersion,
      created_at: "2026-06-23T00:00:00.000Z",
      scope: "research_candidate_review:formation_receipt_contract_sample",
      formation_intent:
        "Record context, candidate, digest, handoff, decision, and result references that influenced an operator-visible outcome.",
      selected_context_refs: selectedContextRefs,
      excluded_context_refs: excludedContextRefs,
      excluded_context_reasons: Object.fromEntries(
        excludedContextRefs.map((ref) => [ref.context_ref_id, ref.reason]),
      ),
      unresolved_tension_ids: unresolvedTensionIds,
      source_refs: sourceRefs,
      candidate_refs: [
        {
          candidate_id: "candidate_ref_selected_context_summary",
          candidate_kind: "context_selection_candidate",
          selection_status: "selected",
          source_refs: ["source_ref_feedback_aggregation_validation_711"],
          reason:
            "Selected for provenance traceability only; it does not create proof or Perspective state.",
        },
        {
          candidate_id: "candidate_ref_excluded_provider_output",
          candidate_kind: "provider_output_candidate",
          selection_status: "excluded",
          source_refs: ["source_ref_feedback_aggregation_validation_711"],
          reason:
      ...
      digest_refs: [
        {
          ref_id: "digest_ref_geometry_sample",
          ref_kind: "geometry_digest_ref",
          ref: sourceRefs[1].ref,
          references_only: true,
        },
      ],
      handoff_refs: [
        {
          ref_id: "handoff_ref_codex_contract_only",
          ref_kind: "handoff_ref",
          ref: "codex_handoff_ref:contract_only_no_execution",
          references_only: true,
        },
      ],
      decision_refs: [
        {
          ref_id: "decision_ref_operator_contract_only",
          ref_kind: "decision_ref",
          ref: "operator_decision_ref:contract_only_selection_context",
          references_only: true,
        },
      ],
      result_refs: [
        {
          ref_id: "result_ref_static_contract",
          ref_kind: "result_link_ref",
          ref: "result_link:formation_receipt_contract_static_fixture",
          references_only: true,
        },
        {
          ref_id: "result_ref_related_pr_711",
          ref_kind: "related_pr_ref",
          ref: "github_pr:711",
          references_only: true,
        },
  ...
  fixture.contract_fingerprint = createFingerprint(fixture);
  return fixture;
}
...
function assertTypeContract() {
  for (const exportName of [
    ...
    "FormationReceiptDurableEventScope",
    "FormationReceiptDurableEventInputs",
    "FormationReceiptSelectedContextPolicy",
    "FormationReceiptExcludedContextPolicy",
    "FormationReceiptUnresolvedTensionPolicy",
    "FormationReceiptDecisionLinkPolicy",
    "FormationReceiptDurableEventFieldContract",
    "FormationReceiptAuthorityBoundary",
    "FormationReceiptValidationPolicy",
...
function assertNoForbiddenRuntimePatterns() {
  ...
  for (const filePath of changedSourceFiles) {
    ...
    for (const { label, regex } of [
      ...
      { label: "durable event writer", regex: /\b(write|insert|persist|create)FormationReceipt(Durable)?Event\b/i },
...
function assertContractShape(value) {
  ...
  assert.equal(
    ...
    `${sourceValidationFixture.validation_version}:${sourceValidationFixturePath}#711`,
  );
  assert.deepEqual(value.receipt_event_scope, {
    records_context_selection: true,
    records_candidate_selection: true,
    records_candidate_exclusion: true,
    records_unresolved_tensions: true,
  ...
  for (const field of [
    ...
    assert.ok(Object.hasOwn(value.durable_event_fields, field), `durable_event_fields must include ${field}`);
  }
  assert.equal(value.durable_event_fields.event_version, eventVersion);
...
function assertSelectedContextPolicy(value) {
  const policy = value.selected_context_policy;
  assert.equal(policy.selected_context_is_provenance_not_proof, true);
  assert.equal(policy.selected_context_is_not_source_of_truth, true);
  assert.equal(policy.selected_context_does_not_imply_perspective_promotion, true);
  assert.equal(policy.selected_context_does_not_imply_work_completion, true);
  assert.equal(policy.selected_context_does_not_imply_product_write, true);
  assert.equal(policy.selected_context_must_keep_source_refs, true);
  for (const ref of value.receipt_event_inputs.selected_context_refs) {
    assert.equal(ref.selection_status, "selected");
    assert.ok(Array.isArray(ref.source_refs) && ref.source_refs.length > 0);
  }
}

function assertExcludedContextPolicy(value) {
  const policy = value.excluded_context_policy;
  assert.equal(policy.exclusions_must_include_reason, true);
  assert.equal(policy.exclusions_delete_source_candidate_feedback_records, false);
  assert.equal(policy.exclusions_suppress_future_review, false);
  assert.equal(policy.exclusions_are_audit_provenance_only, true);
  for (const ref of value.receipt_event_inputs.excluded_context_refs) {
    assert.equal(ref.selection_status, "excluded");
    assert.ok(typeof ref.reason === "string" && ref.reason.length > 20);
    assert.ok(Array.isArray(ref.source_refs) && ref.source_refs.length > 0);
  }
}

function assertUnresolvedTensionPolicy(value) {
  const policy = value.unresolved_tension_policy;
  assert.equal(policy.unresolved_tensions_must_be_preserved, true);
  assert.equal(policy.receipt_creation_resolves_tensions, false);
  assert.equal(policy.unresolved_tensions_may_block_later_promotion, true);
  assert.equal(policy.contract_decides_promotion, false);
  assert.deepEqual(
    value.sample_receipt_event.unresolved_tension_ids,
    value.receipt_event_inputs.unresolved_tension_ids,
  );
}

function assertDecisionLinkPolicy(value) {
  const policy = value.decision_link_policy;
  assert.equal(policy.may_reference_operator_decision, true);
  assert.equal(policy.may_reference_codex_handoff, true);
  assert.equal(policy.may_reference_pr, true);
  assert.equal(policy.may_reference_review_result, true);
  assert.equal(policy.creates_referenced_objects, false);
  assert.equal(policy.approves_merge, false);
  assert.equal(policy.executes_codex_or_github_automation, false);
  assert.equal(policy.mutates_work_status, false);
  for (const ref of [
    ...value.sample_receipt_event.handoff_refs,
    ...value.sample_receipt_event.decision_refs,
    ...value.sample_receipt_event.result_refs,
  ]) {
...
function assertSampleReceiptEvent(value) {
  assert.equal(value.event_version, eventVersion);
  assert.ok(value.formation_receipt_event_id.startsWith("formation_receipt_event_contract_sample"));
  assert.ok(value.selected_context_refs.length >= 1);
  assert.ok(value.excluded_context_refs.length >= 1);
  assert.ok(Object.keys(value.excluded_context_reasons).length >= 1);
  for (const excludedRef of value.excluded_context_refs) {
    assert.equal(value.excluded_context_reasons[excludedRef.context_ref_id], excludedRef.reason);
  }
  assert.ok(value.unresolved_tension_ids.length >= 1);
  assert.ok(value.digest_refs.every((ref) => ref.references_only === true));
  assert.ok(value.handoff_refs.every((ref) => ref.references_only === true));
  assert.ok(value.decision_refs.every((ref) => ref.references_only === true));
  assert.ok(value.result_refs.every((ref) => ref.references_only === true));
  assert.deepEqual(value.authority_boundary, buildAuthorityBoundary());
  assert.deepEqual(value.validation, buildValidationPolicy());
}
...
function assertDocsPointers() {
  for (const requiredText of [
    ...
    smokePath,
    packageScriptName,
    "contract-only durable event shape",
    "provenance/selection/exclusion/tension preservation",
    "no runtime DB write",
    "no route or UI",
    "no browser request",
  ...
  for (const doc of [substrateDoc, surfaceDoc, gateDoc]) {
    assert.match(doc, /Formation Receipt durable event contract v0\.1/i);
    assert.match(doc, /selected\/excluded context/i);
    assert.match(doc, /unresolved tensions/i);
    assert.match(doc, /audit\/provenance|provenance and decision links/i);
...
function assertSourceValidationDownstreamPointer() {
  for (const requiredText of [
    ...
    assert.ok(
      ...
      "#711 browser validation smoke must allow Formation Receipt durable event contract downstream pointer",
    );
  }
}
