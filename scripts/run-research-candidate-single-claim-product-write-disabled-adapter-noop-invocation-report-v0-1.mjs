...
const NOOP_REPORT_PATH = path.join(ARTIFACT_DIR, "noop-invocation-report.json");
const HELPER_INPUT_PATH = path.join(ARTIFACT_DIR, "helper-input.json");

const DRY_RUN_HARNESS_FIXTURE_PATH =
...
const FALLBACK_PACKAGE_ADDED_LINES = [
  '    "smoke:research-candidate-single-claim-product-write-disabled-adapter-noop-invocation-report-v0-1": "node scripts/smoke-research-candidate-single-claim-product-write-disabled-adapter-noop-invocation-report-v0-1.mjs",',
  '    "report:research-candidate-single-claim-product-write-disabled-adapter-noop-invocation-report-v0-1": "node scripts/run-research-candidate-single-claim-product-write-disabled-adapter-noop-invocation-report-v0-1.mjs",',
];
...
async function main() {
  ...
  const dryRunHarnessFixture = await readJson(DRY_RUN_HARNESS_FIXTURE_PATH);
  const selectedSources = await selectSources(dryRunHarnessFixture);
  ...
  const noopReport = await invokeHelper({
    dryRunInvocationHarness: selectedSources.dryRunInvocationHarness,
    reportReviewInput: {},
    staticBoundaryEvidence,
    sourceValidationFailureCodes,
  });
  const matrixUnexpectedFailures = asArray(noopReport.report_validation_matrix)
    .map(asRecord)
    .filter((row) => row.check_status !== "pass");
  const finalStatus =
    asRecord(noopReport.validation).passed === true &&
    noopReport.noop_invocation_report_status === READY_REPORT_STATUS &&
  ...
  const report = {
    ...
    artifact_paths: {
      report: REPORT_PATH,
      noop_invocation_report: NOOP_REPORT_PATH,
    },
    input_paths: {
      dry_run_invocation_harness_fixture: DRY_RUN_HARNESS_FIXTURE_PATH,
    ...
    noop_invocation_report: noopReport,
    noop_invocation_report_status:
      noopReport.noop_invocation_report_status ??
      (finalStatus === "pass" ? READY_REPORT_STATUS : BLOCKED_REPORT_STATUS),
    recommendation_status:
      finalStatus === "pass" ? READY_RECOMMENDATION : BLOCKED_RECOMMENDATION,
    next_recommended_slice:
      finalStatus === "pass" ? NEXT_PREFLIGHT_COMMAND_ENVELOPE : RECHECK_SLICE,
    ...
    validation: {
      ...
      failure_codes: unique([
        ...sourceValidationFailureCodes,
        ...asArray(asRecord(noopReport.validation).failure_codes).map(asString),
  ...
  await writeFile(NOOP_REPORT_PATH, `${JSON.stringify(noopReport, null, 2)}\n`);
    ...
    JSON.stringify(
      {
        ...
        noop_invocation_report_status: report.noop_invocation_report_status,
        recommendation_status: report.recommendation_status,
        next_recommended_slice: report.next_recommended_slice,
        validation_matrix_count: asArray(noopReport.report_validation_matrix).length,
...
async function selectSources(dryRunHarnessFixture) {
  const selections = [];
  const failureCodes = [];
  let dryRunInvocationHarness = dryRunHarnessFixture;
  for (const [
    label,
    optionalReportPath,
    nestedKey,
    ...
    const selection = await selectSource({
      label,
      fixture: label === "dry_run_invocation_harness" ? dryRunHarnessFixture : {},
      fixturePath,
      optionalReportPath,
      nestedKey,
      payloadKey,
    ...
    if (label === "dry_run_invocation_harness") {
      dryRunInvocationHarness = selection.value;
    }
  }
  return {
    dryRunInvocationHarness,
...
async function invokeHelper(input) {
  ...
  const script = `
    ...
    import { buildManualNoteSingleClaimProductWriteDisabledAdapterNoopInvocationReport } from "./${HELPER_PATH}";
    const input = JSON.parse(readFileSync("${HELPER_INPUT_PATH}", "utf8"));
    const report = buildManualNoteSingleClaimProductWriteDisabledAdapterNoopInvocationReport(input);
    writeFileSync("${NOOP_REPORT_PATH}", JSON.stringify(report, null, 2) + "\\n");
  `;
  execFileSync(TSX_PATH, ["--eval", script], { stdio: "pipe" });
  return JSON.parse(await readFile(NOOP_REPORT_PATH, "utf8"));
}
...
function validateStaticRepoBoundary() {
  ...
  const routeFiles = listFiles("app/api").filter((filePath) =>
    /single-claim-product-write-disabled-adapter-noop|noop-invocation-report/i.test(
      filePath,
    ),
  );
...
function summarizeStaticBoundaryResult(result) {
  return {
    note:
      "Live static boundary metadata observed during fixture mode is not included in the no-op invocation report artifact or fingerprint.",
    static_boundary_base_ref: result.static_boundary_base_ref,
    static_boundary_base_mode: result.static_boundary_base_mode,
    static_boundary_base_commit: result.static_boundary_base_commit,
    static_boundary_compare_ref: result.static_boundary_compare_ref,
...
function resolveStaticBoundaryDelta() {
  const envBaseRef =
    process.env
      .AUGNES_PRODUCT_WRITE_DISABLED_ADAPTER_NOOP_INVOCATION_REPORT_BASE_REF?.trim();
  if (envBaseRef) {
    const envDelta = resolveDeltaFromBaseRef(
      envBaseRef,
      "env:AUGNES_PRODUCT_WRITE_DISABLED_ADAPTER_NOOP_INVOCATION_REPORT_BASE_REF",
    );
