...
const SKELETON_PATH = path.join(ARTIFACT_DIR, "disabled-bridge-skeleton.json");
...
async function main() {
  ...
  const committedBridgeDesign = await readJson(BRIDGE_DESIGN_FIXTURE_PATH);
  ...
  const report = {
    ...
    artifact_paths: {
      report: REPORT_PATH,
      disabled_bridge_skeleton: SKELETON_PATH,
    },
    input_paths: {
    ...
    preserved_boundaries: skeleton.explicit_forbidden_surfaces,
    ...
    JSON.stringify(
      {
        ...
        source_bridge_recommendation_status:
...
function buildSkeleton({
  ...
  return {
    ...skeleton,
    local_copy_packet: {
      markdown: buildMarkdown(skeleton),
      json: buildJson(skeleton),
      fingerprint,
...
function validateSkeleton(
  ...
  if (skeleton.product_db_write !== false) {
    failures.push("product_db_write");
  }
  if (skeleton.product_id_allocation !== false) {
  ...
  if (
    skeleton.placeholder_record_mapping?.idempotency_write_executed_now !==
      false ||
    skeleton.placeholder_record_mapping?.rollback_write_executed_now !== false ||
    skeleton.placeholder_record_mapping?.audit_write_executed_now !== false
  ) {
    failures.push("placeholder_record_write_executed_now");
  }
...
function buildMarkdown(skeleton) {
  return [
