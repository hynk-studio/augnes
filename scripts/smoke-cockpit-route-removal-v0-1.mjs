...
const followOnPerspectiveCockpitRouteNamespaceCleanupSourceScanExemptions =
  new Set(
    followOnPerspectiveCockpitRouteNamespaceCleanupFiles.filter((file) =>
      file.startsWith("app/perspective/"),
    ),
  );
...
function assertNoAuthorityDriftInChangedSource(files) {
  ...
  for (const file of sourceFiles) {
    if (
      followOnPerspectiveCockpitRouteNamespaceCleanupSourceScanExemptions.has(
        file,
      )
    ) {
      continue;
