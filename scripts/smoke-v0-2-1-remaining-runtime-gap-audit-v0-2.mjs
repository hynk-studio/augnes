...
const docsPath = "docs/V0_2_1_REMAINING_RUNTIME_GAP_AUDIT_V0_2.md";
const fixturePath = "fixtures/v0-2-1-remaining-runtime-gap-audit.sample.v0.2.json";
const smokePath = "scripts/smoke-v0-2-1-remaining-runtime-gap-audit-v0-2.mjs";
const packagePath = "package.json";
const indexPath = "docs/00_INDEX_LATEST.md";
const roadmapPath = "docs/AUGNES_INTEGRATED_DEVELOPMENT_ROADMAP_V0_2_1_FULL.md";

const previousAuditDocsPath = "docs/V0_2_1_REMAINING_RUNTIME_GAP_AUDIT_V0_1.md";
const previousAuditFixturePath =
  "fixtures/v0-2-1-remaining-runtime-gap-audit.sample.v0.1.json";
const previousAuditSmokePath = "scripts/smoke-v0-2-1-remaining-runtime-gap-audit-v0-1.mjs";

const phase4InstrumentationDocsPath =
  "docs/RUNTIME_AUDIT_SELECTED_ROUTE_INSTRUMENTATION_V0_4_PHASE_4_PROMOTION_STATE_V0_1.md";
const phase4InstrumentationFixturePath =
  "fixtures/runtime-audit-selected-route-instrumentation.v0.4.phase-4-promotion-state.sample.json";
const phase4InstrumentationSmokePath =
...
const fixtureVersion = "v0_2_1_remaining_runtime_gap_audit.sample.v0.2";
const auditVersion = "v0_2_1_remaining_runtime_gap_audit_v0_2";
const previousAuditVersion = "v0_2_1_remaining_runtime_gap_audit_v0_1";
const phase4InstrumentationVersion =
  "runtime_audit_selected_route_instrumentation_v0_4_phase_4_promotion_state_v0_1";
const scope = "project:augnes";

const packageScriptName = "smoke:v0-2-1-remaining-runtime-gap-audit-v0-2";
const packageScriptValue = "node scripts/smoke-v0-2-1-remaining-runtime-gap-audit-v0-2.mjs";
const previousAuditPackageScriptName = "smoke:v0-2-1-remaining-runtime-gap-audit-v0-1";
const phase4InstrumentationPackageScriptName =
...
const phase4RuntimeSurfaceRefs = new Set([
  "phase_4_promotion_durable_state_chain",
  "promotion_decision_store_v0_1",
  "formation_receipt_durable_write_v0_1",
  "durable_perspective_state_apply_v0_1",
  "perspective_trajectory_v0_1",
  phase4InstrumentationVersion,
]);
...
for (const filePath of [
  ...
  indexPath,
  roadmapPath,
  previousAuditDocsPath,
  previousAuditFixturePath,
  previousAuditSmokePath,
  phase4InstrumentationDocsPath,
  phase4InstrumentationFixturePath,
  phase4InstrumentationSmokePath,
  groundingDocsPath,
...
const previousAuditDocsText = readText(previousAuditDocsPath);
const previousAuditFixtureText = readText(previousAuditFixturePath);
const phase4InstrumentationDocsText = readText(phase4InstrumentationDocsPath);
const phase4InstrumentationFixtureText = readText(phase4InstrumentationFixturePath);

assertIncludes(roadmapText, "v0.2.1 FULL", "roadmap mentions v0.2.1 FULL");
assertIncludes(previousAuditDocsText, previousAuditVersion, "v0.1 audit marker in docs");
assertIncludes(previousAuditFixtureText, previousAuditVersion, "v0.1 audit marker in fixture");
assertIncludes(
  phase4InstrumentationDocsText,
  phase4InstrumentationVersion,
  "v0.4 instrumentation marker in docs",
);
assertIncludes(
  phase4InstrumentationFixtureText,
  "runtime_audit_selected_route_instrumentation.v0.4.phase_4_promotion_state.v0.1",
  "v0.4 instrumentation marker in fixture",
);
assertIncludes(
  readText(phase4InstrumentationSmokePath),
  phase4InstrumentationVersion,
  "v0.4 instrumentation marker in smoke",
);

assert.equal(packageJson.scripts?.[packageScriptName], packageScriptValue);
assert(packageJson.scripts?.[previousAuditPackageScriptName], "v0.1 audit package script exists");
assert(
  packageJson.scripts?.[phase4InstrumentationPackageScriptName],
  "v0.4 instrumentation package script exists",
);
...
assert(fixture.phase_inventory.length > 0, "phase_inventory must not be empty");
assert(fixture.runtime_surface_inventory.length > 0, "runtime_surface_inventory must not be empty");
assert(fixture.gated_work_inventory.length > 0, "gated_work_inventory must not be empty");
...
for (const ref of fixture.evidence_refs ?? []) {
  assert.equal(typeof ref, "string", "top-level evidence ref must be string");
  assert(existsSync(ref), `top-level evidence ref must exist: ${ref}`);
}

for (const entry of allInventoryEntries(fixture)) {
  if (!phase4RuntimeSurfaceRefs.has(entry.item_ref)) continue;
  assert.notEqual(
    entry.status_category,
    "runtime_complete_without_audit",
    `${entry.item_ref} must not remain runtime_complete_without_audit after v0.4`,
  );
}
...
const ungatedGaps = allInventoryEntries(fixture).filter(
...
if (ungatedGaps.length > 0) {
  assert.notEqual(
    ...
    "ungated gap must name a concrete next slice",
  );
  assert.doesNotMatch(
    fixture.next_recommended_implementation_slice.item_ref,
    /^(TBD|todo|unknown)$/i,
    "next slice must not be vague",
  );
} else {
  assert.equal(
    fixture.next_recommended_implementation_slice.item_ref,
    "none_without_explicit_approval",
    "no ungated gap must report none_without_explicit_approval",
  );
  assert.equal(
    fixture.remaining_gap_inventory.length,
    0,
    "remaining_gap_inventory must be empty when no ungated gap remains",
  );
}
...
console.log(
  JSON.stringify(
    {
      smoke: "v0-2-1-remaining-runtime-gap-audit-v0-2",
      final_status: "pass",
      ...
      static_prior_smoke_refs_checked: [
        previousAuditPackageScriptName,
        phase4InstrumentationPackageScriptName,
      ],
    },
    null,
    2,
...
function assertNoLiveLookingPayloads(text, label) {
  const forbiddenPatterns = [
    ...
    {
      pattern: /\bprovider[-_ ]?(?:thread|run|session)[-_ ]?id[:=][A-Za-z0-9_-]+/i,
      label: "provider id",
    },
    {
      pattern: /\bhttps?:\/\/(?:localhost|127\.0\.0\.1|10\.|172\.(?:1[6-9]|2\d|3[0-1])\.|192\.168\.)/i,
      label: "private URL",
    },
  ];
...
function assertChangedFileScope() {
  ...
  for (const line of added) {
    ...
    if (
      /(provider|retrieval|github|git-runtime|codex-execution|product-write|product-id|db-schema|route|ui)/i.test(
        filePath,
      ) &&
      !expectedChangedFiles.has(filePath)
    ) {
      forbiddenAdded.push(filePath);
    }
  }
