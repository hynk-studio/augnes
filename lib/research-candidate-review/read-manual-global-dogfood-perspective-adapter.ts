...
export interface ResearchCandidateManualGlobalDogfoodPerspectiveAdapterDbLike {
...
> & {
  ...
  existing_current_working_adapter_compatibility_json: string;
  existing_canonical_state_adapter_compatibility_json: string;
  manual_adapter_write_path_json: string;
...
export function getResearchCandidateManualGlobalDogfoodPerspectiveAdapterWriteAuthorityBoundary(): ResearchCandidateManualGlobalDogfoodPerspectiveAdapterWriteAuthorityBoundary {
  return {
...
export function ensureResearchCandidateManualGlobalDogfoodPerspectiveAdapterSchema(
  db: ResearchCandidateManualGlobalDogfoodPerspectiveAdapterDbLike,
) {
  db.exec(`
...
export function researchCandidateManualGlobalDogfoodPerspectiveAdapterSchemaExists(
  db: ResearchCandidateManualGlobalDogfoodPerspectiveAdapterDbLike,
) {
  const row = db
    .prepare(
...
export function readResearchCandidateManualGlobalDogfoodPerspectiveAdapter({
  scope = DEFAULT_SCOPE,
  receiptId,
  idempotencyKey,
  limit = 25,
  ...
  try {
    if (!researchCandidateManualGlobalDogfoodPerspectiveAdapterSchemaExists(openedDb)) {
    ...
    const rows = openedDb
      .prepare(
        `
          SELECT *
          FROM research_candidate_manual_global_dogfood_perspective_adapter_receipts
    ...
    const latestActiveCommitted =
      recordsByReceipt.find(
        (recordSet) =>
          ...
          Boolean(recordSet.perspective_adapter_record),
      ) ?? null;
...
export function readResearchCandidateManualGlobalDogfoodPerspectiveAdapterByReceiptId(
  receiptId: string,
  {
    scope = DEFAULT_SCOPE,
    db,
...
): ResearchCandidateManualGlobalDogfoodPerspectiveAdapterRecordsByReceipt | null {
  return (
...
function buildRecordsByReceipt({
  ...
  const recordRow = db
    .prepare(
      `
        SELECT *
        FROM research_candidate_manual_global_dogfood_perspective_adapter_records
        WHERE receipt_id = ?
        LIMIT 1
      `,
    )
  ...
  const rollback = db
    .prepare(
      `
        SELECT *
        FROM research_candidate_manual_global_dogfood_perspective_adapter_rollbacks
        WHERE receipt_id = ?
        LIMIT 1
      `,
    )
  ...
  return {
    receipt,
    perspective_adapter_record: recordRow ? normalizeRecord(recordRow) : null,
...
function normalizeRecord(
  ...
  return {
    ...
    existing_current_working_adapter_compatibility: parseJsonObject(
      row.existing_current_working_adapter_compatibility_json,
    ),
    existing_canonical_state_adapter_compatibility: parseJsonObject(
      row.existing_canonical_state_adapter_compatibility_json,
    ),
    manual_adapter_write_path: parseJsonObject(
      row.manual_adapter_write_path_json,
    ),
...
function readbackFlags(hasPerspectiveAdapterRows: boolean) {
  return {
    raw_manual_note_text_present: false,
    raw_result_report_text_present: false,
    operator_notes_persisted: false,
    perspective_adapter_record_written: hasPerspectiveAdapterRows,
