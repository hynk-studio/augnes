...
export interface ResearchCandidateManualGlobalDogfoodCanonicalPerspectiveUpdateDbLike {
...
> & {
  ...
  existing_perspective_update_compatibility_json: string;
...
export function getResearchCandidateManualGlobalDogfoodCanonicalPerspectiveUpdateWriteAuthorityBoundary(): ResearchCandidateManualGlobalDogfoodCanonicalPerspectiveUpdateWriteAuthorityBoundary {
  return {
...
export function ensureResearchCandidateManualGlobalDogfoodCanonicalPerspectiveUpdateSchema(
  db: ResearchCandidateManualGlobalDogfoodCanonicalPerspectiveUpdateDbLike,
) {
  db.exec(`
...
export function researchCandidateManualGlobalDogfoodCanonicalPerspectiveUpdateSchemaExists(
  db: ResearchCandidateManualGlobalDogfoodCanonicalPerspectiveUpdateDbLike,
) {
  const row = db
    .prepare(
...
export function readResearchCandidateManualGlobalDogfoodCanonicalPerspectiveUpdate({
  scope = DEFAULT_SCOPE,
  receiptId,
  idempotencyKey,
  limit = 25,
  ...
  try {
    if (
      !researchCandidateManualGlobalDogfoodCanonicalPerspectiveUpdateSchemaExists(
        openedDb,
      )
    ) {
    ...
    const rows = openedDb
      .prepare(
        `
          SELECT *
          FROM research_candidate_manual_global_dogfood_canonical_perspective_update_receipts
    ...
    const latestActiveCommitted =
      recordsByReceipt.find(
        (recordSet) =>
          ...
          Boolean(recordSet.canonical_perspective_update_record),
      ) ?? null;
...
export function readResearchCandidateManualGlobalDogfoodCanonicalPerspectiveUpdateByReceiptId(
  receiptId: string,
  {
    scope = DEFAULT_SCOPE,
    db,
...
): ResearchCandidateManualGlobalDogfoodCanonicalPerspectiveUpdateRecordsByReceipt | null {
  return (
...
function buildRecordsByReceipt({
  ...
  const recordRow = db
    .prepare(
      `
        SELECT *
        FROM research_candidate_manual_global_dogfood_canonical_perspective_update_records
        WHERE receipt_id = ?
        LIMIT 1
      `,
    )
  ...
  const rollback = db
    .prepare(
      `
        SELECT *
        FROM research_candidate_manual_global_dogfood_canonical_perspective_update_rollbacks
        WHERE receipt_id = ?
        LIMIT 1
      `,
    )
  ...
  return {
    receipt,
    canonical_perspective_update_record: recordRow
...
function normalizeRecord(
  ...
  return {
    ...
    existing_perspective_update_compatibility: parseJsonObject(
      row.existing_perspective_update_compatibility_json,
    ),
...
function readbackFlags(hasCanonicalUpdateRows: boolean) {
  return {
