...
export interface ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationDbLike {
...
> & {
  ...
  existing_state_apply_compatibility_json: string;
...
export function getResearchCandidateManualGlobalDogfoodPerspectiveStateMutationWriteAuthorityBoundary(): ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationWriteAuthorityBoundary {
  return {
...
export function ensureResearchCandidateManualGlobalDogfoodPerspectiveStateMutationSchema(
  db: ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationDbLike,
) {
  db.exec(`
...
export function researchCandidateManualGlobalDogfoodPerspectiveStateMutationSchemaExists(
  db: ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationDbLike,
) {
  const row = db
    .prepare(
...
export function readResearchCandidateManualGlobalDogfoodPerspectiveStateMutation({
  scope = DEFAULT_SCOPE,
  receiptId,
  idempotencyKey,
  limit = 25,
  ...
  try {
    if (
      !researchCandidateManualGlobalDogfoodPerspectiveStateMutationSchemaExists(
        openedDb,
      )
    ) {
    ...
    const rows = openedDb
      .prepare(
        `
          SELECT *
          FROM research_candidate_manual_global_dogfood_perspective_state_mutation_receipts
    ...
    const latestActiveCommitted =
      recordsByReceipt.find(
        (recordSet) =>
          ...
          Boolean(recordSet.perspective_state_mutation_record),
      ) ?? null;
...
export function readResearchCandidateManualGlobalDogfoodPerspectiveStateMutationByReceiptId(
  receiptId: string,
  {
    scope = DEFAULT_SCOPE,
    db,
...
): ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationRecordsByReceipt | null {
  return (
...
function buildRecordsByReceipt({
  ...
  const recordRow = db
    .prepare(
      `
        SELECT *
        FROM research_candidate_manual_global_dogfood_perspective_state_mutation_records
        WHERE receipt_id = ?
        LIMIT 1
      `,
    )
  ...
  const rollback = db
    .prepare(
      `
        SELECT *
        FROM research_candidate_manual_global_dogfood_perspective_state_mutation_rollbacks
        WHERE receipt_id = ?
        LIMIT 1
      `,
    )
  ...
  return {
    receipt,
    perspective_state_mutation_record: recordRow
...
function normalizeRecord(
  ...
  return {
    ...
    existing_state_apply_compatibility: parseJsonObject(
      row.existing_state_apply_compatibility_json,
    ),
...
function readbackFlags(hasPerspectiveStateMutationRows: boolean) {
  return {
