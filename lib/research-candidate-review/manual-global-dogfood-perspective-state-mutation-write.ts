...
import {
  ...
  RESEARCH_CANDIDATE_MANUAL_GLOBAL_DOGFOOD_PERSPECTIVE_STATE_MUTATION_WRITE_VERSION,
  type ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationRecord,
  type ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationRollbackRecord,
  type ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationRollbackResult,
  type ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationWriteReceipt,
  type ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationWriteRequest,
  type ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationWriteResult,
  type ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationWriteValidation,
...
type PerspectiveStateMutationDbLike =
  ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationDbLike;
type JsonRecord = Record<string, unknown>;

const FINGERPRINT_ALGORITHM = "fnv1a32_canonical_json_v0_1" as const;
const AUTHORITY_PROFILE =
  "manual_global_dogfood_perspective_state_mutation_write_authority.v0.1";
...
export function writeResearchCandidateManualGlobalDogfoodPerspectiveStateMutation(
  request: unknown,
  options: { db?: PerspectiveStateMutationDbLike } = {},
): ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationWriteResult {
  const validation =
    validateResearchCandidateManualGlobalDogfoodPerspectiveStateMutationWriteRequest(
      request,
    );
  ...
  const typedRequest =
    request as unknown as ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationWriteRequest;
  const contract = typedRequest.perspective_state_mutation_contract;
  const db =
  ...
  try {
    ...
    const sourcePerspectiveApply = getSourcePerspectiveApplyForRequest({
      db,
      request: typedRequest,
    });
    const receipt = buildReceipt({
      request: typedRequest,
      sourcePerspectiveApply: sourcePerspectiveApply!,
      idempotencyKey: validation.idempotency_key!,
      createdAt,
    });
    const stateMutationRecord = buildPerspectiveStateMutationRecord({
      request: typedRequest,
      sourcePerspectiveApply: sourcePerspectiveApply!,
      receipt,
      createdAt,
    ...
    try {
      ...
      insertPerspectiveStateMutationRecord(db, stateMutationRecord);
    ...
    return {
      ...
      validation,
      receipt,
      perspective_state_mutation_record: stateMutationRecord,
      readback:
        readResearchCandidateManualGlobalDogfoodPerspectiveStateMutation({
          scope: contract.scope,
...
export function rollbackResearchCandidateManualGlobalDogfoodPerspectiveStateMutationReceipt(
  request: unknown,
  options: { db?: PerspectiveStateMutationDbLike } = {},
): ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationRollbackResult {
...
export function validateResearchCandidateManualGlobalDogfoodPerspectiveStateMutationWriteRequest(
  request: unknown,
): ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationWriteValidation {
  ...
  const contractRecord = isRecord(request.perspective_state_mutation_contract)
    ? request.perspective_state_mutation_contract
    : null;
  const reviewRecord = isRecord(request.perspective_state_mutation_review)
    ? request.perspective_state_mutation_review
    : null;
  const operatorAuthorization = isRecord(request.operator_authorization)
    ? request.operator_authorization
    : null;
  if (!contractRecord) {
    reasons.push("perspective_state_mutation_contract_shape_invalid");
  }
  if (!reviewRecord) reasons.push("perspective_state_mutation_review_shape_invalid");
  if (!operatorAuthorization) {
    reasons.push(
      "perspective_state_mutation_operator_authorization_shape_invalid",
    );
  }
  ...
  const contract =
    contractRecord && contractShapeReasons.length === 0
      ? (contractRecord as unknown as ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationContract)
      : null;
  const review =
    reviewRecord && reviewShapeReasons.length === 0
      ? (reviewRecord as unknown as ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationReview)
      : null;
  ...
  const sourceReadback = isRecord(request.source_perspective_apply_readback)
    ? (request.source_perspective_apply_readback as unknown as ResearchCandidateManualGlobalDogfoodPerspectiveApplyReadback)
    : null;
  if (sourceReadback && !sourcePerspectiveApplyReadbackFlagsAreClean(sourceReadback)) {
    reasons.push("perspective_state_mutation_source_readback_forbidden_mutation_flag");
  }

  const idempotencyKey = contract
    ? computePerspectiveStateMutationIdempotencyKey(contract)
    : null;
  return validationResult({
...
function validateContractShape(contract: JsonRecord) {
  ...
  const mapping = isRecord(contract.proposed_state_mutation_mapping)
    ? contract.proposed_state_mutation_mapping
    : null;
  if (!mapping) {
    reasons.push("perspective_state_mutation_mapping_shape_invalid");
  } else {
    if (
      !hasText(mapping.mutation_label) ||
      !hasText(mapping.mutation_rationale) ||
      ...
      !SUPPORTED_APPLY_TARGETS.has(String(mapping.intended_future_mutation_target)) ||
      mapping.can_feed_future_state_mutation_write_candidate !== true ||
  ...
  const candidate = isRecord(contract.proposed_state_mutation_candidate)
    ? contract.proposed_state_mutation_candidate
    : null;
  if (
    !candidate ||
    !hasText(candidate.candidate_status) ||
    !SUPPORTED_APPLY_TARGETS.has(String(candidate.mutation_scope_hint)) ||
    !SUPPORTED_APPLY_STRENGTH_HINTS.has(String(candidate.mutation_strength_hint)) ||
...
function validateContract(
  ...
  const candidate = contract.proposed_state_mutation_candidate;
  if (
    contract.operator_authorization_mode !==
    "ready_for_future_perspective_state_mutation_write_authorization"
  ) {
    reasons.push("perspective_state_mutation_contract_not_ready");
  }
  ...
  if (!contractNonWriteConfirmationStillClean(contract)) {
    reasons.push("perspective_state_mutation_contract_non_write_confirmation_invalid");
  }
  if (!contractAuthorityBoundaryStillReadOnly(contract)) {
    reasons.push(
      "perspective_state_mutation_contract_authority_boundary_not_preview_only",
    );
  }
  if (!sourceRefsPresent(contract)) {
    reasons.push("source_refs_missing");
  }
  if (!hasText(mapping.mutation_label)) {
    reasons.push("mutation_label_missing");
  }
  ...
  if (
    ...
    reasons.push("perspective_state_mutation_explanation_insufficient");
  }
  if (
  ...
  if (candidate.mutation_scope_hint !== "canonical_perspective_state") {
    reasons.push("perspective_state_mutation_scope_hint_must_be_canonical_state");
  }
  if (mapping.intended_future_mutation_target !== "canonical_perspective_state") {
    reasons.push("perspective_state_mutation_target_must_be_canonical_state");
  }
  if (!SUPPORTED_UPDATE_STRENGTHS.has(candidate.mutation_strength_hint)) {
    reasons.push("perspective_state_mutation_contract_not_ready");
  }
  return reasons;
}
...
function validateReviewShape(review: JsonRecord) {
  ...
  if (
    ...
    reasons.push("perspective_state_mutation_review_shape_invalid");
  }
...
function validateReview(
  ...
  if (
    ...
    reasons.push(
      "perspective_state_mutation_review_not_ready_for_future_write_slice",
    );
  }
  if (review.validation.passed !== true) {
    reasons.push("perspective_state_mutation_review_validation_not_passed");
  }
  if (review.validation.no_write_authority !== true) {
    reasons.push(
      "perspective_state_mutation_review_no_write_authority_not_preserved",
    );
  }
  if (
    ...
    reasons.push("perspective_state_mutation_review_operator_decision_not_accept");
  }
  if (review.validation.operator_note_persisted !== false) {
    reasons.push("perspective_state_mutation_review_operator_note_persisted");
  }
  if (!contractAuthorityBoundaryStillReadOnly({ authority_boundary: review.authority_boundary })) {
    reasons.push(
      "perspective_state_mutation_review_authority_boundary_not_preview_only",
    );
  }
  return reasons;
}
...
function validateReviewBoundToContract({
  ...
  if (
    ...
    reasons.push("perspective_state_mutation_review_contract_mismatch");
  }
  const summary = review.accepted_mapping_summary;
  if (!summary) {
    reasons.push("perspective_state_mutation_review_source_mismatch");
    return uniqueStrings(reasons);
  }
  const mapping = contract.proposed_state_mutation_mapping;
  const summaryMismatches = [
    ...
    summary.intended_future_mutation_target !==
      mapping.intended_future_mutation_target,
    summary.mutation_label !== mapping.mutation_label,
    summary.mutation_rationale !== mapping.mutation_rationale,
...
function validateOperatorAuthorization(operatorAuthorization: JsonRecord) {
  ...
  if (
    ...
    reasons.push("perspective_state_mutation_wrong_confirmation");
  }
  if (
...
function validateSourcesForRequest({
  ...
  db: PerspectiveStateMutationDbLike;
  request: ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationWriteRequest;
  ignoreRequestSuppliedReadbacks?: boolean;
}) {
  const reasons: string[] = [];
  const contract = request.perspective_state_mutation_contract;
  const perspectiveApply =
    readResearchCandidateManualGlobalDogfoodPerspectiveApplyByReceiptId(
      contract.source_perspective_apply_receipt_id!,
      { scope: contract.scope, db },
    );
  const perspectiveApplyReadback =
    readResearchCandidateManualGlobalDogfoodPerspectiveApply({
      scope: contract.scope,
      receiptId: contract.source_perspective_apply_receipt_id,
      limit: 1,
      db,
    });
  if (!perspectiveApply) {
  ...
  if (perspectiveApply.receipt.write_status !== "committed") {
    reasons.push(
      "perspective_state_mutation_source_apply_receipt_not_active_committed",
    );
  }
  if (!perspectiveApply.perspective_apply_record) {
    reasons.push("perspective_state_mutation_source_apply_record_mismatch");
    return reasons;
  }
  if (!sourcePerspectiveApplyReadbackFlagsAreClean(perspectiveApplyReadback)) {
    reasons.push("perspective_state_mutation_source_readback_forbidden_mutation_flag");
  }
  if (
    perspectiveApply.perspective_apply_record.intended_future_apply_target !==
    "canonical_perspective_state"
  ) {
    reasons.push(
  ...
  if (
    perspectiveApply.perspective_apply_record.apply_scope_hint !==
    "canonical_perspective_state"
  ) {
    reasons.push("apply_scope_hint_must_be_canonical_perspective_state");
  }
  if (
    !ignoreRequestSuppliedReadbacks &&
    request.source_perspective_apply_readback &&
    !sourcePerspectiveApplyReadbackFlagsAreClean(
      request.source_perspective_apply_readback,
    )
  ) {
    reasons.push("perspective_state_mutation_source_readback_forbidden_mutation_flag");
  }
  if (perspectiveApplyRecordMismatchesContract({ perspectiveApply, contract })) {
    reasons.push("perspective_state_mutation_source_apply_record_mismatch");
  }
  return uniqueStrings(reasons);
}

function getSourcePerspectiveApplyForRequest({
  db,
  request,
}: {
  db: PerspectiveStateMutationDbLike;
  ...
  const receiptId =
    request.perspective_state_mutation_contract
      .source_perspective_apply_receipt_id ?? null;
  if (!receiptId) return null;
  return readResearchCandidateManualGlobalDogfoodPerspectiveApplyByReceiptId(
    receiptId,
    { scope: request.perspective_state_mutation_contract.scope, db },
  );
}

function perspectiveApplyRecordMismatchesContract({
  perspectiveApply,
  contract,
}: {
  perspectiveApply: NonNullable<
    ReturnType<typeof readResearchCandidateManualGlobalDogfoodPerspectiveApplyByReceiptId>
  >;
  contract: ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationContract;
}) {
  const record = perspectiveApply.perspective_apply_record;
  const mapping = contract.proposed_state_mutation_mapping;
  return (
    !record ||
    perspectiveApply.receipt.receipt_id !==
      contract.source_perspective_apply_receipt_id ||
    record.perspective_apply_record_id !==
      contract.source_perspective_apply_record_id ||
    record.perspective_apply_record_fingerprint !==
      contract.source_perspective_apply_record_fingerprint ||
    perspectiveApply.receipt.source_canonical_perspective_update_receipt_id !==
      contract.source_canonical_perspective_update_receipt_id ||
    perspectiveApply.receipt.source_canonical_perspective_update_record_id !==
      contract.source_canonical_perspective_update_record_id ||
    perspectiveApply.receipt.source_canonical_perspective_update_record_fingerprint !==
      contract.source_canonical_perspective_update_record_fingerprint ||
    perspectiveApply.receipt.source_perspective_relay_receipt_id !==
      contract.source_perspective_relay_receipt_id ||
    perspectiveApply.receipt.source_perspective_relay_record_id !==
      contract.source_perspective_relay_record_id ||
    perspectiveApply.receipt.source_perspective_relay_record_fingerprint !==
      contract.source_perspective_relay_record_fingerprint ||
    perspectiveApply.receipt.source_next_work_signal_receipt_id !==
      contract.source_next_work_signal_receipt_id ||
    perspectiveApply.receipt.source_next_work_signal_record_id !==
      contract.source_next_work_signal_record_id ||
    perspectiveApply.receipt.source_next_work_signal_record_fingerprint !==
      contract.source_next_work_signal_record_fingerprint ||
    perspectiveApply.receipt.source_next_work_bias_receipt_id !==
      contract.source_next_work_bias_receipt_id ||
    perspectiveApply.receipt.source_next_work_bias_record_id !==
      contract.source_next_work_bias_record_id ||
    perspectiveApply.receipt.source_next_work_bias_record_fingerprint !==
      contract.source_next_work_bias_record_fingerprint ||
    perspectiveApply.receipt.source_projection_fingerprint !==
      contract.source_projection_fingerprint ||
    perspectiveApply.receipt.source_global_dogfood_ledger_receipt_id !==
      contract.source_global_dogfood_ledger_receipt_id ||
    perspectiveApply.receipt.source_global_dogfood_ledger_record_id !==
      contract.source_global_dogfood_ledger_record_id ||
    perspectiveApply.receipt.source_metric_snapshot_receipt_id !==
      contract.source_metric_snapshot_receipt_id ||
    perspectiveApply.receipt.source_metric_snapshot_record_id !==
      contract.source_metric_snapshot_record_id ||
    perspectiveApply.receipt.source_manual_receipt_id !==
      contract.source_manual_receipt_id ||
    perspectiveApply.receipt.source_handoff_seed_fingerprint !==
      contract.source_handoff_seed_fingerprint ||
    perspectiveApply.receipt.source_result_text_fingerprint !==
      contract.source_result_text_fingerprint ||
    perspectiveApply.receipt.source_expected_observed_delta_record_ref !==
      contract.source_expected_observed_delta_record_ref ||
    perspectiveApply.receipt.source_reuse_outcome_record_ref !==
      contract.source_reuse_outcome_record_ref ||
...
function sourcePerspectiveApplyReadbackFlagsAreClean(
...
function supersedePreviousReceipt({
  ...
  const update = db
    ...
    .run(supersedesReceiptId, request.perspective_state_mutation_contract.scope);
...
function validateSupersedeTargetForRequest({
  ...
  db: PerspectiveStateMutationDbLike;
  request: ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationWriteRequest;
  existingReceipt: ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationWriteReceipt | null;
}) {
  const supersedesReceiptId =
    request.operator_authorization.supersedes_receipt_id ?? "";
  const target =
...
function buildReceipt({
  request,
  sourcePerspectiveApply,
  idempotencyKey,
  createdAt,
}: {
  request: ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationWriteRequest;
  sourcePerspectiveApply: NonNullable<
    ReturnType<typeof readResearchCandidateManualGlobalDogfoodPerspectiveApplyByReceiptId>
  >;
  ...
  const contract = request.perspective_state_mutation_contract;
  const sourceApplyRecord = sourcePerspectiveApply.perspective_apply_record!;
  const source = {
    scope: contract.scope,
    source_perspective_state_mutation_contract_fingerprint:
      contract.validation.contract_fingerprint,
    source_perspective_state_mutation_review_fingerprint:
      request.perspective_state_mutation_review.validation.review_fingerprint,
    source_perspective_apply_receipt_id: sourcePerspectiveApply.receipt.receipt_id,
    source_perspective_apply_record_id:
      sourceApplyRecord.perspective_apply_record_id,
    source_perspective_apply_record_fingerprint:
      sourceApplyRecord.perspective_apply_record_fingerprint,
    source_canonical_perspective_update_receipt_id:
      sourcePerspectiveApply.receipt.source_canonical_perspective_update_receipt_id,
    source_canonical_perspective_update_record_id:
      sourcePerspectiveApply.receipt.source_canonical_perspective_update_record_id,
    source_canonical_perspective_update_record_fingerprint:
      sourcePerspectiveApply.receipt
        .source_canonical_perspective_update_record_fingerprint,
    source_perspective_relay_receipt_id:
      sourcePerspectiveApply.receipt.source_perspective_relay_receipt_id,
    source_perspective_relay_record_id:
      sourcePerspectiveApply.receipt.source_perspective_relay_record_id,
    source_perspective_relay_record_fingerprint:
      sourcePerspectiveApply.receipt.source_perspective_relay_record_fingerprint,
  ...
  const receiptId = `manual-global-dogfood-perspective-state-mutation-receipt:${fingerprint({
    ...source,
...
function buildPerspectiveStateMutationRecord({
  request,
  sourcePerspectiveApply,
  receipt,
  createdAt,
  ...
  const sourceRefs = uniqueStrings([
    contract.source_perspective_apply_readback_ref,
    sourcePerspectiveApply.receipt.receipt_id,
    sourceApplyRecord.perspective_apply_record_id,
    sourceApplyRecord.perspective_apply_record_fingerprint,
    contract.source_canonical_perspective_update_receipt_id,
    contract.source_canonical_perspective_update_record_id,
  ...
  const source = {
    ...
    intended_future_mutation_target: "canonical_perspective_state" as const,
    mutation_scope_hint: "canonical_perspective_state" as const,
    mutation_strength_hint: candidate.mutation_strength_hint as
      | "low"
      | "medium"
      | "high",
    intended_future_apply_target: "canonical_perspective_state" as const,
    apply_scope_hint: "canonical_perspective_state" as const,
    apply_strength_hint: sourceApplyRecord.apply_strength_hint,
    ...
    existing_state_apply_compatibility:
      contract.proposed_existing_state_apply_compatibility as unknown as Record<
        string,
        unknown
      >,
    source_refs: sourceRefs,
  };
  const recordId = `manual-global-dogfood-perspective-state-mutation-record:${fingerprint({
    ...source,
    created_at: createdAt,
  })}`;
  return {
    perspective_state_mutation_record_id: recordId,
    created_at: createdAt,
    ...source,
    outcome_signal: source.outcome_signal as "positive" | "negative" | "ambiguous",
    authority_profile: AUTHORITY_PROFILE,
    perspective_state_mutation_record_fingerprint: fingerprint({
      ...source,
      perspective_state_mutation_record_id: recordId,
    }),
  };
...
function buildRollbackRecord({
  ...
  createdAt: string;
}): ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationRollbackRecord {
  const rollbackId = `manual-global-dogfood-perspective-state-mutation-rollback:${fingerprint({
    receipt_id: receiptId,
...
function insertPerspectiveStateMutationRecord(
  db: PerspectiveStateMutationDbLike,
  record: ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationRecord,
) {
  db.prepare(
    `
    ...
    existing_state_apply_compatibility_json: JSON.stringify(
      record.existing_state_apply_compatibility,
    ),
...
function insertRollback(
  db: PerspectiveStateMutationDbLike,
  rollback: ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationRollbackRecord,
) {
  db.prepare(
    `
      INSERT INTO research_candidate_manual_global_dogfood_perspective_state_mutation_rollbacks (
        rollback_id,
        created_at,
        receipt_id,
        rollback_reason,
...
function duplicateReplayResult({
  ...
  readback: ReturnType<
    typeof readResearchCandidateManualGlobalDogfoodPerspectiveStateMutation
  >;
}): ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationWriteResult {
  ...
  return {
    ...
    perspective_state_mutation_record:
      existing?.perspective_state_mutation_record ?? null,
    readback,
    refusal_reasons: [],
...
function refusedResult({
  ...
  return {
    ...
    validation,
    receipt: null,
    perspective_state_mutation_record: null,
    readback: null,
...
function validationResult({
  ...
  const mapping = isRecord(
    contractRecord?.proposed_state_mutation_mapping,
  )
    ? contractRecord.proposed_state_mutation_mapping
    : null;
  const candidate = isRecord(contractRecord?.proposed_state_mutation_candidate)
    ? contractRecord.proposed_state_mutation_candidate
    : null;
  return {
    ...
    exact_operator_confirmation_present:
      ...
      request?.operator_authorization.operator_confirmation_text ===
        RESEARCH_CANDIDATE_MANUAL_GLOBAL_DOGFOOD_PERSPECTIVE_STATE_MUTATION_WRITE_CONFIRMATION,
    ready_perspective_state_mutation_contract_present:
      isRecord(contractRecord) &&
      contractRecord.operator_authorization_mode ===
        "ready_for_future_perspective_state_mutation_write_authorization",
    accepted_perspective_state_mutation_review_present:
      isRecord(request?.perspective_state_mutation_review) &&
      request?.perspective_state_mutation_review.review_status ===
        "ready_for_future_perspective_state_mutation_write_slice",
    ...
    source_perspective_apply_receipt_active_committed:
      !uniqueFailureCodes.includes(
        "perspective_state_mutation_source_apply_receipt_not_active_committed",
      ),
    source_perspective_apply_record_matches_contract:
      !uniqueFailureCodes.includes(
        "perspective_state_mutation_source_apply_record_mismatch",
      ),
    source_readback_flags_preserve_no_forbidden_mutation:
      !uniqueFailureCodes.includes(
        "perspective_state_mutation_source_readback_forbidden_mutation_flag",
      ),
    mutation_label_present:
      isRecord(mapping) && hasText(mapping.mutation_label),
    mutation_rationale_present:
      isRecord(mapping) && hasText(mapping.mutation_rationale),
    apply_label_present:
    ...
    mutation_scope_hint_is_canonical_perspective_state:
      isRecord(candidate) &&
      candidate.mutation_scope_hint === "canonical_perspective_state",
    intended_future_mutation_target_is_canonical_perspective_state:
      isRecord(mapping) &&
      mapping.intended_future_mutation_target === "canonical_perspective_state",
    ...
    proposed_perspective_state_mutation_candidate_ready:
      isRecord(candidate) &&
      candidate.candidate_status ===
        "ready_for_future_perspective_state_mutation_write_authorization",
  };
...
function computePerspectiveStateMutationIdempotencyKey(
  contract: ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationContract,
) {
  const mapping = contract.proposed_state_mutation_mapping;
  const candidate = contract.proposed_state_mutation_candidate;
  return `manual-global-dogfood-perspective-state-mutation:${fingerprint({
    write_version:
      RESEARCH_CANDIDATE_MANUAL_GLOBAL_DOGFOOD_PERSPECTIVE_STATE_MUTATION_WRITE_VERSION,
    ...
    outcome_label: mapping.outcome_label,
    outcome_signal: mapping.outcome_signal,
    intended_future_mutation_target: mapping.intended_future_mutation_target,
    mutation_scope_hint: candidate.mutation_scope_hint,
    mutation_strength_hint: candidate.mutation_strength_hint,
    intended_future_apply_target: "canonical_perspective_state",
    apply_scope_hint: "canonical_perspective_state",
    apply_strength_hint: "source_apply_record_revalidated",
    selected_candidate_context_refs: mapping.selected_candidate_context_refs,
    source_next_work_candidate_card_ids:
      mapping.source_next_work_candidate_card_ids,
    manual_only_context_refs: mapping.manual_only_context_refs,
...
function writerAuthorityBoundaryIsNarrow() {
  const boundary =
    getResearchCandidateManualGlobalDogfoodPerspectiveStateMutationWriteAuthorityBoundary();
  return (
    boundary.can_write_perspective_state_mutation_record === true &&
    boundary.can_write_perspective_state_mutation_receipt === true &&
    boundary.can_write_perspective_state_mutation_rollback_metadata === true &&
    boundary.can_write_manual_canonical_perspective_state_mutation_record === true &&
...
function validateRequestedSideEffects(value: unknown) {
  ...
  return Object.entries(value)
    ...
    .map(
      ([key]) =>
        `perspective_state_mutation_requested_side_effects_refused:${key}`,
    );
}
...
function writeResultFlags(hasPerspectiveStateMutationRows: boolean) {
  return {
    perspective_state_mutation_record_written: hasPerspectiveStateMutationRows,
    manual_canonical_perspective_state_mutation_record_written:
      hasPerspectiveStateMutationRows,
...
function rollbackWriteTransaction(db: PerspectiveStateMutationDbLike) {
  try {
