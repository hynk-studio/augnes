...
export interface ResearchCandidateManualGlobalDogfoodPerspectiveApplyDbLike {
...
> & {
  ...
  existing_apply_path_compatibility_json: string;
...
export function getResearchCandidateManualGlobalDogfoodPerspectiveApplyWriteAuthorityBoundary(): ResearchCandidateManualGlobalDogfoodPerspectiveApplyWriteAuthorityBoundary {
  return {
...
export function ensureResearchCandidateManualGlobalDogfoodPerspectiveApplySchema(
  db: ResearchCandidateManualGlobalDogfoodPerspectiveApplyDbLike,
) {
  db.exec(`
...
export function researchCandidateManualGlobalDogfoodPerspectiveApplySchemaExists(
  db: ResearchCandidateManualGlobalDogfoodPerspectiveApplyDbLike,
) {
  const row = db
    .prepare(
...
export function readResearchCandidateManualGlobalDogfoodPerspectiveApply({
  scope = DEFAULT_SCOPE,
  receiptId,
  idempotencyKey,
  limit = 25,
  ...
  try {
    if (
      !researchCandidateManualGlobalDogfoodPerspectiveApplySchemaExists(
        openedDb,
      )
    ) {
    ...
    const rows = openedDb
      .prepare(
        `
          SELECT *
          FROM research_candidate_manual_global_dogfood_perspective_apply_receipts
    ...
    const latestActiveCommitted =
      recordsByReceipt.find(
        (recordSet) =>
          ...
          Boolean(recordSet.perspective_apply_record),
      ) ?? null;
...
export function readResearchCandidateManualGlobalDogfoodPerspectiveApplyByReceiptId(
  receiptId: string,
  {
    scope = DEFAULT_SCOPE,
    db,
...
): ResearchCandidateManualGlobalDogfoodPerspectiveApplyRecordsByReceipt | null {
  return (
...
function buildRecordsByReceipt({
  ...
  const recordRow = db
    .prepare(
      `
        SELECT *
        FROM research_candidate_manual_global_dogfood_perspective_apply_records
        WHERE receipt_id = ?
        LIMIT 1
      `,
    )
  ...
  const rollback = db
    .prepare(
      `
        SELECT *
        FROM research_candidate_manual_global_dogfood_perspective_apply_rollbacks
        WHERE receipt_id = ?
        LIMIT 1
      `,
    )
  ...
  return {
    receipt,
    perspective_apply_record: recordRow
...
function normalizeRecord(
  ...
  return {
    ...
    existing_apply_path_compatibility: parseJsonObject(
      row.existing_apply_path_compatibility_json,
    ),
...
function readbackFlags(hasPerspectiveApplyRows: boolean) {
  return {
