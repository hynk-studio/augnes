...
import type {
  ...
  ResearchCandidateManualGlobalDogfoodNextWorkBiasWriteAuthorityBoundary,
...
export interface ResearchCandidateManualGlobalDogfoodNextWorkBiasDbLike {
...
export function getResearchCandidateManualGlobalDogfoodNextWorkBiasWriteAuthorityBoundary(): ResearchCandidateManualGlobalDogfoodNextWorkBiasWriteAuthorityBoundary {
  return {
...
export function ensureResearchCandidateManualGlobalDogfoodNextWorkBiasSchema(
  db: ResearchCandidateManualGlobalDogfoodNextWorkBiasDbLike,
) {
  db.exec(`
...
export function researchCandidateManualGlobalDogfoodNextWorkBiasSchemaExists(
  db: ResearchCandidateManualGlobalDogfoodNextWorkBiasDbLike,
) {
  const row = db
    .prepare(
...
export function readResearchCandidateManualGlobalDogfoodNextWorkBias({
  scope = DEFAULT_SCOPE,
  receiptId,
  idempotencyKey,
  limit = 25,
  ...
  try {
    if (!researchCandidateManualGlobalDogfoodNextWorkBiasSchemaExists(openedDb)) {
    ...
    const rows = openedDb
      .prepare(
        `
          SELECT *
          FROM research_candidate_manual_global_dogfood_next_work_bias_receipts
    ...
    return {
      readback_kind:
        "research_candidate_manual_global_dogfood_next_work_bias_readback",
      readback_version:
        "research_candidate_manual_global_dogfood_next_work_bias_readback.v0.1",
      scope,
...
export function readResearchCandidateManualGlobalDogfoodNextWorkBiasByReceiptId(
  receiptId: string,
  {
    scope = DEFAULT_SCOPE,
    db,
...
function readRecordsByReceipt(
  ...
  const recordRow = db
    .prepare(
      `
        SELECT *
        FROM research_candidate_manual_global_dogfood_next_work_bias_records
        WHERE receipt_id = ?
        LIMIT 1
      `,
    )
  ...
  const rollbackRow = db
    .prepare(
      `
        SELECT *
        FROM research_candidate_manual_global_dogfood_next_work_bias_rollbacks
        WHERE receipt_id = ?
        LIMIT 1
      `,
    )
  ...
  return {
    receipt,
    next_work_bias_record: recordRow ? parseRecordRow(recordRow) : null,
