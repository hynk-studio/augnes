...
export interface ResearchCandidateManualGlobalDogfoodPerspectiveRelayDbLike {
...
export function getResearchCandidateManualGlobalDogfoodPerspectiveRelayWriteAuthorityBoundary(): ResearchCandidateManualGlobalDogfoodPerspectiveRelayWriteAuthorityBoundary {
  return {
...
export function ensureResearchCandidateManualGlobalDogfoodPerspectiveRelaySchema(
  db: ResearchCandidateManualGlobalDogfoodPerspectiveRelayDbLike,
) {
  db.exec(`
...
export function researchCandidateManualGlobalDogfoodPerspectiveRelaySchemaExists(
  db: ResearchCandidateManualGlobalDogfoodPerspectiveRelayDbLike,
) {
  const row = db
    .prepare(
...
export function readResearchCandidateManualGlobalDogfoodPerspectiveRelay({
  scope = DEFAULT_SCOPE,
  receiptId,
  idempotencyKey,
  limit = 25,
  ...
  try {
    if (!researchCandidateManualGlobalDogfoodPerspectiveRelaySchemaExists(openedDb)) {
    ...
    const rows = openedDb
      .prepare(
        `
          SELECT *
          FROM research_candidate_manual_global_dogfood_perspective_relay_receipts
    ...
    const latestActiveCommitted =
      recordsByReceipt.find(
        (recordSet) =>
          ...
          Boolean(recordSet.perspective_relay_record),
      ) ?? null;
...
export function readResearchCandidateManualGlobalDogfoodPerspectiveRelayByReceiptId(
  receiptId: string,
  {
    scope = DEFAULT_SCOPE,
    db,
...
): ResearchCandidateManualGlobalDogfoodPerspectiveRelayRecordsByReceipt | null {
  return (
...
function buildRecordsByReceipt({
  ...
  const recordRow = db
    .prepare(
      `
        SELECT *
        FROM research_candidate_manual_global_dogfood_perspective_relay_records
        WHERE receipt_id = ?
        LIMIT 1
      `,
    )
  ...
  const rollback = db
    .prepare(
      `
        SELECT *
        FROM research_candidate_manual_global_dogfood_perspective_relay_rollbacks
        WHERE receipt_id = ?
        LIMIT 1
      `,
    )
  ...
  return {
    receipt,
    perspective_relay_record: recordRow ? normalizeRecord(recordRow) : null,
...
function normalizeRecord(row: RecordRow): ResearchCandidateManualGlobalDogfoodPerspectiveRelayRecord {
  return {
    ...row,
...
function readbackFlags(hasRelayRows: boolean) {
  return {
