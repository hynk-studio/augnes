...
import {
  ...
  type HandoffSendDecisionAuthorityBoundary,
  type HandoffSendDecisionPreviewInput,
  type HandoffSendDecisionPreviewStatus,
  type HandoffSendDecisionReadiness,
  type HandoffSendOperatorDecisionIntent,
  type HandoffSendOperatorDecisionPreview,
...
const decisionIntents: HandoffSendOperatorDecisionIntent[] = [
  approvalIntent,
  "keep_preview_only",
  "reject",
];

export function createHandoffSendDecisionAuthorityBoundaryV01():
  HandoffSendDecisionAuthorityBoundary {
  return {
    read_only: true,
    advisory_only: true,
    ...
    notes: [
      "Decision preview is read-only and cannot write handoff send records.",
...
export function buildHandoffSendOperatorDecisionPreviewV01({
  ...
  as_of,
  source_refs,
}: HandoffSendDecisionPreviewInput = {}):
  HandoffSendOperatorDecisionPreview {
  const sendPreview = isHandoffSendPreview(
    handoff_send_preview,
  )
    ? handoff_send_preview
    : null;
  const sourceRefs = uniqueCandidateIngressStringsV01([
    ...(source_refs ?? []),
    ...(sendPreview?.source_refs ?? []),
  ]);
  const evidenceRefs = uniqueCandidateIngressStringsV01(
    sendPreview?.evidence_summary.evidence_refs ?? [],
  );
  const unsupportedIntent = Boolean(
    operator_decision_intent &&
      !decisionIntents.includes(
        operator_decision_intent as HandoffSendOperatorDecisionIntent,
      ),
  );
  const unsafeRefs = [
    ...sourceRefs,
  ...
  const blockingReasons = uniqueCandidateIngressStringsV01([
    ...(handoff_send_preview !== undefined &&
    handoff_send_preview !== null &&
    !sendPreview
      ? ["handoff_send_preview_malformed"]
      : []),
    ...(sendPreview &&
    sendPreview.send_preview_status !==
      "ready_for_future_handoff_send_record_write"
      ? ["handoff_send_preview_not_ready"]
      : []),
    ...(operator_decision_intent &&
  ...
  return {
    preview_version: HANDOFF_SEND_DECISION_PREVIEW_VERSION,
    scope: scope ?? HANDOFF_SEND_SCOPE,
    as_of: as_of ?? sendPreview?.as_of ?? new Date().toISOString(),
    ...
    input_summary: {
      has_handoff_send_preview: Boolean(sendPreview),
      handoff_send_preview_ready:
        sendPreview?.send_preview_status ===
        "ready_for_future_handoff_send_record_write",
    ...
    source_status: {
      handoff_send_preview: sendPreview
        ? sendPreview.send_preview_status ===
          "ready_for_future_handoff_send_record_write"
          ? "supplied"
          : "not_ready"
    ...
    approval_requirements: [
      "confirm_handoff_send_preview_is_ready",
      "confirm_scoped_local_send_record_is_the_only_write_target",
    ...
    evidence_summary: {
      has_ready_handoff_send_preview:
        sendPreview?.send_preview_status ===
        "ready_for_future_handoff_send_record_write",
    ...
    authority_boundary: createHandoffSendDecisionAuthorityBoundaryV01(),
  };
}

function isHandoffSendPreview(
  value: unknown,
): value is HandoffSendPreview {
  return Boolean(
    isRecord(value) &&
      value.preview_version === HANDOFF_SEND_PREVIEW_VERSION &&
      value.scope === HANDOFF_SEND_SCOPE &&
      isRecord(value.send_readiness) &&
...
function determineStatus({
  ...
  sendPreview: HandoffSendPreview | null;
  ready: boolean;
  blockingReasons: string[];
  missingEvidence: string[];
  refusalReasons: string[];
  insufficientData: string[];
}): HandoffSendDecisionPreviewStatus {
  if (ready) return "ready_for_future_handoff_send_record_write";
  if (!sendPreview || insufficientData.length > 0) return "insufficient_data";
