...
import {
  ...
  type CurrentWorkingPerspectiveUpdateContractDecisionAuthorityBoundary,
  type CurrentWorkingPerspectiveUpdateContractDecisionPreviewInput,
  type CurrentWorkingPerspectiveUpdateContractDecisionPreviewStatus,
  type CurrentWorkingPerspectiveUpdateContractOperatorDecisionIntent,
...
export function buildCurrentWorkingPerspectiveUpdateContractOperatorDecisionPreviewV01({
  ...
  as_of,
  source_refs,
}: CurrentWorkingPerspectiveUpdateContractDecisionPreviewInput = {}): CurrentWorkingPerspectiveUpdateContractOperatorDecisionPreview {
  const contractPreview = isContractPreview(
    current_working_perspective_update_contract_preview,
  )
    ? current_working_perspective_update_contract_preview
    : null;
  ...
  const blockingReasons = uniqueCandidateIngressStringsV01([
    ...(isSupplied(current_working_perspective_update_contract_preview) &&
    !contractPreview
      ? ["current_working_perspective_update_contract_preview_malformed"]
      : []),
    ...(contractPreview && !contractReady
      ? ["current_working_perspective_update_contract_preview_not_ready"]
      : []),
  ...
  const refusalReasons = uniqueCandidateIngressStringsV01([
    ...unsafeRefs.map(() => "current_working_perspective_update_contract_decision_refs_unsafe"),
    ...(operator_decision_intent &&
    ![
  ...
  const insufficientData = uniqueCandidateIngressStringsV01([
    ...(!contractPreview
      ? ["current_working_perspective_update_contract_preview_missing"]
      : []),
  ...
  return {
    preview_version:
      CURRENT_WORKING_PERSPECTIVE_UPDATE_CONTRACT_DECISION_PREVIEW_VERSION,
    scope: scope ?? contractPreview?.scope ?? CWP_CONTRACT_SCOPE,
    ...
    source_status: {
      current_working_perspective_update_contract_preview: contractPreview
        ? contractReady
          ? "supplied"
          : "not_ready"
        : isSupplied(current_working_perspective_update_contract_preview)
    ...
    approval_requirements: [
      "review_ready_current_working_perspective_update_contract_preview",
      "approve_only_scoped_local_contract_record_write",
    ...
    would_write_current_working_perspective_update_contract_decision_preview: {
      ...
      proposed_patch_entry_count:
        contractPreview?.proposed_current_working_perspective_update_contract
          .proposed_patch_entries.length ?? 0,
      source_refs: sourceRefs,
      evidence_refs: evidenceRefs,
    ...
    operator_review_checklist: [
      ...
      "confirm_future_apply_slice_is_separate",
    ],
    would_not_write: [
    ...
    authority_boundary:
      createCurrentWorkingPerspectiveUpdateContractDecisionAuthorityBoundaryV01(),
  };
}

export function createCurrentWorkingPerspectiveUpdateContractDecisionAuthorityBoundaryV01():
  CurrentWorkingPerspectiveUpdateContractDecisionAuthorityBoundary {
  return {
    read_only: true,
    advisory_only: true,
    ...
    notes: [
      "Read-only operator decision preview for a scoped local CWP update contract record.",
...
function determineStatus({
  ...
  writeReadiness: CurrentWorkingPerspectiveUpdateContractReadiness;
  blockingReasons: string[];
  refusalReasons: string[];
  insufficientData: string[];
}): CurrentWorkingPerspectiveUpdateContractDecisionPreviewStatus {
  if (!hasContractPreview) {
    return "no_current_working_perspective_update_contract_preview";
  }
...
function determineRecommendedDecision(
  status: CurrentWorkingPerspectiveUpdateContractDecisionPreviewStatus,
): CurrentWorkingPerspectiveUpdateContractOperatorDecisionPreview["recommended_operator_decision"] {
  if (
    status ===
    "ready_for_future_current_working_perspective_update_contract_record_write"
  ) {
    return "approve_for_current_working_perspective_update_contract_record";
  }
  if (status === "blocked") {
...
function isContractPreview(
  value: unknown,
): value is CurrentWorkingPerspectiveUpdateContractPreview {
  return (
    isRecord(value) &&
    ...
    isRecord(
      value.would_write_current_working_perspective_update_contract_record_preview,
    )
  );
}
