...
} from "@/lib/workplane/handoff-send-contract-record-review";
import {
...
export function readHandoffSendContractRecordReviewForWebV01(
  input: {
  ...
  if (input.store_result || input.records) {
    return buildHandoffSendContractRecordReviewV01({
  ...
  if (!isSafeHandoffSendContractReadDbPathV01(input.db_path)) {
    return buildHandoffSendContractRecordReviewV01({
  ...
  const db = new Database(resolvedPath, {
    readonly: true,
    fileMustExist: true,
  }) as Database.Database & HandoffSendContractWriteDbLike;
  try {
    if (!handoffSendContractWriteSchemaExistsV01(db)) {
      return buildHandoffSendContractRecordReviewV01({
        store_result: {
          store_version: "handoff_send_contract_store.v0.1",
          ...
          records: [],
          receipt: {
            receipt_version: "handoff_send_contract_receipt.v0.1",
            record_id: null,
    ...
    const result = input.selected_record_id
      ? readHandoffSendContractRecordByIdV01(input.selected_record_id, {
          db,
        })
      : listHandoffSendContractRecordsV01({ db, limit: 50 });
    return buildHandoffSendContractRecordReviewV01({
      store_result: result,
      selected_record_id: input.selected_record_id ?? null,
      source_refs: input.source_refs,
...
export function isSafeHandoffSendContractReadDbPathV01(
  value: unknown,
): value is string {
...
function createEmptyNoSideEffects(): HandoffSendContractNoSideEffects {
  return {
    handoff_send_contract_record_written: false,
    handoff_send_contract_receipt_written: false,
    handoff_send_contract_persisted: false,
    handoff_send_contract_written: false,
    handoff_sent: false,
