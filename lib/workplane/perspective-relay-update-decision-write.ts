...
import {
  ...
  type PerspectiveRelayUpdateDecisionReceipt,
  type PerspectiveRelayUpdateDecisionStoreResult,
  type PerspectiveRelayUpdateDecisionWriteAuthorityBoundary,
  type PerspectiveRelayUpdateDecisionWriteInput,
  type PerspectiveRelayUpdateDecisionWriteStatus,
} from "@/types/perspective-relay-update-decision-write";

export const PERSPECTIVE_RELAY_UPDATE_DECISION_WRITE_TABLE =
  "perspective_relay_update_decision_records" as const;

export interface PerspectiveRelayUpdateDecisionWriteDbLike {
...
export interface PerspectiveRelayUpdateDecisionWriteListOptions {
  idempotency_key?: string;
  operator_ref?: string;
  limit?: number;
}

interface PerspectiveRelayUpdateDecisionWriteRow {
  record_id: string;
  idempotency_key: string;
  created_at: string;
  scope: string;
...
interface ValidationResult {
  ...
  input: PerspectiveRelayUpdateDecisionWriteInput | null;
...
const allowedRequestedSideEffectKeys = new Set([
  ...
  "can_create_perspective_relay_update_decision_receipt",
...
export const perspectiveRelayUpdateDecisionWriteSchemaSqlV01 = `
CREATE TABLE IF NOT EXISTS perspective_relay_update_decision_records (
...
CREATE INDEX IF NOT EXISTS idx_perspective_relay_update_decision_records_scope_created
  ON perspective_relay_update_decision_records(scope, created_at, record_id);

CREATE INDEX IF NOT EXISTS idx_perspective_relay_update_decision_records_operator
  ON perspective_relay_update_decision_records(scope, operator_ref, created_at, record_id);
`;

export function ensurePerspectiveRelayUpdateDecisionWriteSchemaV01(
  db: PerspectiveRelayUpdateDecisionWriteDbLike,
): void {
  db.exec(perspectiveRelayUpdateDecisionWriteSchemaSqlV01);
}

export function perspectiveRelayUpdateDecisionWriteSchemaExistsV01(
  db: PerspectiveRelayUpdateDecisionWriteDbLike,
): boolean {
  const row = db
    .prepare(
    ...
    .get(PERSPECTIVE_RELAY_UPDATE_DECISION_WRITE_TABLE) as
    | { name?: string }
    | undefined;
  return row?.name === PERSPECTIVE_RELAY_UPDATE_DECISION_WRITE_TABLE;
}

export function validatePerspectiveRelayUpdateDecisionWriteInputV01(
  input: unknown,
): ValidationResult {
  if (!isRecord(input)) {
  ...
  return validationResult({
    ...
    input:
      reasons.length === 0
        ? (input as unknown as PerspectiveRelayUpdateDecisionWriteInput)
        : null,
    idempotency_key: idempotencyKey,
  });
}

export function writePerspectiveRelayUpdateDecisionRecordV01(
  input: unknown,
  options: { db: PerspectiveRelayUpdateDecisionWriteDbLike },
): PerspectiveRelayUpdateDecisionStoreResult {
  const validation = validatePerspectiveRelayUpdateDecisionWriteInputV01(input);
  ...
  ensurePerspectiveRelayUpdateDecisionWriteSchemaV01(options.db);
  const record = buildPerspectiveRelayUpdateDecisionRecord(
    validation as ValidationResult & {
      ok: true;
      input: PerspectiveRelayUpdateDecisionWriteInput;
      idempotency_key: string;
    },
  );
  const existing = readPerspectiveRelayUpdateDecisionRecordByIdempotencyKeyV01(
...
export function refusePerspectiveRelayUpdateDecisionWriteV01(
  input: unknown,
  extraReasons: string[] = [],
): PerspectiveRelayUpdateDecisionStoreResult {
  const validation = validatePerspectiveRelayUpdateDecisionWriteInputV01(input);
...
export function readPerspectiveRelayUpdateDecisionRecordByIdV01(
  recordId: string,
  options: { db: PerspectiveRelayUpdateDecisionWriteDbLike },
): PerspectiveRelayUpdateDecisionStoreResult {
  if (!perspectiveRelayUpdateDecisionWriteSchemaExistsV01(options.db)) {
    return storeResult(
      "schema_missing",
      null,
      [],
  ...
  const row = options.db
    .prepare(
      `SELECT * FROM perspective_relay_update_decision_records
       WHERE record_id = ? AND scope = ?`,
    )
    .get(recordId, PERSPECTIVE_RELAY_UPDATE_DECISION_SCOPE) as
    | PerspectiveRelayUpdateDecisionWriteRow
    | undefined;
  if (!row) {
    return storeResult(
      "not_found",
...
export function readPerspectiveRelayUpdateDecisionRecordByIdempotencyKeyV01(
  ...
  const row = options.db
    ...
    .get(idempotencyKey, PERSPECTIVE_RELAY_UPDATE_DECISION_SCOPE) as
    | PerspectiveRelayUpdateDecisionWriteRow
    | undefined;
  if (!row) {
    return storeResult(
...
export function listPerspectiveRelayUpdateDecisionRecordsV01(
  options: { db: PerspectiveRelayUpdateDecisionWriteDbLike } & PerspectiveRelayUpdateDecisionWriteListOptions,
): PerspectiveRelayUpdateDecisionStoreResult {
  if (!perspectiveRelayUpdateDecisionWriteSchemaExistsV01(options.db)) {
    return storeResult(
      "schema_missing",
  ...
  const clauses = ["scope = ?"];
  const params: unknown[] = [PERSPECTIVE_RELAY_UPDATE_DECISION_SCOPE];
  ...
  const rows = options.db
    ...
    .all(...params, limit) as PerspectiveRelayUpdateDecisionWriteRow[];
...
export function createPerspectiveRelayUpdateDecisionWriteAuthorityBoundaryV01({
  writeNow,
}: {
  writeNow: boolean;
}): PerspectiveRelayUpdateDecisionWriteAuthorityBoundary {
  return {
    ...
    can_create_perspective_relay_update_decision_record: writeNow,
    can_create_perspective_relay_update_decision_receipt: writeNow,
    can_write_perspective_relay_update_decision: writeNow,
    ...
    notes: [
      "Authority is limited to one local PerspectiveRelayUpdate decision record and receipt.",
      "This writer cannot update PerspectiveUnit, NextWorkBias, CWP, relay, handoff, memory, metrics, upstream ledgers, provider, GitHub, Codex, PR, autonomous, graph, vector, RAG, crawler, browser, or Workbench action state.",
    ],
  };
}
...
function validateDecisionPreview(preview: Record<string, unknown> | null): string[] {
  ...
  if (preview.scope !== PERSPECTIVE_RELAY_UPDATE_DECISION_SCOPE) {
  ...
  if (
    ...
    reasons.push(
      "decision_preview_not_ready_for_future_perspective_relay_update_decision_record_write",
    );
  }
  if (
  ...
  if (!material) {
    reasons.push(
      "would_write_perspective_relay_update_decision_record_preview_missing",
    );
  } else {
...
function validateWouldWriteMaterial(material: Record<string, unknown>): string[] {
  const reasons: string[] = [];
  const selectedRefs = safeStringArray(material.selected_candidate_refs);
  const selectableRefs = safeStringArray(material.selectable_candidate_refs);
  const selectedSummaries = Array.isArray(material.selected_candidate_summaries)
    ? material.selected_candidate_summaries
    : [];
  if (selectedRefs.length === 0) reasons.push("selected_candidate_refs_missing");
  if (!isSafeRefArray(material.selected_candidate_refs)) {
    reasons.push("selected_candidate_refs_unsafe");
  }
  if (!isSafeRefArray(material.selectable_candidate_refs)) {
    reasons.push("selectable_candidate_refs_unsafe");
  }
  if (selectedRefs.some((ref) => !selectableRefs.includes(ref))) {
    reasons.push("selected_candidate_refs_not_subset_of_selectable_refs");
  }
  ...
  const selectedByTarget = getRecord(material, "selected_candidate_refs_by_target");
  const selectableByTarget = getRecord(material, "selectable_candidate_refs_by_target");
  for (const target of candidateTargets) {
    if (!isSafeRefArray(selectedByTarget?.[target])) {
      reasons.push(`selected_${target}_candidate_refs_unsafe`);
    }
    if (!isSafeRefArray(selectableByTarget?.[target])) {
      reasons.push(`selectable_${target}_candidate_refs_unsafe`);
    }
  }
  if (
    typeof material.source_perspective_relay_update_candidate_bridge_preview_ref ===
      "string" &&
    !isCandidateIngressPublicSafeRefV01(
      material.source_perspective_relay_update_candidate_bridge_preview_ref,
    )
  ) {
    reasons.push("source_perspective_relay_update_candidate_bridge_preview_ref_unsafe");
  }
  const selectedSummaryRefs = new Set<string>();
  for (const item of selectedSummaries) {
    if (!isRecord(item)) {
      reasons.push("selected_candidate_summary_malformed");
      continue;
    }
    if (
      typeof item.candidate_ref !== "string" ||
      !selectedRefs.includes(item.candidate_ref)
    ) {
      reasons.push("selected_candidate_summary_not_selected");
    } else {
      selectedSummaryRefs.add(item.candidate_ref);
    }
    if (typeof item.target !== "string" || !candidateTargets.has(item.target)) {
      reasons.push("selected_candidate_summary_target_invalid");
    }
    if (typeof item.bucket !== "string" || !candidateBuckets.has(item.bucket)) {
      reasons.push("selected_candidate_summary_bucket_invalid");
    }
    if (
      typeof item.summary !== "string" ||
      containsCandidateIngressUnsafeMarkerV01(item.summary)
    ) {
      reasons.push("selected_candidate_summary_unsafe");
    }
    if (!isSafeRefArray(item.evidence_refs)) {
      reasons.push("selected_candidate_summary_evidence_refs_unsafe");
    }
  }
  if (selectedRefs.some((ref) => !selectedSummaryRefs.has(ref))) {
    reasons.push("selected_candidate_summary_missing");
  }
  return uniqueCandidateIngressStringsV01(reasons);
}
...
function buildPerspectiveRelayUpdateDecisionRecord(
  ...
  const material =
    validation.input.decision_preview
      .would_write_perspective_relay_update_decision_record_preview;
  const createdAt = validation.input.operator_approval.approved_at;
  const selectedSummaries = material.selected_candidate_summaries;
  const baseRecord = {
    record_version: PERSPECTIVE_RELAY_UPDATE_DECISION_RECORD_VERSION,
    idempotency_key: validation.idempotency_key,
    created_at: createdAt,
    scope: PERSPECTIVE_RELAY_UPDATE_DECISION_SCOPE,
    ...
    selected_candidate_refs_by_target: material.selected_candidate_refs_by_target,
    selected_perspective_unit_candidate_refs:
      material.selected_candidate_refs_by_target.perspective_unit,
    selected_next_work_bias_candidate_refs:
      material.selected_candidate_refs_by_target.next_work_bias,
    selected_continuity_relay_candidate_refs:
      material.selected_candidate_refs_by_target.continuity_relay,
    selected_candidate_summaries: selectedSummaries,
    perspective_unit_candidates: selectedSummaries.filter(
      (candidate) => candidate.target === "perspective_unit",
    ),
    next_work_bias_candidates: selectedSummaries.filter(
      (candidate) => candidate.target === "next_work_bias",
    ),
    continuity_relay_candidates: selectedSummaries.filter(
    ...
    authority_boundary: createPerspectiveRelayUpdateDecisionWriteAuthorityBoundaryV01({
      writeNow: true,
    }),
    notes: validation.input.notes ?? [],
  } satisfies Omit<PerspectiveRelayUpdateDecisionRecord, "record_id" | "record_fingerprint">;
  const fingerprint = hashJson(baseRecord);
  return {
    ...baseRecord,
    record_id: `perspective-relay-update-decision:${fingerprint.slice(0, 24)}`,
...
}): PerspectiveRelayUpdateDecisionReceipt {
  return {
    receipt_version: PERSPECTIVE_RELAY_UPDATE_DECISION_RECEIPT_VERSION,
    ...
    store_ref: record
      ? `${PERSPECTIVE_RELAY_UPDATE_DECISION_WRITE_TABLE}:${record.record_id}`
      : null,
...
function storeResult(
  ...
  return {
    store_version: PERSPECTIVE_RELAY_UPDATE_DECISION_STORE_VERSION,
    scope: PERSPECTIVE_RELAY_UPDATE_DECISION_SCOPE,
    status,
...
function noSideEffects({
  ...
  return {
    perspective_relay_update_decision_record_written: recordWritten,
    perspective_relay_update_decision_receipt_written: receiptWritten,
    perspective_relay_update_decision_persisted: persisted,
    perspective_unit_written: false,
    next_work_bias_written: false,
    current_working_perspective_updated: false,
    continuity_relay_written: false,
...
function rowToRecord(
  row: PerspectiveRelayUpdateDecisionWriteRow,
): PerspectiveRelayUpdateDecisionRecord {
  return JSON.parse(row.record_json) as PerspectiveRelayUpdateDecisionRecord;
}

function rowToReceipt(
  row: PerspectiveRelayUpdateDecisionWriteRow,
): PerspectiveRelayUpdateDecisionReceipt {
  return JSON.parse(row.receipt_json) as PerspectiveRelayUpdateDecisionReceipt;
}
