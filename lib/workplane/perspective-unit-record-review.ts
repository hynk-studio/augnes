import {
  ...
  type PerspectiveUnitNoSideEffectsSummary,
  type PerspectiveUnitRecordReview,
  type PerspectiveUnitRecordReviewAuthorityBoundary,
  type PerspectiveUnitRecordReviewInput,
  type PerspectiveUnitRecordReviewStatus,
  type PerspectiveUnitRecordSummary,
} from "@/types/perspective-unit-record-review";
import {
...
const forbiddenNoSideEffectFields: Array<keyof PerspectiveUnitNoSideEffects> = [
...
export function buildPerspectiveUnitRecordReviewV01(
  input: PerspectiveUnitRecordReviewInput = {},
): PerspectiveUnitRecordReview {
  const asOf = input.as_of ?? new Date(0).toISOString();
  const scope = input.scope ?? PERSPECTIVE_UNIT_SCOPE;
  ...
  const validRecords = evaluations
    ...
    .filter((record): record is PerspectiveUnitRecord => Boolean(record))
  ...
  const materialSummary = {
    ...
    perspective_unit_entry_count: sum(
      validSummaries.map((summary) => summary.perspective_unit_entry_count),
    ),
    reinforce_count: sum(
      validSummaries.map((summary) => summary.reinforce_count),
    ),
    weaken_or_warn_count: sum(
      validSummaries.map((summary) => summary.weaken_or_warn_count),
    ),
    retire_or_deprioritize_count: sum(
      validSummaries.map((summary) => summary.retire_or_deprioritize_count),
    ),
    split_or_review_count: sum(
      validSummaries.map((summary) => summary.split_or_review_count),
    ),
  };

  return {
    review_version: PERSPECTIVE_UNIT_RECORD_REVIEW_VERSION,
    scope,
    as_of: asOf,
    ...
    input_summary: {
      ...
      selected_perspective_unit_candidate_ref_count:
        materialSummary.selected_perspective_unit_candidate_count,
      perspective_unit_entry_count: materialSummary.perspective_unit_entry_count,
    ...
    perspective_unit_material_summary: materialSummary,
    receipt_no_side_effects_summary: sideEffects,
    blocked_reasons: uniqueStrings([
      ...problemRecordIds.map(
        (recordId) => `problem_perspective_unit_record:${recordId}`,
      ),
    ...
    operator_review_checklist: [
      "review_latest_perspective_unit_record",
      "confirm_receipt_allows_only_scoped_perspective_unit_record_write",
      "confirm_no_next_work_bias_cwp_relay_handoff_memory_metric_or_external_mutation",
    ],
    would_not_do: [
    ...
    authority_boundary:
      createPerspectiveUnitRecordReviewAuthorityBoundaryV01(),
  };
}

export function createPerspectiveUnitRecordReviewAuthorityBoundaryV01(): PerspectiveUnitRecordReviewAuthorityBoundary {
  return {
    ...
    notes: [
      "Read-only review of already-read scoped PerspectiveUnit records.",
...
function evaluateRecord(
  ...
  summary: PerspectiveUnitRecordSummary;
} {
  const problemReasons: string[] = [];
  if (!isPerspectiveUnitRecord(record)) {
    problemReasons.push("perspective_unit_record_malformed");
  }
  ...
  const entries = safeEntryArray(maybeRecord.perspective_unit_entries);
  const receiptValid = storeSideEffectProblemReasons.length === 0;
  if (!receiptValid) {
    problemReasons.push("perspective_unit_receipt_no_side_effects_invalid");
  }
  const summary = {
    record_id:
    ...
    reinforce_count: countEntries(entries, "reinforce"),
    weaken_or_warn_count: countEntries(entries, "weaken_or_warn"),
    retire_or_deprioritize_count: countEntries(entries, "retire_or_deprioritize"),
    split_or_review_count: countEntries(entries, "split_or_review"),
    record_fingerprint:
  ...
  return {
    valid,
    record: valid ? (record as PerspectiveUnitRecord) : null,
    summary,
  };
}

function isPerspectiveUnitRecord(
  value: unknown,
): value is PerspectiveUnitRecord {
  if (!isRecord(value)) return false;
  return (
    value.record_version === PERSPECTIVE_UNIT_RECORD_VERSION &&
    typeof value.record_id === "string" &&
    typeof value.idempotency_key === "string" &&
    typeof value.created_at === "string" &&
    value.scope === PERSPECTIVE_UNIT_SCOPE &&
    ...
    Array.isArray(value.perspective_unit_entries) &&
    typeof value.perspective_unit_entry_count === "number" &&
    value.review_status === "recorded_as_scoped_perspective_unit" &&
    typeof value.record_fingerprint === "string" &&
    isRecord(value.authority_boundary) &&
    value.authority_boundary.can_write_perspective_unit === true &&
...
function storeResultSideEffectProblemReasons(
  storeResult: PerspectiveUnitStoreResult | null,
): string[] {
  ...
  for (const sideEffects of [
    ...
    if (
      ...
      reasons.push("perspective_unit_receipt_no_side_effects_invalid");
    }
  }
...
function buildNoSideEffectsSummary(
  storeResult: PerspectiveUnitStoreResult | null,
): PerspectiveUnitNoSideEffectsSummary {
  const sideEffects = [
    ...(storeResult?.receipt?.no_side_effects
      ? [storeResult.receipt.no_side_effects]
      : []),
  ...
  return {
    perspective_unit_record_written_count: countTruthy(
      sideEffects,
      "perspective_unit_record_written",
    ),
    perspective_unit_receipt_written_count: countTruthy(
      sideEffects,
      "perspective_unit_receipt_written",
    ),
    perspective_unit_persisted_count: countTruthy(
      sideEffects,
...
}): PerspectiveUnitRecordReviewStatus {
...
function buildInsufficientDataReasons({
  ...
  reviewStatus: PerspectiveUnitRecordReviewStatus;
  selectedRecordId: string | null;
  selectedRecordFound: boolean;
}): string[] {
  if (reviewStatus === "no_records") {
    return ["perspective_unit_records_missing"];
  }
  if (reviewStatus === "schema_missing") {
    return ["perspective_unit_schema_missing"];
  }
  if (selectedRecordId && !selectedRecordFound) {
    return ["selected_perspective_unit_record_missing"];
  }
  return [];
}

function compareRecordsNewestFirst(
  left: PerspectiveUnitRecord,
  right: PerspectiveUnitRecord,
): number {
...
function compareSummariesNewestFirst(
  left: PerspectiveUnitRecordSummary,
  right: PerspectiveUnitRecordSummary,
): number {
  return right.created_at.localeCompare(left.created_at) ||
    right.record_id.localeCompare(left.record_id);
}
...
function countTruthy(
  sideEffects: Array<Partial<PerspectiveUnitNoSideEffects>>,
  field: keyof PerspectiveUnitNoSideEffects,
): number {
...
function countEntries(
  ...
  return entries.filter(
    (entry) => entry.lifecycle_directive === lifecycleDirective,
  ).length;
}
