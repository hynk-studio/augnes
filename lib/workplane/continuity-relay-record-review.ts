import {
  ...
  type ContinuityRelayNoSideEffectsSummary,
  type ContinuityRelayRecordReview,
  type ContinuityRelayRecordReviewAuthorityBoundary,
  type ContinuityRelayRecordReviewInput,
  type ContinuityRelayRecordReviewStatus,
  type ContinuityRelayRecordSummary,
} from "@/types/continuity-relay-record-review";
import {
...
const forbiddenNoSideEffectFields: Array<keyof ContinuityRelayNoSideEffects> = [
...
export function buildContinuityRelayRecordReviewV01(
  input: ContinuityRelayRecordReviewInput = {},
): ContinuityRelayRecordReview {
  const asOf = input.as_of ?? new Date(0).toISOString();
  const scope = input.scope ?? CONTINUITY_RELAY_SCOPE;
  ...
  const validRecords = evaluations
    ...
    .filter((record): record is ContinuityRelayRecord => Boolean(record))
  ...
  const materialSummary = {
    ...
    continuity_relay_entry_count: sum(
      validSummaries.map((summary) => summary.continuity_relay_entry_count),
    ),
    preserve_anchor_count: sum(
      validSummaries.map((summary) => summary.preserve_anchor_count),
    ),
    warn_anchor_count: sum(
      validSummaries.map((summary) => summary.warn_anchor_count),
    ),
    stop_if_missing_count: sum(
      validSummaries.map((summary) => summary.stop_if_missing_count),
    ),
    next_focus_count: sum(
      validSummaries.map((summary) => summary.next_focus_count),
    ),
    relay_update_suggestion_count: sum(
      validSummaries.map((summary) => summary.relay_update_suggestion_count),
    ),
  };

  return {
    review_version: CONTINUITY_RELAY_RECORD_REVIEW_VERSION,
    scope,
    as_of: asOf,
    ...
    input_summary: {
      ...
      selected_continuity_relay_candidate_ref_count:
        materialSummary.selected_continuity_relay_candidate_count,
      continuity_relay_entry_count: materialSummary.continuity_relay_entry_count,
    ...
    continuity_relay_material_summary: materialSummary,
    receipt_no_side_effects_summary: sideEffects,
    blocked_reasons: uniqueStrings([
      ...problemRecordIds.map(
        (recordId) => `problem_continuity_relay_record:${recordId}`,
      ),
    ...
    operator_review_checklist: [
      "review_latest_continuity_relay_record",
      "confirm_receipt_allows_only_scoped_continuity_relay_record_write",
      "confirm_no_perspective_unit_next_work_bias_cwp_live_relay_handoff_memory_metric_or_external_mutation",
    ],
    would_not_do: [
    ...
    authority_boundary:
      createContinuityRelayRecordReviewAuthorityBoundaryV01(),
  };
}

export function createContinuityRelayRecordReviewAuthorityBoundaryV01(): ContinuityRelayRecordReviewAuthorityBoundary {
  return {
    ...
    notes: [
      "Read-only review of already-read scoped ContinuityRelay records.",
...
function evaluateRecord(
  ...
  summary: ContinuityRelayRecordSummary;
} {
  const problemReasons: string[] = [];
  if (!isContinuityRelayRecord(record)) {
    problemReasons.push("continuity_relay_record_malformed");
  }
  ...
  const entries = safeEntryArray(maybeRecord.continuity_relay_entries);
  const receiptValid = storeSideEffectProblemReasons.length === 0;
  if (!receiptValid) {
    problemReasons.push("continuity_relay_receipt_no_side_effects_invalid");
  }
  const summary = {
    record_id:
    ...
    preserve_anchor_count: countEntries(entries, "preserve_anchor"),
    warn_anchor_count: countEntries(entries, "warn_anchor"),
    stop_if_missing_count: countEntries(entries, "stop_if_missing"),
    next_focus_count: countEntries(entries, "next_focus"),
    relay_update_suggestion_count: countEntries(
      entries,
  ...
  return {
    valid,
    record: valid ? (record as ContinuityRelayRecord) : null,
    summary,
  };
}

function isContinuityRelayRecord(
  value: unknown,
): value is ContinuityRelayRecord {
  if (!isRecord(value)) return false;
  return (
    value.record_version === CONTINUITY_RELAY_RECORD_VERSION &&
    typeof value.record_id === "string" &&
    typeof value.idempotency_key === "string" &&
    typeof value.created_at === "string" &&
    value.scope === CONTINUITY_RELAY_SCOPE &&
    typeof value.operator_ref === "string" &&
    Array.isArray(value.source_refs) &&
    Array.isArray(value.evidence_refs) &&
    Array.isArray(value.source_perspective_relay_update_decision_record_refs) &&
    Array.isArray(value.related_perspective_unit_record_refs) &&
    Array.isArray(value.related_next_work_bias_record_refs) &&
    Array.isArray(value.selected_continuity_relay_candidate_refs) &&
    Array.isArray(value.continuity_relay_entries) &&
    typeof value.continuity_relay_entry_count === "number" &&
    value.review_status === "recorded_as_scoped_continuity_relay" &&
    typeof value.record_fingerprint === "string" &&
    isRecord(value.authority_boundary) &&
    value.authority_boundary.can_write_continuity_relay === true &&
...
function storeResultSideEffectProblemReasons(
  storeResult: ContinuityRelayStoreResult | null,
): string[] {
  ...
  for (const sideEffects of [
    ...
    if (
      ...
      reasons.push("continuity_relay_receipt_no_side_effects_invalid");
    }
  }
...
function buildNoSideEffectsSummary(
  storeResult: ContinuityRelayStoreResult | null,
): ContinuityRelayNoSideEffectsSummary {
  const sideEffects = [
    ...(storeResult?.receipt?.no_side_effects
      ? [storeResult.receipt.no_side_effects]
      : []),
  ...
  return {
    continuity_relay_record_written_count: countTruthy(
      sideEffects,
      "continuity_relay_record_written",
    ),
    continuity_relay_receipt_written_count: countTruthy(
      sideEffects,
      "continuity_relay_receipt_written",
    ),
    continuity_relay_persisted_count: countTruthy(
      sideEffects,
      "continuity_relay_persisted",
    ),
    continuity_relay_written_count: countTruthy(
      sideEffects,
      "continuity_relay_written",
    ),
    perspective_unit_written_count: countTruthy(
      sideEffects,
      "perspective_unit_written",
    ),
    ...
    current_working_perspective_mutated_count: countTruthy(
      sideEffects,
      "current_working_perspective_mutated",
    ),
    continuity_relay_updated_count: countTruthy(
      sideEffects,
      "continuity_relay_updated",
    ),
...
}): ContinuityRelayRecordReviewStatus {
...
function buildInsufficientDataReasons({
  ...
  reviewStatus: ContinuityRelayRecordReviewStatus;
  selectedRecordId: string | null;
  selectedRecordFound: boolean;
}): string[] {
  if (reviewStatus === "no_records") {
    return ["continuity_relay_records_missing"];
  }
  if (reviewStatus === "schema_missing") {
    return ["continuity_relay_schema_missing"];
  }
  if (selectedRecordId && !selectedRecordFound) {
    return ["selected_continuity_relay_record_missing"];
  }
  return [];
}

function compareRecordsNewestFirst(
  left: ContinuityRelayRecord,
  right: ContinuityRelayRecord,
): number {
...
function compareSummariesNewestFirst(
  left: ContinuityRelayRecordSummary,
  right: ContinuityRelayRecordSummary,
): number {
  return right.created_at.localeCompare(left.created_at) ||
    right.record_id.localeCompare(left.record_id);
}
...
function countTruthy(
  sideEffects: Array<Partial<ContinuityRelayNoSideEffects>>,
  field: keyof ContinuityRelayNoSideEffects,
): number {
...
function countEntries(
  ...
  relayDirective: string,
): number {
  return entries.filter(
    (entry) => entry.relay_directive === relayDirective,
  ).length;
}
