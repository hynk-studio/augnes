...
import {
  ...
  type ProjectHistoryIntakeDecisionAuthorityBoundary,
  type ProjectHistoryIntakeOperatorDecisionPreview,
  type ProjectHistoryIntakeOperatorDecisionPreviewInput,
  type ProjectHistoryIntakeOperatorDecisionStatus,
  type ProjectHistoryIntakeRecommendedOperatorDecision,
  type ProjectHistoryWouldWriteCandidateRecordPreview,
} from "@/types/project-history-intake-decision";
import {
...
] as const satisfies readonly (keyof ProjectHistoryIntakeAuthorityBoundary)[];
...
export function buildProjectHistoryIntakeOperatorDecisionPreviewV01({
  ...
  scope,
  source_refs,
}: ProjectHistoryIntakeOperatorDecisionPreviewInput = {}): ProjectHistoryIntakeOperatorDecisionPreview {
  const previewStatus = getPreviewSourceStatus(project_history_intake_preview);
  const intakePreview = isProjectHistoryIntakePreview(project_history_intake_preview)
    ? project_history_intake_preview
    : null;
  ...
  const refusalReasons = uniqueCandidateIngressStringsV01([
    ...(previewStatus === "wrong_version" ? ["project_history_intake_preview_wrong_version"] : []),
    ...(previewStatus === "malformed" ? ["project_history_intake_preview_malformed"] : []),
    ...unknownSelectedRefs.map(() => "unknown_selected_project_history_candidate_ref"),
  ...
  return {
    runtime: "augnes",
    preview_version: PROJECT_HISTORY_INTAKE_OPERATOR_DECISION_PREVIEW_VERSION,
    ...
    input_summary: {
      has_valid_project_history_intake_preview: previewStatus === "supplied",
    ...
    source_status: {
      project_history_intake_preview: previewStatus,
    ...
    write_readiness: {
      ...
      readiness_label: writeReady
        ...
        : "not_ready_for_project_history_candidate_record_write",
    ...
    approval_requirements: [
      "valid_project_history_intake_preview",
      "selected_candidate_refs_from_intake_preview",
      "operator_privacy_review_confirmation",
      "idempotency_key",
      "source_ref_operator_ref_project_ref_and_evidence_refs",
      "operator_confirmation_for_project_history_candidate_record_only",
    ],
    ...
    review_checklist: [
      "review_project_history_candidates_before_record_write",
      "confirm_selected_candidate_refs_belong_to_project_history_intake_preview",
      "confirm_privacy_review_confirmation_ref",
      "confirm_idempotency_key",
      "confirm_project_history_candidate_record_is_not_memory_or_perspective_state",
    ],
    non_goals: [
      "memory_write",
    ...
    notes: [
      "Project history intake operator decision preview consumes only the already-built intake preview.",
      "Readiness here is not write authority; the scoped writer separately validates operator approval.",
    ],
  };
}

export function createProjectHistoryIntakeOperatorDecisionAuthorityBoundaryV01(): ProjectHistoryIntakeDecisionAuthorityBoundary {
  return {
    read_only: true,
    advisory_only: true,
    ...
    notes: [
      "Project history intake decision preview is read-only and advisory.",
      "It cannot create project history records, memory, Perspective, CWP, relay, handoff state, provider calls, GitHub calls, Codex execution, or autonomous actions.",
    ],
  };
}
...
function getPreviewSourceStatus(
  value: unknown,
): ProjectHistoryIntakeOperatorDecisionPreview["source_status"]["project_history_intake_preview"] {
  if (!value) return "missing";
  if (!isRecord(value) || value.preview_version !== PROJECT_HISTORY_INTAKE_PREVIEW_VERSION) {
    return "wrong_version";
  }
  return isProjectHistoryIntakePreview(value) ? "supplied" : "malformed";
}

function isProjectHistoryIntakePreview(
  value: unknown,
): value is ProjectHistoryIntakePreview {
  if (!isRecord(value)) return false;
  if (value.preview_version !== PROJECT_HISTORY_INTAKE_PREVIEW_VERSION) return false;
  return (
...
function intakeAuthorityIsReadOnly(
  authority: ProjectHistoryIntakeAuthorityBoundary,
): boolean {
  return (
...
function getIngestableCandidates(
  material: ProjectHistoryCandidateMaterial,
): CandidateIngressNormalizedCandidate[] {
  return [
    ...material.timeline_event_candidates,
    ...material.project_state_summary_candidates,
    ...material.decision_candidates,
    ...material.requirement_candidates,
    ...material.changed_artifact_candidates,
...
function buildWouldWritePreview({
  ...
  intakePreview: ProjectHistoryIntakePreview | null;
  ...
  evidenceRefs: string[];
  sourceRefs: string[];
}): ProjectHistoryWouldWriteCandidateRecordPreview {
  ...
  return {
    proposed_record_kind: "project_history_intake_record.v0.1",
    proposed_receipt_kind: "project_history_intake_receipt.v0.1",
    ...
    review_summary: selectedCandidates.length > 0
      ? "Project history candidate material selected for a future scoped candidate ingest record."
      : "No project history candidate refs have been operator-selected.",
  };
}
...
function determineStatus({
  ...
  selectableRefs: string[];
}): ProjectHistoryIntakeOperatorDecisionStatus {
  if (previewStatus === "missing") return "no_project_history_intake_preview";
...
function determineRecommendedDecision({
  ...
  decisionStatus: ProjectHistoryIntakeOperatorDecisionStatus;
  insufficientDataReasons: string[];
  missingEvidence: string[];
  blockingReasons: string[];
}): ProjectHistoryIntakeRecommendedOperatorDecision {
  ...
  if (insufficientDataReasons.some((reason) => reason.includes("missing"))) return "defer_until_history_material_supplied";
  if (decisionStatus === "ready_for_future_candidate_record_write") return "approve_for_project_history_candidate_ingest";
