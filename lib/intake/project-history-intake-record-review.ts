import {
  PROJECT_HISTORY_INTAKE_RECORD_REVIEW_VERSION,
  type ProjectHistoryIntakeNoSideEffectsSummary,
  type ProjectHistoryIntakeRecordReview,
  type ProjectHistoryIntakeRecordReviewAuthorityBoundary,
  type ProjectHistoryIntakeRecordReviewInput,
  type ProjectHistoryIntakeRecordReviewStatus,
  type ProjectHistoryIntakeRecordSummary,
} from "@/types/project-history-intake-record-review";
import {
...
export function buildProjectHistoryIntakeRecordReviewV01(
  input: ProjectHistoryIntakeRecordReviewInput = {},
): ProjectHistoryIntakeRecordReview {
  const asOf = input.as_of ?? new Date(0).toISOString();
  const scope = input.scope ?? PROJECT_HISTORY_INTAKE_SCOPE;
  ...
  const validRecords = evaluations
    ...
    .filter((record): record is ProjectHistoryIntakeRecord => Boolean(record))
  ...
  return {
    review_version: PROJECT_HISTORY_INTAKE_RECORD_REVIEW_VERSION,
    scope,
    as_of: asOf,
    ...
    blocked_reasons: problemRecordIds.map(
      (recordId) => `problem_project_history_intake_record:${recordId}`,
    ),
    ...
    operator_review_checklist: [
      "review_latest_project_history_candidate_ingest_record",
      ...
      "confirm_project_history_intake_record_is_not_memory_or_perspective_state",
    ],
    would_not_do: [
    ...
    authority_boundary:
      createProjectHistoryIntakeRecordReviewAuthorityBoundaryV01(),
  };
}

export function createProjectHistoryIntakeRecordReviewAuthorityBoundaryV01(): ProjectHistoryIntakeRecordReviewAuthorityBoundary {
  return {
    ...
    notes: [
      "Read-only review of already-read project history candidate ingest records.",
...
function evaluateRecord(recordLike: unknown): {
  ...
  summary: ProjectHistoryIntakeRecordSummary;
  valid: boolean;
} {
  ...
  return {
    ...
    summary: {
      ...
      receipt_no_side_effects_valid: problemReasons.every(
        (reason) =>
          !/side_effect|mutation|memory|handoff|provider|github|codex/i.test(
            reason,
          ),
      ),
...
function recordProblems(record: Partial<ProjectHistoryIntakeRecord> | null): string[] {
  ...
  if (record.scope !== PROJECT_HISTORY_INTAKE_SCOPE) {
...
function buildNoSideEffectsSummary(
  summaries: ProjectHistoryIntakeRecordSummary[],
): ProjectHistoryIntakeNoSideEffectsSummary {
  const invalidCount = summaries.filter(
    (summary) => !summary.receipt_no_side_effects_valid,
  ).length;
  return {
    project_history_intake_record_written_count: summaries.length,
    project_history_intake_receipt_written_count: summaries.length,
    project_history_persisted_as_candidate_record_count: summaries.length,
...
}): ProjectHistoryIntakeRecordReviewStatus {
...
function buildInsufficientDataReasons({
  ...
  reviewStatus: ProjectHistoryIntakeRecordReviewStatus;
  selectedRecordId: string | null;
  selectedRecordFound: boolean;
}): string[] {
  const reasons: string[] = [];
  if (reviewStatus === "no_records") {
    reasons.push("project_history_intake_records_missing");
  }
  if (reviewStatus === "schema_missing") {
    reasons.push("project_history_intake_record_schema_missing");
  }
  if (selectedRecordId && !selectedRecordFound) {
    reasons.push("selected_project_history_intake_record_missing");
  }
  return reasons;
}

function compareRecordsNewestFirst(
  left: ProjectHistoryIntakeRecord,
  right: ProjectHistoryIntakeRecord,
): number {
...
function compareSummariesNewestFirst(
  left: ProjectHistoryIntakeRecordSummary,
  right: ProjectHistoryIntakeRecordSummary,
): number {
  return `${right.created_at}:${right.record_id}`.localeCompare(
    `${left.created_at}:${left.record_id}`,
  );
