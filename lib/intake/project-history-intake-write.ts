...
import { PROJECT_HISTORY_INTAKE_OPERATOR_DECISION_PREVIEW_VERSION } from "@/types/project-history-intake-decision";
import {
  PROJECT_HISTORY_INTAKE_RECEIPT_VERSION,
  PROJECT_HISTORY_INTAKE_RECORD_VERSION,
  PROJECT_HISTORY_INTAKE_SCOPE,
  PROJECT_HISTORY_INTAKE_STORE_VERSION,
  type ProjectHistoryIntakeNoSideEffects,
  type ProjectHistoryIntakeRecord,
  type ProjectHistoryIntakeReceipt,
  type ProjectHistoryIntakeStoreResult,
  type ProjectHistoryIntakeWriteAuthorityBoundary,
  type ProjectHistoryIntakeWriteInput,
  type ProjectHistoryIntakeWriteStatus,
} from "@/types/project-history-intake-write";

export const PROJECT_HISTORY_INTAKE_WRITE_TABLE =
  "project_history_intake_records" as const;

export interface ProjectHistoryIntakeWriteDbLike {
...
export interface ProjectHistoryIntakeWriteListOptions {
...
interface ProjectHistoryIntakeWriteRow {
  record_id: string;
  idempotency_key: string;
  created_at: string;
  scope: string;
...
interface ValidationResult {
  ...
  input: ProjectHistoryIntakeWriteInput | null;
...
const allowedRequestedSideEffectKeys = new Set([
  ...
  "can_write_project_history_candidate_record",
]);
...
export const projectHistoryIntakeWriteSchemaSqlV01 = `
CREATE TABLE IF NOT EXISTS project_history_intake_records (
...
CREATE INDEX IF NOT EXISTS idx_project_history_intake_records_scope_created
  ON project_history_intake_records(scope, created_at, record_id);

CREATE INDEX IF NOT EXISTS idx_project_history_intake_records_operator
  ON project_history_intake_records(scope, operator_ref, created_at, record_id);

CREATE INDEX IF NOT EXISTS idx_project_history_intake_records_project
  ON project_history_intake_records(scope, project_ref, created_at, record_id);
`;

export function ensureProjectHistoryIntakeWriteSchemaV01(
  db: ProjectHistoryIntakeWriteDbLike,
): void {
  db.exec(projectHistoryIntakeWriteSchemaSqlV01);
}

export function projectHistoryIntakeWriteSchemaExistsV01(
  db: ProjectHistoryIntakeWriteDbLike,
): boolean {
  const row = db
    .prepare(
    ...
    .get(PROJECT_HISTORY_INTAKE_WRITE_TABLE) as { name?: string } | undefined;
  return row?.name === PROJECT_HISTORY_INTAKE_WRITE_TABLE;
}

export function validateProjectHistoryIntakeWriteInputV01(
  input: unknown,
): ValidationResult {
  if (!isRecord(input)) {
  ...
  return validationResult({
    ...
    input:
      reasons.length === 0
        ? (input as unknown as ProjectHistoryIntakeWriteInput)
        : null,
    idempotency_key: idempotencyKey,
  });
}

export function writeProjectHistoryIntakeRecordV01(
  input: unknown,
  options: { db: ProjectHistoryIntakeWriteDbLike },
): ProjectHistoryIntakeStoreResult {
  const validation = validateProjectHistoryIntakeWriteInputV01(input);
  ...
  ensureProjectHistoryIntakeWriteSchemaV01(options.db);
  const record = buildProjectHistoryIntakeRecord(
    validation as ValidationResult & {
      ok: true;
      input: ProjectHistoryIntakeWriteInput;
      idempotency_key: string;
    },
  );
  const existing = readProjectHistoryIntakeRecordByIdempotencyKeyV01(
  ...
  try {
    ...
    options.db
      .prepare(
        `INSERT INTO project_history_intake_records (
          record_id,
          idempotency_key,
          created_at,
          scope,
...
export function refuseProjectHistoryIntakeWriteV01(
  input: unknown,
  extraReasons: string[] = [],
): ProjectHistoryIntakeStoreResult {
  const validation = validateProjectHistoryIntakeWriteInputV01(input);
...
export function readProjectHistoryIntakeRecordByIdV01(
  recordId: string,
  options: { db: ProjectHistoryIntakeWriteDbLike },
): ProjectHistoryIntakeStoreResult {
  ...
  if (!projectHistoryIntakeWriteSchemaExistsV01(options.db)) {
    return storeResult(
      "schema_missing",
      null,
      [],
  ...
  const row = options.db
    .prepare(
      `SELECT * FROM project_history_intake_records
       WHERE record_id = ? AND scope = ?`,
    )
    .get(recordId, PROJECT_HISTORY_INTAKE_SCOPE) as
    | ProjectHistoryIntakeWriteRow
    | undefined;
  if (!row) {
    return storeResult(
      "not_found",
...
export function readProjectHistoryIntakeRecordByIdempotencyKeyV01(
  ...
  const row = options.db
    ...
    .get(idempotencyKey, PROJECT_HISTORY_INTAKE_SCOPE) as
    | ProjectHistoryIntakeWriteRow
    | undefined;
  if (!row) {
    return storeResult(
...
export function listProjectHistoryIntakeRecordsV01(
  options: ProjectHistoryIntakeWriteListOptions & {
    db: ProjectHistoryIntakeWriteDbLike;
  },
): ProjectHistoryIntakeStoreResult {
  if (!projectHistoryIntakeWriteSchemaExistsV01(options.db)) {
  ...
  const clauses = ["scope = ?"];
  const params: unknown[] = [PROJECT_HISTORY_INTAKE_SCOPE];
  ...
  const rows = options.db
    ...
    .all(...params, limit) as ProjectHistoryIntakeWriteRow[];
...
export function createProjectHistoryIntakeWriteAuthorityBoundaryV01({
  writeNow,
}: {
  writeNow: boolean;
}): ProjectHistoryIntakeWriteAuthorityBoundary {
  return {
    ...
    can_write_project_history_candidate_record: writeNow,
    ...
    notes: [
      "Authority is limited to one local project history candidate ingest record and receipt.",
...
function validateDecisionPreview(preview: Record<string, unknown> | null): string[] {
  ...
  if (
    ...
    PROJECT_HISTORY_INTAKE_OPERATOR_DECISION_PREVIEW_VERSION
  ) {
    reasons.push("decision_preview_version_invalid");
  }
  if (preview.scope !== PROJECT_HISTORY_INTAKE_SCOPE) {
...
function validateApproval({
  ...
  const reasons: string[] = [];
  if (
    approval.operator_decision !== "approve_for_project_history_candidate_ingest"
  ) {
...
function buildProjectHistoryIntakeRecord(
  ...
  const recordWithoutFingerprint: Omit<ProjectHistoryIntakeRecord, "record_fingerprint"> = {
    record_version: PROJECT_HISTORY_INTAKE_RECORD_VERSION,
    record_id: recordId,
    idempotency_key: validation.idempotency_key,
    ...
    source_kind: material.source_kind ?? "project_history_digest",
    ...
    authority_boundary: createProjectHistoryIntakeWriteAuthorityBoundaryV01({
      writeNow: true,
    }),
...
}): ProjectHistoryIntakeReceipt {
  return {
    receipt_version: PROJECT_HISTORY_INTAKE_RECEIPT_VERSION,
    ...
    store_ref: record ? `project_history_intake_record:${record.record_id}` : null,
    source_refs: record?.source_refs ?? [],
    no_side_effects: noSideEffects({
      wroteProjectHistoryRecord: wrote && !refused && !idempotentReplay,
    }),
  };
}
...
function createRefusedReceipt(
  ...
  return {
    ...
    store_ref: null,
    source_refs: [],
    no_side_effects: noSideEffects({ wroteProjectHistoryRecord: false }),
  };
}
...
function noSideEffects({
  wroteProjectHistoryRecord,
}: {
  wroteProjectHistoryRecord: boolean;
}): ProjectHistoryIntakeNoSideEffects {
  return {
    project_history_intake_record_written: wroteProjectHistoryRecord,
    project_history_intake_receipt_written: wroteProjectHistoryRecord,
    project_history_persisted_as_candidate_record: wroteProjectHistoryRecord,
...
function rowToRecord(row: ProjectHistoryIntakeWriteRow): ProjectHistoryIntakeRecord {
  return JSON.parse(row.record_json) as ProjectHistoryIntakeRecord;
}

function rowToReceipt(row: ProjectHistoryIntakeWriteRow): ProjectHistoryIntakeReceipt {
  return JSON.parse(row.receipt_json) as ProjectHistoryIntakeReceipt;
}
...
function createRecordId(idempotencyKey: string): string {
  return `project_history_intake_record:${createHash("sha256")
...
function containsRawOrPrivateMarkers(value: unknown): boolean {
  ...
  return (
    /raw_text|raw_digest|raw_excerpt|raw_history/i.test(text) ||
