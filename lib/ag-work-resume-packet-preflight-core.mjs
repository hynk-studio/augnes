...
export function preflightAgWorkResumePacket(packet, options = {}) {
...
function buildOutput({ strict, inputMode, packet, checks, }) {
...
function addCorePacketChecks(packet, checks, strict) {
    ...
    checks.push({
        id: "packet_kind",
        status: packetKind && KNOWN_PACKET_KINDS.has(packetKind)
            ? "pass"
            : strict
                ? "fail"
                : "warn",
        message: packetKind && KNOWN_PACKET_KINDS.has(packetKind)
            ? "Recognized AG resume packet kind."
            : packetKind
                ? `Unknown packet kind: ${packetKind}.`
                : "Packet kind is missing.",
    });
    addRequiredStringCheck(checks, "packet_id", jsonString(packet.packet_id), "Packet id is present.", "Packet id is missing.", "Packet id is a placeholder.", strict);
    ...
    addRequiredStringCheck(checks, "issuer_runtime_instance_id", jsonString(issuer?.runtime_instance_id), "Runtime instance id is present.", "Runtime instance id is missing.", "Runtime instance id is a placeholder.", strict);
...
function addTargetPolicyChecks(packet, checks, strict) {
...
function addRedactionChecks(packet, checks, strict) {
...
function addBoundsChecks(packet, checks, strict) {
...
function addUnsafeContentChecks(rawInput, packet, checks, strict) {
...
function addRequiredStringCheck(checks, id, value, presentMessage, missingMessage, placeholderMessage, strict) {
    let status = "pass";
...
function addArrayPresenceCheck(checks, id, value, presentMessage, missingMessage, strict) {
    checks.push({
        id,
...
function addExpiresAtCheck(checks, value, strict) {
    if (value === null) {
        checks.push({
            id: "expires_at",
            status: "pass",
...
function addRequiredTrueCheck(checks, id, value, passMessage, strict) {
    let status = "pass";
...
function addRequiredFalseCheck(checks, id, value, passMessage, strict) {
    let status = "pass";
    let message = passMessage;
    if (value !== false) {
        status = value === undefined ? strict ? "fail" : "warn" : "fail";
...
function addNoDirectRecordCheck(checks, id, value, label, strict) {
    let status = "pass";
    ...
    if (value === true) {
        ...
        const safe = /requires explicit user\/Core approval/i.test(value) &&
...
function addMaxCountCheck({ checks, id, configuredMax, hardMax, actualCount, label, strict, }) {
    const configuredIsNumber = typeof configuredMax === "number" && Number.isInteger(configuredMax);
    let status = "pass";
...
function checkResumeCodeSemantics(packet, strict) {
...
function collectValuesByKeyPattern(value, pattern, matches = []) {
    if (Array.isArray(value)) {
        for (const item of value)
            collectValuesByKeyPattern(item, pattern, matches);
        return matches;
    }
    if (!isRecord(value))
        return matches;
