...
The builder and CLI reject unsupported validate summary versions, unsupported modes, missing or invalid JSON, unknown result states, candidate count mismatches, unsafe `PASS` summaries with warnings, pointer warnings, blocked reasons, non-`fits_contract` contract fit, non-`ready_for_review` direct validation, non-`sufficient_for_review` basis quality, non-`existing_validator_compatible` shape, authority drift, or non-`actionable_advisory` Worker-Facing Guidance; `PASS with follow-up` summaries with blocked reasons, missing candidate-compatible review material, committed authority, non-advisory Worker-Facing Guidance, blocked direct validation, violating contract fit, or non-compatible shape; `BLOCKED` summaries with empty blocked reasons or review candidate availability claims; authority flag drift, trusted-runtime-state drift, non-review-only candidate material, direct-success alignment drift, output path collisions, and unsafe private/provider/token/browser/source/candidate markers in public snapshots.
