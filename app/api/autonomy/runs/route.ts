...
import type { CreateAutonomyRunInput } from "../../../../types/autonomy-runner-execution";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

const ROUTE_VERSION = "autonomy_runner_runs_route.v0.1" as const;
...
export function GET(request: Request) {
  ...
  if (scope !== SCOPE) return jsonResponse(errorResponse("invalid_scope"), 400);
...
export async function POST(request: Request) {
  ...
  const inputBody = body as CreateAutonomyRunInput & { db_path?: unknown };
  const dbPath = optionalSafeRouteDbPath(inputBody.db_path);
  if (dbPath === false) return jsonResponse(errorResponse("invalid_db_path"), 400);
  const scope = inputBody.scope ?? SCOPE;
  if (scope !== SCOPE) return jsonResponse(errorResponse("invalid_scope"), 400);
  ...
  try {
    ...
    return jsonResponse(
      errorResponse(error instanceof Error ? error.message : "create_failed"),
      400,
    );
  }
}
