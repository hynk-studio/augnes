...
import {
  dogfoodMetricSnapshotWriteSchemaExistsV01,
  listDogfoodMetricSnapshotRecordsV01,
  readDogfoodMetricSnapshotRecordByIdV01,
  readDogfoodMetricSnapshotRecordByIdempotencyKeyV01,
  refuseDogfoodMetricSnapshotWriteV01,
  validateDogfoodMetricSnapshotWriteInputV01,
  writeDogfoodMetricSnapshotRecordV01,
  type DogfoodMetricSnapshotWriteDbLike,
} from "@/lib/dogfooding/dogfood-metric-snapshot-write";
import {
...
const routeVersion = "dogfood_metric_snapshot_route.v0.1" as const;
const safeDogfoodMetricSnapshotDbPathPrefixes = [
  "tmp/dogfood-metric-snapshots/",
  ".tmp/dogfood-metric-snapshots/",
] as const;
...
export async function GET(request: Request) {
  ...
  if (!isSafeDogfoodMetricSnapshotRouteDbPathV01(dbPath)) {
    return jsonResponse(errorResponse("invalid_db_path"), 400);
  }

  const opened = openReadOnlyDogfoodMetricSnapshotDb(dbPath);
  ...
  const db = opened.db;
  try {
    if (!dogfoodMetricSnapshotWriteSchemaExistsV01(db)) {
    ...
    const result = recordId
      ? readDogfoodMetricSnapshotRecordByIdV01(recordId, { db })
      : idempotencyKey
        ? readDogfoodMetricSnapshotRecordByIdempotencyKeyV01(idempotencyKey, {
            db,
          })
        : listDogfoodMetricSnapshotRecordsV01({
            db,
...
export async function POST(request: Request) {
  ...
  if (!isSafeDogfoodMetricSnapshotRouteDbPathV01(requestBody.db_path)) {
    return jsonResponse(errorResponse("invalid_db_path"), 400);
  }

  const validation = validateDogfoodMetricSnapshotWriteInputV01(
    requestBody.input,
  );
  if (!validation.ok) {
    const result = refuseDogfoodMetricSnapshotWriteV01(requestBody.input);
    return jsonResponse(storeResponse(result), storeStatus(result, true));
  }

  const db = openWriteDogfoodMetricSnapshotDb(requestBody.db_path);
  try {
    const result = writeDogfoodMetricSnapshotRecordV01(requestBody.input, {
      db,
    });
    return jsonResponse(storeResponse(result), storeStatus(result, true));
  } finally {
...
export function isSafeDogfoodMetricSnapshotRouteDbPathV01(
  value: unknown,
): value is string {
  ...
  if (
    !safeDogfoodMetricSnapshotDbPathPrefixes.some((prefix) =>
...
function openReadOnlyDogfoodMetricSnapshotDb(
  dbPath: string,
):
  | { db: Database.Database & DogfoodMetricSnapshotWriteDbLike }
...
function openWriteDogfoodMetricSnapshotDb(
  dbPath: string,
): Database.Database & DogfoodMetricSnapshotWriteDbLike {
  const resolvedPath = resolve(process.cwd(), dbPath);
...
function storeStatus(
  result: DogfoodMetricSnapshotStoreResult,
  fromCreate = false,
): number {
...
function storeResponse(result: DogfoodMetricSnapshotStoreResult) {
  return {
    ...
    dogfood_metric_snapshot_record_written: result.status === "written",
    dogfood_metric_snapshot_receipt_written: result.status === "written",
    dogfood_metric_snapshot_persisted: result.status === "written",
...
function errorResponse(errorCode: string) {
  return {
    ...
    store_version: DOGFOOD_METRIC_SNAPSHOT_STORE_VERSION,
    dogfood_metric_snapshot_record_written: false,
    dogfood_metric_snapshot_receipt_written: false,
    dogfood_metric_snapshot_persisted: false,
    dogfood_metrics_global_state_updated: false,
    reuse_outcome_ledger_written: false,
    expected_observed_delta_written: false,
    work_episode_written: false,
