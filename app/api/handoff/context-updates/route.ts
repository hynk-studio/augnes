...
const routeVersion =
  "operator_approved_handoff_context_update_route.v0.1" as const;
...
export async function GET(request: Request) {
  ...
  if (!isSafeHandoffContextUpdateRouteDbPathV01(dbPath)) {
    return jsonResponse(errorResponse("invalid_db_path"), 400);
  }

  const opened = openReadOnlyContextUpdateDb(dbPath);
  ...
  try {
    ...
    const result = recordId
      ? readHandoffContextUpdateRecordByIdV01(recordId, { db })
      : idempotencyKey
        ? readHandoffContextUpdateRecordByIdempotencyKeyV01(idempotencyKey, {
            db,
          })
        : listHandoffContextUpdateRecordsV01({
            db,
...
export async function POST(request: Request) {
  ...
  if (!isSafeHandoffContextUpdateRouteDbPathV01(requestBody.db_path)) {
    return jsonResponse(errorResponse("invalid_db_path"), 400);
  }
  ...
  if (!validation.ok) {
    const result = refuseOperatorApprovedHandoffContextUpdateWriteV01(
      requestBody.input,
    );
    return jsonResponse(storeResponse(result), storeStatus(result, true));
  }

  const db = openWriteContextUpdateDb(requestBody.db_path);
  try {
    const result = writeOperatorApprovedHandoffContextUpdateV01(
      requestBody.input,
      { db },
    );
    return jsonResponse(storeResponse(result), storeStatus(result, true));
...
export function isSafeHandoffContextUpdateRouteDbPathV01(
  value: unknown,
): value is string {
  ...
  if (
    !safeContextUpdateDbPathPrefixes.some((prefix) => value.startsWith(prefix))
  ) {
    return false;
  }
...
function openReadOnlyContextUpdateDb(
  dbPath: string,
):
  | { db: Database.Database & HandoffContextUpdateWriteDbLike }
...
function openWriteContextUpdateDb(
  dbPath: string,
): Database.Database & HandoffContextUpdateWriteDbLike {
  const resolvedPath = resolve(process.cwd(), dbPath);
...
function storeStatus(
  result: OperatorApprovedHandoffContextUpdateStoreResult,
  fromCreate = false,
): number {
...
function storeResponse(result: OperatorApprovedHandoffContextUpdateStoreResult) {
  return {
...
function errorResponse(errorCode: string) {
  return {
    ...
    store_version: OPERATOR_APPROVED_HANDOFF_CONTEXT_UPDATE_STORE_VERSION,
    operator_approved_handoff_context_update_record_written: false,
    no_side_effects: {
