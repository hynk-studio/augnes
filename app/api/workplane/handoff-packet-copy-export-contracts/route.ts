...
import {
  ...
  readHandoffPacketCopyExportContractRecordByIdempotencyKeyV01,
  refuseHandoffPacketCopyExportContractWriteV01,
  validateHandoffPacketCopyExportContractWriteInputV01,
  writeHandoffPacketCopyExportContractRecordV01,
...
const routeVersion = "handoff_packet_copy_export_contract_route.v0.1" as const;
...
export async function GET(request: Request) {
  ...
  if (!isSafeHandoffPacketCopyExportContractRouteDbPathV01(dbPath)) {
  ...
  try {
    ...
    const result = recordId
      ? readHandoffPacketCopyExportContractRecordByIdV01(recordId, { db })
      : idempotencyKey
        ? readHandoffPacketCopyExportContractRecordByIdempotencyKeyV01(
            idempotencyKey,
            { db },
...
export async function POST(request: Request) {
  ...
  if (!isSafeHandoffPacketCopyExportContractRouteDbPathV01(requestBody.db_path)) {
    return jsonResponse(errorResponse("invalid_db_path"), 400);
  }

  const validation = validateHandoffPacketCopyExportContractWriteInputV01(
    requestBody.input,
  );
  if (!validation.ok) {
    const result = refuseHandoffPacketCopyExportContractWriteV01(requestBody.input);
    return jsonResponse(storeResponse(result), storeStatus(result, true));
  }

  const db = openWriteDb(requestBody.db_path);
  try {
    const result = writeHandoffPacketCopyExportContractRecordV01(
      requestBody.input,
      { db },
    );
    return jsonResponse(storeResponse(result), storeStatus(result, true));
...
export function isSafeHandoffPacketCopyExportContractRouteDbPathV01(
  value: unknown,
): value is string {
...
function openReadOnlyDb(
  ...
  | { db: Database.Database & HandoffPacketCopyExportContractWriteDbLike }
...
function openWriteDb(
  dbPath: string,
): Database.Database & HandoffPacketCopyExportContractWriteDbLike {
  const resolvedPath = resolve(process.cwd(), dbPath);
...
function storeStatus(
  result: HandoffPacketCopyExportContractStoreResult,
  fromCreate = false,
): number {
...
function storeResponse(result: HandoffPacketCopyExportContractStoreResult) {
  return {
    ...
    handoff_packet_copy_export_contract_receipt_written:
      result.status === "written",
    handoff_packet_copy_export_contract_persisted: result.status === "written",
    handoff_packet_copy_export_contract_written: result.status === "written",
...
function jsonResponse(body: unknown, status: number) {
  return NextResponse.json(body, {
    ...
    headers: {
      ...
      "x-augnes-handoff-packet-copy-export-contract": routeVersion,
    },
  });
}
