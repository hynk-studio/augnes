...
import {
  ...
  refuseHandoffSendWriteV01,
  validateHandoffSendWriteInputV01,
...
const routeVersion = "handoff_send_route.v0.1" as const;
...
export async function GET(request: Request) {
  ...
  if (!isSafeHandoffSendRouteDbPathV01(dbPath)) {
  ...
  try {
    ...
    const result = recordId
      ? readHandoffSendRecordByIdV01(recordId, { db })
      : idempotencyKey
        ? readHandoffSendRecordByIdempotencyKeyV01(idempotencyKey, {
            db,
          })
        : listHandoffSendRecordsV01({
            db,
            ...
            fulfillment_status:
              url.searchParams.get("fulfillment_status") || undefined,
...
export async function POST(request: Request) {
  ...
  if (!isSafeHandoffSendRouteDbPathV01(requestBody.db_path)) {
    return jsonResponse(errorResponse("invalid_db_path"), 400);
  }

  const validation = validateHandoffSendWriteInputV01(
    requestBody.input,
  );
  if (!validation.ok) {
    const result = refuseHandoffSendWriteV01(requestBody.input);
    return jsonResponse(storeResponse(result), storeStatus(result, true));
  }

  const db = openWriteDb(requestBody.db_path);
  try {
    const result = writeHandoffSendRecordV01(requestBody.input, {
      db,
    });
    return jsonResponse(storeResponse(result), storeStatus(result, true));
  } finally {
...
export function isSafeHandoffSendRouteDbPathV01(
  value: unknown,
): value is string {
...
function openReadOnlyDb(
  ...
  | { db: Database.Database & HandoffSendWriteDbLike }
...
function openWriteDb(
  dbPath: string,
): Database.Database & HandoffSendWriteDbLike {
  const resolvedPath = resolve(process.cwd(), dbPath);
...
function storeResponse(result: HandoffSendStoreResult) {
  return {
    ...
    handoff_send_record_written: result.status === "written",
    handoff_send_receipt_written: result.status === "written",
    handoff_send_persisted: result.status === "written",
    local_handoff_send_fulfillment_recorded: result.status === "written",
    handoff_sent: false,
    ...
    no_side_effects: result.receipt.no_side_effects,
  };
}
...
function jsonResponse(body: unknown, status: number) {
  return NextResponse.json(body, {
    ...
    headers: {
      ...
      "x-augnes-handoff-send": routeVersion,
    },
  });
}
