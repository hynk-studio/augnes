...
import {
  handoffSendContractWriteSchemaExistsV01,
  listHandoffSendContractRecordsV01,
  readHandoffSendContractRecordByIdV01,
  readHandoffSendContractRecordByIdempotencyKeyV01,
  refuseHandoffSendContractWriteV01,
  validateHandoffSendContractWriteInputV01,
  writeHandoffSendContractRecordV01,
  type HandoffSendContractWriteDbLike,
} from "@/lib/workplane/handoff-send-contract-write";
import {
...
const routeVersion = "handoff_send_contract_route.v0.1" as const;
const safeDbPathPrefixes = [
  "tmp/handoff-send-contracts/",
  ".tmp/handoff-send-contracts/",
] as const;
...
export async function GET(request: Request) {
  ...
  if (!isSafeHandoffSendContractRouteDbPathV01(dbPath)) {
  ...
  const db = opened.db;
  try {
    if (!handoffSendContractWriteSchemaExistsV01(db)) {
    ...
    const result = recordId
      ? readHandoffSendContractRecordByIdV01(recordId, { db })
      : idempotencyKey
        ? readHandoffSendContractRecordByIdempotencyKeyV01(idempotencyKey, {
            db,
          })
        : listHandoffSendContractRecordsV01({
            db,
            operator_ref: url.searchParams.get("operator_ref") || undefined,
            send_surface: url.searchParams.get("send_surface") || undefined,
            delivery_mode: url.searchParams.get("delivery_mode") || undefined,
            recipient_ref: url.searchParams.get("recipient_ref") || undefined,
...
export async function POST(request: Request) {
  ...
  if (!isSafeHandoffSendContractRouteDbPathV01(requestBody.db_path)) {
    return jsonResponse(errorResponse("invalid_db_path"), 400);
  }

  const validation = validateHandoffSendContractWriteInputV01(
    requestBody.input,
  );
  if (!validation.ok) {
    const result = refuseHandoffSendContractWriteV01(requestBody.input);
    return jsonResponse(storeResponse(result), storeStatus(result, true));
  }

  const db = openWriteDb(requestBody.db_path);
  try {
    const result = writeHandoffSendContractRecordV01(requestBody.input, {
      db,
    });
    return jsonResponse(storeResponse(result), storeStatus(result, true));
  } finally {
...
export function isSafeHandoffSendContractRouteDbPathV01(
  value: unknown,
): value is string {
...
function openReadOnlyDb(
  ...
  | { db: Database.Database & HandoffSendContractWriteDbLike }
...
function openWriteDb(
  dbPath: string,
): Database.Database & HandoffSendContractWriteDbLike {
  const resolvedPath = resolve(process.cwd(), dbPath);
...
function storeStatus(
  result: HandoffSendContractStoreResult,
  fromCreate = false,
): number {
...
function storeResponse(result: HandoffSendContractStoreResult) {
  return {
    ...
    handoff_send_contract_record_written: result.status === "written",
    handoff_send_contract_receipt_written: result.status === "written",
    handoff_send_contract_persisted: result.status === "written",
    handoff_send_contract_written: result.status === "written",
    handoff_sent: false,
...
function jsonResponse(body: unknown, status: number) {
  return NextResponse.json(body, {
    ...
    headers: {
      ...
      "x-augnes-handoff-send-contract": routeVersion,
    },
  });
}
