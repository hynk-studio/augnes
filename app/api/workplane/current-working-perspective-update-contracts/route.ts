...
import {
  currentWorkingPerspectiveUpdateContractWriteSchemaExistsV01,
  listCurrentWorkingPerspectiveUpdateContractRecordsV01,
  readCurrentWorkingPerspectiveUpdateContractRecordByIdV01,
  readCurrentWorkingPerspectiveUpdateContractRecordByIdempotencyKeyV01,
  refuseCurrentWorkingPerspectiveUpdateContractWriteV01,
  validateCurrentWorkingPerspectiveUpdateContractWriteInputV01,
  writeCurrentWorkingPerspectiveUpdateContractRecordV01,
  type CurrentWorkingPerspectiveUpdateContractWriteDbLike,
} from "@/lib/workplane/current-working-perspective-update-contract-write";
import {
...
const routeVersion =
  "current_working_perspective_update_contract_route.v0.1" as const;
const safeDbPathPrefixes = [
  "tmp/current-working-perspective-update-contracts/",
  ".tmp/current-working-perspective-update-contracts/",
] as const;
...
export async function GET(request: Request) {
  ...
  if (!isSafeCurrentWorkingPerspectiveUpdateContractRouteDbPathV01(dbPath)) {
  ...
  const db = opened.db;
  try {
    if (!currentWorkingPerspectiveUpdateContractWriteSchemaExistsV01(db)) {
    ...
    const result = recordId
      ? readCurrentWorkingPerspectiveUpdateContractRecordByIdV01(recordId, {
          db,
        })
      : idempotencyKey
        ? readCurrentWorkingPerspectiveUpdateContractRecordByIdempotencyKeyV01(
...
export async function POST(request: Request) {
  ...
  if (
    !isSafeCurrentWorkingPerspectiveUpdateContractRouteDbPathV01(
      requestBody.db_path,
    )
  ) {
    return jsonResponse(errorResponse("invalid_db_path"), 400);
  ...
  const validation = validateCurrentWorkingPerspectiveUpdateContractWriteInputV01(
    requestBody.input,
  );
  if (!validation.ok) {
    const result = refuseCurrentWorkingPerspectiveUpdateContractWriteV01(
      requestBody.input,
    );
  ...
  try {
    const result = writeCurrentWorkingPerspectiveUpdateContractRecordV01(
      requestBody.input,
      { db },
    );
    return jsonResponse(storeResponse(result), storeStatus(result, true));
...
export function isSafeCurrentWorkingPerspectiveUpdateContractRouteDbPathV01(
  value: unknown,
): value is string {
...
function openReadOnlyDb(
  ...
  | {
      db: Database.Database &
        CurrentWorkingPerspectiveUpdateContractWriteDbLike;
    }
...
function openWriteDb(
  dbPath: string,
): Database.Database & CurrentWorkingPerspectiveUpdateContractWriteDbLike {
  const resolvedPath = resolve(process.cwd(), dbPath);
...
function storeStatus(
  result: CurrentWorkingPerspectiveUpdateContractStoreResult,
  fromCreate = false,
): number {
...
function storeResponse(result: CurrentWorkingPerspectiveUpdateContractStoreResult) {
  return {
...
function errorResponse(errorCode: string) {
  return {
    ...
    current_working_perspective_update_contract_receipt_written: false,
    current_working_perspective_update_contract_persisted: false,
    current_working_perspective_update_contract_written: false,
    current_working_perspective_updated: false,
    current_working_perspective_mutated: false,
    current_working_perspective_live_state_written: false,
    current_working_perspective_update_applied: false,
...
function jsonResponse(body: unknown, status: number) {
  return NextResponse.json(body, {
    ...
    headers: {
      ...
      "x-augnes-current-working-perspective-update-contract-route":
        routeVersion,
    },
  });
}
