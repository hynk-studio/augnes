...
import {
  listPerspectiveUnitRecordsV01,
  perspectiveUnitWriteSchemaExistsV01,
  readPerspectiveUnitRecordByIdV01,
  readPerspectiveUnitRecordByIdempotencyKeyV01,
  refusePerspectiveUnitWriteV01,
  validatePerspectiveUnitWriteInputV01,
  writePerspectiveUnitRecordV01,
  type PerspectiveUnitWriteDbLike,
} from "@/lib/workplane/perspective-unit-write";
import {
...
const routeVersion = "perspective_unit_route.v0.1" as const;
const safePerspectiveUnitDbPathPrefixes = [
  "tmp/perspective-units/",
  ".tmp/perspective-units/",
] as const;
...
export async function GET(request: Request) {
  ...
  if (!isSafePerspectiveUnitRouteDbPathV01(dbPath)) {
    return jsonResponse(errorResponse("invalid_db_path"), 400);
  }

  const opened = openReadOnlyPerspectiveUnitDb(dbPath);
  ...
  const db = opened.db;
  try {
    if (!perspectiveUnitWriteSchemaExistsV01(db)) {
    ...
    const result = recordId
      ? readPerspectiveUnitRecordByIdV01(recordId, { db })
      : idempotencyKey
        ? readPerspectiveUnitRecordByIdempotencyKeyV01(idempotencyKey, {
            db,
          })
        : listPerspectiveUnitRecordsV01({
            db,
...
export async function POST(request: Request) {
  ...
  if (!isSafePerspectiveUnitRouteDbPathV01(requestBody.db_path)) {
    return jsonResponse(errorResponse("invalid_db_path"), 400);
  }

  const validation = validatePerspectiveUnitWriteInputV01(
    requestBody.input,
  );
  if (!validation.ok) {
    const result = refusePerspectiveUnitWriteV01(requestBody.input);
    return jsonResponse(storeResponse(result), storeStatus(result, true));
  }

  const db = openWritePerspectiveUnitDb(requestBody.db_path);
  try {
    const result = writePerspectiveUnitRecordV01(requestBody.input, {
      db,
    });
    return jsonResponse(storeResponse(result), storeStatus(result, true));
  } finally {
...
export function isSafePerspectiveUnitRouteDbPathV01(
  value: unknown,
): value is string {
  ...
  if (
    !safePerspectiveUnitDbPathPrefixes.some((prefix) =>
...
function openReadOnlyPerspectiveUnitDb(
  dbPath: string,
):
  | { db: Database.Database & PerspectiveUnitWriteDbLike }
...
function openWritePerspectiveUnitDb(
  dbPath: string,
): Database.Database & PerspectiveUnitWriteDbLike {
  const resolvedPath = resolve(process.cwd(), dbPath);
...
function storeStatus(
  result: PerspectiveUnitStoreResult,
  fromCreate = false,
): number {
...
function storeResponse(result: PerspectiveUnitStoreResult) {
  return {
    ...
    perspective_unit_record_written: result.status === "written",
    perspective_unit_receipt_written: result.status === "written",
    perspective_unit_persisted: result.status === "written",
    perspective_unit_written: result.status === "written",
...
function errorResponse(errorCode: string) {
  return {
    ...
    store_version: PERSPECTIVE_UNIT_STORE_VERSION,
    perspective_unit_record_written: false,
    perspective_unit_receipt_written: false,
    perspective_unit_persisted: false,
    next_work_bias_written: false,
