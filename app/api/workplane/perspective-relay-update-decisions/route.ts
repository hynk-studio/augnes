...
import {
  listPerspectiveRelayUpdateDecisionRecordsV01,
  perspectiveRelayUpdateDecisionWriteSchemaExistsV01,
  readPerspectiveRelayUpdateDecisionRecordByIdV01,
  readPerspectiveRelayUpdateDecisionRecordByIdempotencyKeyV01,
  refusePerspectiveRelayUpdateDecisionWriteV01,
  validatePerspectiveRelayUpdateDecisionWriteInputV01,
  writePerspectiveRelayUpdateDecisionRecordV01,
  type PerspectiveRelayUpdateDecisionWriteDbLike,
} from "@/lib/workplane/perspective-relay-update-decision-write";
import {
...
const routeVersion = "perspective_relay_update_decision_route.v0.1" as const;
const safePerspectiveRelayUpdateDecisionDbPathPrefixes = [
  "tmp/perspective-relay-update-decisions/",
  ".tmp/perspective-relay-update-decisions/",
] as const;
...
export async function GET(request: Request) {
  ...
  if (!isSafePerspectiveRelayUpdateDecisionRouteDbPathV01(dbPath)) {
    return jsonResponse(errorResponse("invalid_db_path"), 400);
  }

  const opened = openReadOnlyPerspectiveRelayUpdateDecisionDb(dbPath);
  ...
  const db = opened.db;
  try {
    if (!perspectiveRelayUpdateDecisionWriteSchemaExistsV01(db)) {
    ...
    const result = recordId
      ? readPerspectiveRelayUpdateDecisionRecordByIdV01(recordId, { db })
      : idempotencyKey
        ? readPerspectiveRelayUpdateDecisionRecordByIdempotencyKeyV01(idempotencyKey, {
            db,
          })
        : listPerspectiveRelayUpdateDecisionRecordsV01({
            db,
...
export async function POST(request: Request) {
  ...
  if (!isSafePerspectiveRelayUpdateDecisionRouteDbPathV01(requestBody.db_path)) {
    return jsonResponse(errorResponse("invalid_db_path"), 400);
  }

  const validation = validatePerspectiveRelayUpdateDecisionWriteInputV01(
    requestBody.input,
  );
  if (!validation.ok) {
    const result = refusePerspectiveRelayUpdateDecisionWriteV01(requestBody.input);
    return jsonResponse(storeResponse(result), storeStatus(result, true));
  }

  const db = openWritePerspectiveRelayUpdateDecisionDb(requestBody.db_path);
  try {
    const result = writePerspectiveRelayUpdateDecisionRecordV01(requestBody.input, {
      db,
    });
    return jsonResponse(storeResponse(result), storeStatus(result, true));
  } finally {
...
export function isSafePerspectiveRelayUpdateDecisionRouteDbPathV01(
  value: unknown,
): value is string {
  ...
  if (
    !safePerspectiveRelayUpdateDecisionDbPathPrefixes.some((prefix) =>
...
function openReadOnlyPerspectiveRelayUpdateDecisionDb(
  dbPath: string,
):
  | { db: Database.Database & PerspectiveRelayUpdateDecisionWriteDbLike }
...
function openWritePerspectiveRelayUpdateDecisionDb(
  dbPath: string,
): Database.Database & PerspectiveRelayUpdateDecisionWriteDbLike {
  const resolvedPath = resolve(process.cwd(), dbPath);
...
function storeStatus(
  result: PerspectiveRelayUpdateDecisionStoreResult,
  fromCreate = false,
): number {
...
function storeResponse(result: PerspectiveRelayUpdateDecisionStoreResult) {
  return {
    ...
    perspective_relay_update_decision_record_written: result.status === "written",
    perspective_relay_update_decision_receipt_written: result.status === "written",
    perspective_relay_update_decision_persisted: result.status === "written",
...
function errorResponse(errorCode: string) {
  return {
    ...
    store_version: PERSPECTIVE_RELAY_UPDATE_DECISION_STORE_VERSION,
    perspective_relay_update_decision_record_written: false,
    perspective_relay_update_decision_receipt_written: false,
    perspective_relay_update_decision_persisted: false,
    perspective_unit_written: false,
    next_work_bias_written: false,
    current_working_perspective_updated: false,
    continuity_relay_written: false,
