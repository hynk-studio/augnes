...
const routeVersion =
  "research_candidate_manual_global_dogfood_canonical_perspective_update_route.v0.1" as const;
...
export async function GET(request: Request) {
  ...
  return NextResponse.json({
    ...
    readback: readResearchCandidateManualGlobalDogfoodCanonicalPerspectiveUpdate({
      scope: DEFAULT_SCOPE,
      receiptId,
      idempotencyKey,
      limit,
...
export async function POST(request: Request) {
  ...
  const result = writeResearchCandidateManualGlobalDogfoodCanonicalPerspectiveUpdate(body);
  return NextResponse.json(
    {
      ok: result.ok,
      route_version: routeVersion,
      ...
      canonical_perspective_update_record: result.canonical_perspective_update_record,
