...
} from "@/lib/research-candidate-review/manual-global-dogfood-perspective-state-mutation-write";
import {
...
const routeVersion =
  "research_candidate_manual_global_dogfood_perspective_state_mutation_route.v0.1" as const;
...
export async function GET(request: Request) {
  ...
  return NextResponse.json({
    ...
    readback: readResearchCandidateManualGlobalDogfoodPerspectiveStateMutation({
      scope: DEFAULT_SCOPE,
      receiptId,
      idempotencyKey,
      limit,
...
export async function POST(request: Request) {
  ...
  const result = writeResearchCandidateManualGlobalDogfoodPerspectiveStateMutation(body);
  return NextResponse.json(
    {
      ok: result.ok,
      route_version: routeVersion,
      ...
      perspective_state_mutation_record: result.perspective_state_mutation_record,
