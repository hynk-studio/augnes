...
} from "@/lib/research-candidate-review/manual-global-dogfood-perspective-adapter-write";
import {
...
const routeVersion =
  "research_candidate_manual_global_dogfood_perspective_adapter_route.v0.1" as const;
...
export async function POST(request: Request) {
  ...
  const result = writeResearchCandidateManualGlobalDogfoodPerspectiveAdapter(body);
  return NextResponse.json(
    {
      ok: result.ok,
      route_version: routeVersion,
      ...
      perspective_adapter_record: result.perspective_adapter_record,
