...
export type FeedbackControlsExpandedIntentSummary = {
...
export type FeedbackControlsExpandedAggregateSummary = {
  aggregate_id: string;
  target_surface: string;
  target_surface_ref: string;
  target_candidate_ref: string;
...
export type FeedbackControlsExpandedRuleFailureCandidateSummary = {
...
export type FeedbackControlsExpandedAuditPanelProps = {
...
export const feedbackControlsExpandedDefaultAuthorityBoundary = {
...
export function FeedbackControlsExpandedAuditPanel({
  feedbackIntents = [],
  feedbackAggregates = [],
  ruleFailureCandidates = [],
  authorityBoundary = feedbackControlsExpandedDefaultAuthorityBoundary,
  className,
}: FeedbackControlsExpandedAuditPanelProps) {
  return (
    <section
      className={className ?? "perspective-inspector-section"}
      aria-label="Feedback Controls Expansion audit summaries"
      data-feedback-controls-expanded-audit="local-intent-only read-only no-feedback-persistence no-candidate-mutation no-product-write"
    >
      <div className="perspective-constellation-shell-header">
        <div>
          <p className="panel-eyebrow">Feedback Controls Expansion</p>
          <h3>Audit panel is read-only</h3>
        ...
        <div className="perspective-constellation-shell-status">
          <span className="status-pill">optional summaries</span>
      ...
      <section className="perspective-inspector-section">
        <h4>Feedback intent summaries</h4>
        {feedbackIntents.length > 0 ? (
          ...
          <p>No feedback intents supplied. The legacy folded preview remains fixture-backed.</p>
        )}
      </section>
