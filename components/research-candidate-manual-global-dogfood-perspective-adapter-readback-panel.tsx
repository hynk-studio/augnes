...
export function ResearchCandidateManualGlobalDogfoodPerspectiveAdapterReadbackPanel({
  ...
  return (
    <section
      className="perspective-inspector-section manual-global-dogfood-perspective-adapter-readback"
      aria-label="Manual global dogfood Perspective adapter readback"
      data-augnes-authority="manual-perspective-adapter-readback no-current-working-perspective no-existing-canonical-state no-promotion no-memory no-work no-proof no-metrics"
    >
      <div className="perspective-constellation-shell-header">
        <div>
          <p className="panel-eyebrow">AUGNES / Perspective Adapter Readback</p>
          <h4>Manual Perspective adapter record readback</h4>
          <p>
            Readback is limited to manual-derived Perspective adapter receipts,
            adapter records, and rollback metadata. It does not update
      ...
      {readback ? (
        <>
          <div className="perspective-formation-summary-grid">
            <div>
              ...
              <small>source refs preserved in manual Perspective adapter tables</small>
            </div>
            <div>
            ...
            <div className="perspective-detail-stack">
              {readback.records_by_receipt.length > 0 ? (
                readback.records_by_receipt.map((recordSet) => (
                  <PerspectiveAdapterReceiptCard
                    key={recordSet.receipt.receipt_id}
                    recordSet={recordSet}
                  />
                ))
              ) : (
                <p>No manual Perspective adapter receipts have been written.</p>
              )}
            </div>
          </section>
          ...
          <section className="cockpit-surface-card">
            ...
            <div className="perspective-workbench-status-row">
              <BoundaryFlag
                ...
                value={readback.authority_boundary.can_write_perspective_adapter_record}
              />
              <BoundaryFlag
                label="can_write_perspective_adapter_receipt"
                value={readback.authority_boundary.can_write_perspective_adapter_receipt}
              />
              <BoundaryFlag
                label="can_write_perspective_adapter_rollback_metadata"
                value={
                  readback.authority_boundary
                    .can_write_perspective_adapter_rollback_metadata
                }
              />
              <BoundaryFlag
        ...
        <p className="manual-note-runtime-hint">
          No Perspective adapter readback has been loaded yet.
        </p>
      )}
    </section>
  );
...
function PerspectiveAdapterReceiptCard({
  recordSet,
}: {
  recordSet: ResearchCandidateManualGlobalDogfoodPerspectiveAdapterRecordsByReceipt;
}) {
  const { receipt, perspective_adapter_record: record, rollback } = recordSet;
  return (
    <section className="cockpit-surface-card">
      ...
      <dl>
        <dt>source_perspective_adapter_contract_fingerprint</dt>
        <dd>{receipt.source_perspective_adapter_contract_fingerprint}</dd>
        <dt>source_perspective_adapter_review_fingerprint</dt>
        <dd>{receipt.source_perspective_adapter_review_fingerprint}</dd>
        ...
        <dd>{record?.intended_future_adapter_target ?? "record missing"}</dd>
        <dt>default_future_adapter_target</dt>
        <dd>{record?.default_future_adapter_target ?? "record missing"}</dd>
        <dt>adapter_scope_hint</dt>
        <dd>{record?.adapter_scope_hint ?? "record missing"}</dd>
