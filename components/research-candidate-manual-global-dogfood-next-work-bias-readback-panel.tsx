import type {
  ...
  ResearchCandidateManualGlobalDogfoodNextWorkBiasRecordsByReceipt,
} from "@/types/research-candidate-manual-global-dogfood-next-work-bias-write";

export function ResearchCandidateManualGlobalDogfoodNextWorkBiasReadbackPanel({
  readback,
  isLoading = false,
  error,
}: {
  ...
  return (
    <section
      className="perspective-inspector-section manual-global-dogfood-next-work-bias-readback"
      aria-label="Manual global dogfood next-work bias readback"
      data-augnes-authority="manual-next-work-bias-readback no-work no-perspective no-proof no-metrics no-memory"
    >
      <div className="perspective-constellation-shell-header">
        <div>
          <p className="panel-eyebrow">AUGNES / Next-Work Bias Readback</p>
          <h4>Manual next-work bias record readback</h4>
          <p>
            Readback is limited to manual-derived next-work bias receipts,
            records, and rollback metadata. It does not mutate work, write
            Perspective relay or state, promote Perspective, write memory,
      ...
      {readback ? (
        <>
          <div className="perspective-formation-summary-grid">
            <div>
              ...
              <small>source refs preserved in manual bias tables</small>
            </div>
            <div>
            ...
            <div>
              ...
              <strong>
                {String(
                  readback.work_mutated ||
                    readback.perspective_relay_written ||
              ...
              <small>work/Perspective/metrics/proof/memory/product</small>
            </div>
          </div>
            ...
            <div className="perspective-detail-stack">
              {readback.records_by_receipt.length > 0 ? (
                readback.records_by_receipt.map((recordSet) => (
                  <NextWorkBiasReceiptCard
                    key={recordSet.receipt.receipt_id}
                    recordSet={recordSet}
                  />
                ))
              ) : (
                <p>No manual next-work bias receipts have been written.</p>
              )}
            </div>
          </section>
          ...
          <section className="cockpit-surface-card">
            ...
            <div className="perspective-workbench-status-row">
              <BoundaryFlag
                label="can_write_next_work_bias_record"
                value={readback.authority_boundary.can_write_next_work_bias_record}
              />
              <BoundaryFlag
                label="can_write_next_work_bias_receipt"
                value={readback.authority_boundary.can_write_next_work_bias_receipt}
              />
              <BoundaryFlag
              ...
              <BoundaryFlag
                ...
                value={readback.authority_boundary.can_write_perspective_relay}
              />
              <BoundaryFlag
        ...
        <p className="manual-note-runtime-hint">
          No next-work bias readback has been loaded yet.
        </p>
      )}
    </section>
  );
...
function NextWorkBiasReceiptCard({
  recordSet,
}: {
  recordSet: ResearchCandidateManualGlobalDogfoodNextWorkBiasRecordsByReceipt;
}) {
  const { receipt, next_work_bias_record: record, rollback } = recordSet;
  return (
    <section className="cockpit-surface-card">
      ...
      <dl>
        <dt>source_next_work_bias_contract_fingerprint</dt>
        <dd>{receipt.source_next_work_bias_contract_fingerprint}</dd>
        <dt>source_next_work_bias_review_fingerprint</dt>
        <dd>{receipt.source_next_work_bias_review_fingerprint}</dd>
        ...
        <dt>rationale</dt>
        <dd>{record?.rationale ?? "record missing"}</dd>
        <dt>outcome_label</dt>
        ...
        <dd>{record?.bias_strength_hint ?? "record missing"}</dd>
