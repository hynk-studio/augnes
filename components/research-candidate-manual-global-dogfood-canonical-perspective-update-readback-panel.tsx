...
export function ResearchCandidateManualGlobalDogfoodCanonicalPerspectiveUpdateReadbackPanel({
  readback,
  isLoading = false,
  error,
}: {
  ...
  return (
    <section
      className="perspective-inspector-section manual-global-dogfood-canonical-perspective-update-readback"
      aria-label="Manual global dogfood canonical Perspective update readback"
      data-augnes-authority="manual-canonical-perspective-update-readback no-current-working-perspective no-promotion no-memory no-work no-proof no-metrics"
    >
      <div className="perspective-constellation-shell-header">
        <div>
          <p className="panel-eyebrow">AUGNES / Canonical Perspective Update Readback</p>
          <h4>Manual canonical Perspective update record readback</h4>
          <p>
            Readback is limited to manual-derived canonical Perspective update receipts,
      ...
      {readback ? (
        <>
          <div className="perspective-formation-summary-grid">
            <div>
              ...
              <small>source refs preserved in manual canonical update tables</small>
            </div>
            <div>
            ...
            <div className="perspective-detail-stack">
              {readback.records_by_receipt.length > 0 ? (
                readback.records_by_receipt.map((recordSet) => (
                  <CanonicalPerspectiveUpdateReceiptCard
                    key={recordSet.receipt.receipt_id}
                    recordSet={recordSet}
                  />
                ))
              ) : (
                <p>No manual canonical Perspective update receipts have been written.</p>
              )}
            </div>
          </section>
          ...
          <section className="cockpit-surface-card">
            ...
            <div className="perspective-workbench-status-row">
              <BoundaryFlag
                label="can_write_canonical_perspective_update_record"
                value={readback.authority_boundary.can_write_canonical_perspective_update_record}
              />
              <BoundaryFlag
                label="can_write_canonical_perspective_update_receipt"
                value={readback.authority_boundary.can_write_canonical_perspective_update_receipt}
              />
              <BoundaryFlag
              ...
              />
              <BoundaryFlag
                label="can_write_canonical_perspective_update_rollback_metadata"
                value={
                  readback.authority_boundary
                    .can_write_canonical_perspective_update_rollback_metadata
                }
              />
              <BoundaryFlag
        ...
        <p className="manual-note-runtime-hint">
          No canonical Perspective update readback has been loaded yet.
        </p>
      )}
    </section>
  );
...
function CanonicalPerspectiveUpdateReceiptCard({
  recordSet,
}: {
  recordSet: ResearchCandidateManualGlobalDogfoodCanonicalPerspectiveUpdateRecordsByReceipt;
}) {
  const { receipt, canonical_perspective_update_record: record, rollback } = recordSet;
  return (
    <section className="cockpit-surface-card">
      ...
      <dl>
        <dt>source_canonical_perspective_update_contract_fingerprint</dt>
        <dd>{receipt.source_canonical_perspective_update_contract_fingerprint}</dd>
        <dt>source_canonical_perspective_update_review_fingerprint</dt>
        <dd>{receipt.source_canonical_perspective_update_review_fingerprint}</dd>
