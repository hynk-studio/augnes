import {
  WorkplanePanelMetric,
  WorkplanePanelMetricGrid,
  WorkplanePanelShell,
  workplaneBadgeStyle,
  workplaneCopyStyle,
  workplaneItemStyle,
  workplaneListStyle,
} from "@/components/workplane/workplane-panel-shell";
import type { DogfoodMetricSnapshotOperatorDecisionPreview } from "@/types/dogfood-metric-snapshot-decision";

type DogfoodMetricSnapshotDecisionPanelProps = {
  preview: DogfoodMetricSnapshotOperatorDecisionPreview;
};

export function DogfoodMetricSnapshotDecisionPanel({
  preview,
}: DogfoodMetricSnapshotDecisionPanelProps) {
  return (
    <WorkplanePanelShell
      kicker="Metric snapshot decision"
      title="Dogfood Metric Snapshot Operator Decision Preview"
      ariaLabel="Dogfood Metric Snapshot Operator Decision Preview panel"
    >
      <p style={workplaneCopyStyle}>
        Read-only operator decision preview for a future scoped local dogfood
        metric snapshot record. It cannot update global metrics or Perspective
        state.
      </p>

      <WorkplanePanelMetricGrid>
        <WorkplanePanelMetric label="Decision status" value={preview.decision_preview_status} />
        <WorkplanePanelMetric label="recommended decision" value={preview.recommended_operator_decision} />
        <WorkplanePanelMetric label="selected refs" value={String(preview.input_summary.selected_metric_candidate_ref_count)} />
        <WorkplanePanelMetric label="write ready" value={String(preview.write_readiness.write_ready)} />
      </WorkplanePanelMetricGrid>

      <section style={workplaneItemStyle}>
        <span style={workplaneBadgeStyle}>would write preview</span>
        <span style={workplaneCopyStyle}>
          selectable {preview.input_summary.selectable_metric_candidate_ref_count};
          would write {preview.input_summary.would_write_metric_candidate_count}
        </span>
        <span style={workplaneCopyStyle}>
          record{" "}
          {preview.would_write_metric_snapshot_record_preview.proposed_record_kind ?? "none"};
          receipt{" "}
          {preview.would_write_metric_snapshot_record_preview.proposed_receipt_kind ?? "none"}
        </span>
      </section>

      <ReasonList
        title="decision blockers"
        reasons={[
          ...preview.blocking_reasons,
          ...preview.missing_evidence,
          ...preview.refusal_reasons,
          ...preview.write_readiness.current_insufficient_data,
        ]}
      />

      <section style={workplaneItemStyle}>
        <span style={workplaneBadgeStyle}>approval requirements</span>
        <ul style={workplaneListStyle}>
          {preview.approval_requirements.slice(0, 8).map((item) => (
            <li key={item} style={workplaneItemStyle}>
              <span style={workplaneCopyStyle}>{item}</span>
            </li>
          ))}
        </ul>
      </section>

      <section style={workplaneItemStyle}>
        <span style={workplaneBadgeStyle}>authority boundary</span>
        <strong>Read-only decision preview</strong>
        <span style={workplaneCopyStyle}>
          can_write_db {String(preview.authority_boundary.can_write_db)};
          can_write_dogfood_metric_snapshot{" "}
          {String(preview.authority_boundary.can_write_dogfood_metric_snapshot)};
          can_update_metrics {String(preview.authority_boundary.can_update_metrics)}
        </span>
        <span style={workplaneCopyStyle}>
          can_write_memory {String(preview.authority_boundary.can_write_memory)};
          can_update_continuity_relay{" "}
          {String(preview.authority_boundary.can_update_continuity_relay)};
          can_execute_codex {String(preview.authority_boundary.can_execute_codex)}
        </span>
      </section>
    </WorkplanePanelShell>
  );
}

function ReasonList({ title, reasons }: { title: string; reasons: string[] }) {
  return (
    <section style={workplaneItemStyle}>
      <span style={workplaneBadgeStyle}>{title}</span>
      <ul style={workplaneListStyle}>
        {(reasons.length > 0 ? reasons : ["none"]).slice(0, 8).map((reason) => (
          <li key={reason} style={workplaneItemStyle}>
            <span style={workplaneCopyStyle}>{reason}</span>
          </li>
        ))}
      </ul>
    </section>
  );
}
