...
import type { ContinuityRelayScopedWritePreview } from "@/types/continuity-relay-scoped-write-preview";

type ContinuityRelayScopedWritePreviewPanelProps = {
  preview: ContinuityRelayScopedWritePreview;
};

export function ContinuityRelayScopedWritePreviewPanel({
  preview,
}: ContinuityRelayScopedWritePreviewPanelProps) {
  return (
    <WorkplanePanelShell
      kicker="Scoped ContinuityRelay write"
      title="ContinuityRelay Scoped Write Preview"
      ariaLabel="ContinuityRelay Scoped Write Preview panel"
    >
      <p style={workplaneCopyStyle}>
        Read-only preview for the scoped local ContinuityRelay record and receipt.
      ...
      <WorkplanePanelMetricGrid>
        ...
        <WorkplanePanelMetric
          label="relay entries"
          value={String(preview.input_summary.continuity_relay_entry_count)}
        />
      </WorkplanePanelMetricGrid>

      <section style={workplaneItemStyle}>
        <span style={workplaneBadgeStyle}>selected ContinuityRelay</span>
        <strong>
          {preview.input_summary.selected_continuity_relay_candidate_count} refs
        </strong>
        ...
        <span style={workplaneCopyStyle}>
          related PerspectiveUnit record refs, advisory only:{" "}
          {preview.related_perspective_unit_record_refs.length}
        </span>
        <span style={workplaneCopyStyle}>
      ...
      <section style={workplaneItemStyle}>
        ...
        <span style={workplaneCopyStyle}>
          {preview.continuity_relay_entries
            .slice(0, 6)
            .map(
              (entry) =>
                `${entry.relay_directive}:${entry.source_candidate_ref}`,
            )
            .join("; ") || "none"}
        </span>
      </section>
