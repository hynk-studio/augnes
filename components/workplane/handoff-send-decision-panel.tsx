...
import type { HandoffSendOperatorDecisionPreview } from "@/types/handoff-send-decision";
...
export function HandoffSendDecisionPanel({
  preview,
}: {
  preview: HandoffSendOperatorDecisionPreview;
}) {
  return (
    <section aria-label="Handoff Send Decision" style={cardStyle}>
      <div>
        <p style={kickerStyle}>Handoff send decision preview</p>
        <h3 style={titleStyle}>{preview.decision_preview_status}</h3>
      </div>
      <p style={textStyle}>
        Operator decision preview only. It does not write records, send
        externally, call providers or external messaging systems, transfer Codex
        sessions, write clipboard, create downloads, write files, or mutate live
        handoff context.
      </p>
      <p style={textStyle}>
      ...
      <p style={textStyle}>
        ...
        create record{" "}
        {String(preview.authority_boundary.can_create_handoff_send_record)},
        send {String(preview.authority_boundary.can_send_handoff)}, provider{" "}
        {String(preview.authority_boundary.can_call_send_provider)}, email{" "}
        {String(preview.authority_boundary.can_call_email)}, webhook{" "}
