...
export function ResearchCandidateManualGlobalDogfoodNextWorkSignalReadbackPanel({
  readback,
  isLoading = false,
  error,
}: {
  ...
  return (
    <section
      className="perspective-inspector-section manual-global-dogfood-next-work-signal-readback"
      aria-label="Manual global dogfood next-work signal readback"
      data-augnes-authority="manual-next-work-signal-readback no-next-work-bias no-work no-perspective no-proof no-metrics no-memory"
    >
      <div className="perspective-constellation-shell-header">
        <div>
          <p className="panel-eyebrow">AUGNES / Next-Work Signal Readback</p>
          <h4>Manual next-work signal decision readback</h4>
          <p>
            Readback is limited to manual-derived next-work signal decision
            receipts, records, and rollback metadata. It does not write
        ...
        <div className="perspective-constellation-shell-status">
          ...
          <span className="status-pill">next-work bias false</span>
      ...
      {readback ? (
        <>
          <div className="perspective-formation-summary-grid">
            ...
            <div>
              ...
              <small>bias/work/Perspective/metrics/proof/memory/product</small>
            </div>
          </div>
            ...
            <div className="perspective-detail-stack">
              {readback.records_by_receipt.length > 0 ? (
                readback.records_by_receipt.map((recordSet) => (
                  <NextWorkSignalReceiptCard
                    key={recordSet.receipt.receipt_id}
                    recordSet={recordSet}
                  />
                ))
              ) : (
                <p>No manual next-work signal decision receipts have been written.</p>
              )}
            </div>
          </section>
          ...
          <section className="cockpit-surface-card">
            ...
            <div className="perspective-workbench-status-row">
              <BoundaryFlag
                label="can_write_next_work_signal_decision_record"
                value={
                  readback.authority_boundary
                    .can_write_next_work_signal_decision_record
                }
              />
              <BoundaryFlag
                label="can_write_next_work_signal_decision_receipt"
                value={
                  readback.authority_boundary
                    .can_write_next_work_signal_decision_receipt
                }
              />
              <BoundaryFlag
              ...
              <BoundaryFlag
                ...
                value={readback.authority_boundary.can_write_metric_snapshot}
              />
              <BoundaryFlag
        ...
        <p className="manual-note-runtime-hint">
          No next-work signal decision readback has been loaded yet.
        </p>
      )}
    </section>
  );
...
function NextWorkSignalReceiptCard({
  recordSet,
}: {
  recordSet: ResearchCandidateManualGlobalDogfoodNextWorkSignalRecordsByReceipt;
}) {
  const { receipt, next_work_signal_record: record, rollback } = recordSet;
  return (
    <section className="cockpit-surface-card">
      ...
      <dl>
        ...
        <dd>{record?.candidate_priority_hint ?? "record missing"}</dd>
