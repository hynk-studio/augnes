...
export function ProjectOnboardingHome({ initialRecent }: { initialRecent: RecentProjectEntryV01[] }) {
  ...
  async function confirm() {
    ...
    } catch (error) { setMessage(error instanceof Error && error.message === "inspection_stale" ? "The folder changed. Choose it again before confirming." : "The project could not be added."); setPicker(null); }
    finally { setBusy(false); }
  }
  async function open(entry: RecentProjectEntryV01) {
    ...
    setBusy(true);
    try {
      const active = recent.find((item) => item.is_active);
      const value = await mutate({ action: "open", project_id: entry.project.project_id, expected_project_id: active?.project.project_id ?? null });
      window.location.assign(value.result.destination);
  ...
  async function remove(entry: RecentProjectEntryV01) {
    setBusy(true);
    try { await mutate({ action: "remove", project_id: entry.project.project_id }); setRecent((items) => items.filter((item) => item.project.project_id !== entry.project.project_id)); setMessage("Removed from recent projects. Project data remains stored."); }
    catch { setMessage("The project could not be removed from recents."); }
    finally { setBusy(false); }
  }
  async function locate(entry: RecentProjectEntryV01) {
    ...
    try {
      ...
      window.location.assign(value.result.destination);
    } catch { setMessage("The replacement root conflicts with another project or changed during confirmation."); }
    finally { setBusy(false); }
  }

  return <main className="project-selector-shell">
