...
export function ResearchCandidateManualGlobalDogfoodPerspectiveStateApplicationReadbackPanel({
  readback,
  isLoading = false,
  error,
}: {
  ...
  return (
    <section
      ...
      aria-label="Manual global dogfood Perspective state application readback"
    ...
    >
      <div className="perspective-constellation-shell-header">
        <div>
          <p className="panel-eyebrow">AUGNES / Perspective State Application Readback</p>
          <h4>Manual Perspective state application record readback</h4>
          <p>
            Readback is limited to manual-derived Perspective state application receipts,
            state_application records, and rollback metadata. It does not update
      ...
      {readback ? (
        <>
          <div className="perspective-formation-summary-grid">
            <div>
              ...
              <small>source refs preserved in manual Perspective state application tables</small>
            </div>
            <div>
            ...
            <div className="perspective-detail-stack">
              {readback.records_by_receipt.length > 0 ? (
                readback.records_by_receipt.map((recordSet) => (
                  <PerspectiveStateApplicationReceiptCard
                    key={recordSet.receipt.receipt_id}
                    recordSet={recordSet}
                  />
                ))
              ) : (
                <p>No manual Perspective state application receipts have been written.</p>
              )}
            </div>
          </section>
          ...
          <section className="cockpit-surface-card">
            ...
            <div className="perspective-workbench-status-row">
              <BoundaryFlag
                ...
                value={readback.authority_boundary.can_write_perspective_state_application_record}
              />
              <BoundaryFlag
                label="can_write_perspective_state_application_receipt"
                value={readback.authority_boundary.can_write_perspective_state_application_receipt}
              />
              <BoundaryFlag
                label="can_write_perspective_state_application_rollback_metadata"
                value={
                  readback.authority_boundary
                    .can_write_perspective_state_application_rollback_metadata
                }
              />
              <BoundaryFlag
        ...
        <p className="manual-note-runtime-hint">
          No Perspective state application readback has been loaded yet.
        </p>
      )}
    </section>
  );
...
function PerspectiveStateApplicationReceiptCard({
  recordSet,
}: {
  recordSet: ResearchCandidateManualGlobalDogfoodPerspectiveStateApplicationRecordsByReceipt;
}) {
  const { receipt, perspective_state_application_record: record, rollback } = recordSet;
  return (
    <section className="cockpit-surface-card">
      ...
      <dl>
        <dt>source_perspective_state_application_contract_fingerprint</dt>
        <dd>{receipt.source_perspective_state_application_contract_fingerprint}</dd>
        <dt>source_perspective_state_application_review_fingerprint</dt>
        <dd>{receipt.source_perspective_state_application_review_fingerprint}</dd>
        ...
        <dd>{record?.state_application_label ?? "record missing"}</dd>
        <dt>state_application_rationale</dt>
        <dd>{record?.state_application_rationale ?? "record missing"}</dd>
        <dt>adapter_label</dt>
        ...
        <dd>{record?.intended_future_state_application_target ?? "record missing"}</dd>
        <dt>default_future_state_application_target</dt>
        <dd>{record?.default_future_state_application_target ?? "record missing"}</dd>
        <dt>state_application_scope_hint</dt>
        <dd>{record?.state_application_scope_hint ?? "record missing"}</dd>
