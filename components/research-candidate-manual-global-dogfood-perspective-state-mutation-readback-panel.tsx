import type {
  ...
  ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationRecordsByReceipt,
} from "@/types/research-candidate-manual-global-dogfood-perspective-state-mutation-write";
import { ResearchCandidateManualGlobalDogfoodPerspectiveAdapterContractPanel } from "@/components/research-candidate-manual-global-dogfood-perspective-adapter-contract-panel";

export function ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationReadbackPanel({
  readback,
  isLoading = false,
  error,
}: {
  ...
  return (
    <section
      className="perspective-inspector-section manual-global-dogfood-perspective-state-mutation-readback"
      aria-label="Manual global dogfood Perspective state mutation readback"
      data-augnes-authority="manual-perspective-state-mutation-readback no-current-working-perspective no-canonical-state no-promotion no-memory no-work no-proof no-metrics"
    >
      <div className="perspective-constellation-shell-header">
        <div>
          <p className="panel-eyebrow">AUGNES / Perspective State Mutation Readback</p>
          <h4>Manual Perspective state mutation record readback</h4>
          <p>
            Readback is limited to manual-derived Perspective state mutation receipts,
            state mutation records, and rollback metadata. It does not update
            current-working Perspective, directly mutate existing canonical
            Perspective state tables, promote Perspective, write Perspective Memory, mutate work,
      ...
      {readback ? (
        <>
          <div className="perspective-formation-summary-grid">
            <div>
              ...
              <small>source refs preserved in manual Perspective state mutation tables</small>
            </div>
            <div>
            ...
            <div className="perspective-detail-stack">
              {readback.records_by_receipt.length > 0 ? (
                readback.records_by_receipt.map((recordSet) => (
                  <PerspectiveStateMutationReceiptCard
                    key={recordSet.receipt.receipt_id}
                    recordSet={recordSet}
                  />
                ))
              ) : (
                <p>No manual Perspective state mutation receipts have been written.</p>
              )}
            </div>
          </section>
          ...
          <section className="cockpit-surface-card">
            ...
            <div className="perspective-workbench-status-row">
              <BoundaryFlag
                label="can_write_perspective_state_mutation_record"
                value={readback.authority_boundary.can_write_perspective_state_mutation_record}
              />
              <BoundaryFlag
                label="can_write_perspective_state_mutation_receipt"
                value={readback.authority_boundary.can_write_perspective_state_mutation_receipt}
              />
              <BoundaryFlag
              ...
              />
              <BoundaryFlag
                label="can_write_manual_canonical_perspective_state_mutation_record"
                value={
                  readback.authority_boundary
                    .can_write_manual_canonical_perspective_state_mutation_record
                }
              />
              <BoundaryFlag
                label="can_write_direct_canonical_perspective_state_table"
                value={
                  readback.authority_boundary
                    .can_write_direct_canonical_perspective_state_table
                }
              />
              <BoundaryFlag
                label="can_write_perspective_state_mutation_rollback_metadata"
                value={
                  readback.authority_boundary
                    .can_write_perspective_state_mutation_rollback_metadata
                }
              />
              <BoundaryFlag
                label="can_mutate_perspective_apply_record"
                value={
                  readback.authority_boundary.can_mutate_perspective_apply_record
                }
              />
              <BoundaryFlag
        ...
        <p className="manual-note-runtime-hint">
          No Perspective state mutation readback has been loaded yet.
        </p>
      )}
    </section>
  );
...
function PerspectiveStateMutationReceiptCard({
  recordSet,
}: {
  recordSet: ResearchCandidateManualGlobalDogfoodPerspectiveStateMutationRecordsByReceipt;
}) {
  const { receipt, perspective_state_mutation_record: record, rollback } = recordSet;
  return (
    <section className="cockpit-surface-card">
      ...
      <dl>
        <dt>source_perspective_state_mutation_contract_fingerprint</dt>
        <dd>{receipt.source_perspective_state_mutation_contract_fingerprint}</dd>
        <dt>source_perspective_state_mutation_review_fingerprint</dt>
        <dd>{receipt.source_perspective_state_mutation_review_fingerprint}</dd>
        ...
        <dd>{record?.mutation_rationale ?? "record missing"}</dd>
        <dt>apply_label</dt>
        ...
        <dd>{record?.intended_future_mutation_target ?? "record missing"}</dd>
        <dt>mutation_scope_hint</dt>
        <dd>{record?.mutation_scope_hint ?? "record missing"}</dd>
