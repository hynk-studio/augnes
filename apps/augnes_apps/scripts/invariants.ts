...
import { createMcpAppServer, AUGNES_BRIDGE_TOOL_NAMES, LEGACY_PUBLIC_TOOL_NAMES, PUBLIC_TOOL_NAMES } from "../src/server.js";
...
function assertBridgeTools(tools: Record<string, RegisteredTool>) {
  ...
  for (const name of ["augnes_observe", "augnes_record_action_result"] as const) {
    const tool = tools[name];
    assert.ok(tool, `${name} should be registered`);
    assert.equal(tool.enabled, true, `${name} should be enabled`);
...
function assertPublicToolSurface() {
  ...
  assert.deepEqual(AUGNES_BRIDGE_TOOL_NAMES, INTENDED_BRIDGE_TOOL_NAMES, "bridge tools must remain the intended Issue #2 surface");
  assert.deepEqual(PUBLIC_TOOL_NAMES, INTENDED_PUBLIC_TOOL_NAMES, "PUBLIC_TOOL_NAMES must remain the original public-only surface");
  ...
  try {
    ...
    assert.deepEqual(registeredNames, INTENDED_PUBLIC_TOOL_NAMES, "default registered tools must be exactly the original public nine");
  ...
  try {
    ...
    assert.deepEqual(
      registeredNames,
      [...INTENDED_LEGACY_TOOL_NAMES, ...INTENDED_BRIDGE_TOOL_NAMES],
      "bridge-enabled registered tools must include legacy tools plus the Augnes bridge surface"
    );
