...
present and removed. It removes state generated by a full run during final
cleanup. An owner-targeted run may generate `.next` through typecheck; when the
path was absent before execution, the executor removes that generated state
afterward. It does not remove pre-existing targeted-run state, use broad
`git clean`, or delete unrelated files. Existing Canonical children continue to
own their bounded OS-temporary resources; the executor does not create another
checkout or Git working copy.
