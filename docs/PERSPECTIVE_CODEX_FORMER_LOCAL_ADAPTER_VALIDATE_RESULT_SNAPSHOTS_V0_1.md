...
- `PASS` summaries whose `candidate_shape_status` is not `existing_validator_compatible`;
...
- `PASS with follow-up` summaries with `candidate_authority` other than `non_committed`;
- `PASS with follow-up` summaries with `worker_facing_guidance_advisory_only: false`;
...
- `PASS with follow-up` summaries whose `candidate_shape_status` is not `existing_validator_compatible`;
