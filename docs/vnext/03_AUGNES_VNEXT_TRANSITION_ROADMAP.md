...
- 현재 macro-slice는 pure provider-neutral `StateTransitionReceiptV01` type, builder,
...
- 현재 macro-slice는 `EpisodeDeltaProposalV01` + `ReviewDecisionV01` + 모든 target의
...
- 현재 macro-slice는 synthetic applied receipt에서 explicit later
  `TaskContextPacketV01`로 이어지는 pure relation path를 Level 2 Integrated Core Path로
  만든다. Exact receipt lineage, present after-state selection, retired before-state
  exclusion, multi-target atomicity와 cross-project fail-closed isolation을 검증하지만
  packet을 자동 생성하거나 mutation하는 runtime path는 추가하지 않는다.
- gate, current-state, application, durable-record, receipt와 later-packet fixtures는 모두
  synthetic conformance material이다. 이 checkpoint는 실제 authorization, 실제 durable
  transition 또는 observed use를 증명하지 않는다.

M3는 여전히 완료되지 않았다. 실제 사용자 decision persistence, production semantic
commit gate, durable vNext writer, real `StateTransitionReceipt`, 실제 later
`TaskContextPacket` selection 변화의 observed use, 이후 outcome validation이 없기
때문이다. 이 checkpoint는 Project Home, Semantic Workbench, Inspector 또는 runtime
Context Compiler 구현 완료도 의미하지 않는다.
