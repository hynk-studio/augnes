...
### Workstream H: Inspector Consolidation

#### 목표

Blank State, Workplane, Perspective와 legacy readback의 유용한 부분을 하나의 Inspector 정보 구조로 수렴한다.
...
- 기존 route는 compatibility redirect/read-only로 유지 가능
- 새 panel보다 projection composition과 drill-down 우선
- 긴 authority copy 대신 현재 decision에 필요한 이유 표시

#### 완료 조건

- 주요 review가 Inspector와 host-native card에서 가능
- Workplane 사용 없이도 current state와 attention을 파악 가능
- legacy surface 사용량과 removal dependency를 측정 가능
...
| 현재 기능 | vNext 처분 | 비고 |
|---|---|---|
| Blank State | Inspector `Now/Attention`으로 흡수 | compatibility route 가능 |
| Agent Workplane | `Work & Runs/Timeline`으로 분해 | 기본 작업면 지위 종료 |
...
| diff/terminal/browser replicas | 제거/중단 | native host 사용 |
...
### M4. Inspector and Legacy Consolidation
