...
- Additive `vnext_core_records` immutable ledger와
  `vnext_semantic_state_entries` current-state projection은 persisted semantic-state aggregate를
...
- Read-only preview는 exact proposal/decision relation, current projection, candidate-derived
  semantic content와 expected revision을 confirmation digest에 결합한다. Explicit confirmation
  path는 decision actor와 digest, expiry, local observation, authorized applier를 검증하여 gate
...
- Transactional writer는 store 안에서 current state와 applied lineage를 다시 읽고 M3B
  ...
  multi-target all-or-nothing apply를 revision + content-fingerprint CAS로 처리한다. Exact replay,
  conflicting result, stale state, project isolation과 bounded test-only failure injection을
