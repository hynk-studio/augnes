...
|---|---|---|---|
| SQLite transaction, exact readback, reopen, monotonic target-head/ABA CAS, rollback, integrity와 backup/restore | `real_local_observation` | bounded local storage mechanics와 absent-state stale-gate refusal가 실제 temp DB I/O에서 동작함 | product/user DB transition, user authorization, product use |
