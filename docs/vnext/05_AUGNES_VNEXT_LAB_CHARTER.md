...
- Lab data는 project scope를 가진다.
- personal affect·preference 추론은 Personal Vault 또는 ephemeral Lab record로 분리한다.
...
Lab 기능이 Core 또는 Inspector의 기본 기능이 되려면 다음을 모두 만족해야 한다.
...
3. 실제 RunReceipt 기반 diagnostics
4. external-project evaluation
5. productization candidate review
```
