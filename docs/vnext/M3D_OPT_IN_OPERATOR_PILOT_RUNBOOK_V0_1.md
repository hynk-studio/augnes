...
1. root dependency를 `npm ci`로 provision한다.
2. nested app dependency를 `npm --prefix apps/augnes_apps ci`로 provision한다.
3. `vnext_m3d_evidence_runner_qualification.v0.1` portable mode를 실행한다.
4. 같은 intended environment에서 local-full mode를 실행한다.
5. public-safe qualification receipt를 보존한다.
6. application commit, qualification version, Node major, platform, architecture, 두 lockfile
   hash와 local-full browser identity를 독립적으로 확인한다.
7. 두 mode가 qualified이고 identity가 유지될 때에만 다음 chain ID를 할당하고 product
