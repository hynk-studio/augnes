...
Personal continuity가 필요하면 별도 opt-in Personal Vault 경계 후로 둔다.
...
Personal Perspective persistence 전에 workspace-scoped proposal, review,
authorization, Decision/Transition, retention, deletion, selective project sharing,
context inclusion, remote egress, backup/restore와 cross-project isolation을 함께 다루는
별도 architecture decision이 필요하다. 현재 PR은 새 protocol type, production
TypeScript contract, DB table, API route, state-transition path 또는 TaskContextPacket
field를 승인하지 않는다.
