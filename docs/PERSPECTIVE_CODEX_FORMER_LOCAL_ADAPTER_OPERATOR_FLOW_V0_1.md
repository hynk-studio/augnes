...
`/cockpit/perspective/codex-former/local-adapter-operator-flow` is a local-only operator shell for the manual Augnes / Codex loop. It lets a user see the current source and prepare references, copy a bounded packet into a separate user-started Codex session, paste or load one returned envelope, preview local validation, inspect bounded candidate review material, and choose a local draft next action from one route.
...
2. Review the bounded Copy For Codex packet preview.
...
- The route avoids clipboard APIs. The Copy For Codex panel is a selectable bounded preview.
