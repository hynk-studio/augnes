...
- `docs/AGENT_WORKPLANE_V0_1.md`: Phase 5A read-only Agent Workplane shell
  ...
  Phase 5C adds read-only / preview-only Agent Workplane skeletons for
  Projection Candidates, Delta Batch, Handoff Builder preview, Run Postmortem,
  and Trace / Diagnostics. It preserves Phase 5A/5B panels and legacy Cockpit
  compatibility content, uses existing Current Working Perspective and Augnes
  Delta Projection read context, keeps trace/diagnostics bounded, and adds no
  DB schema/migration, DB write, API write route, MCP/App tool,
...
- `docs/GUIDEBRIEF_CONTRACT_V0_1.md`: Phase 6A GuideBrief /
  ...
  Historical Phase 7A–7F transfer-packet experiments are retired from normal
  product use by R5 PR D. The route, Web, App/MCP, skill, copy/export, fixture,
  and smoke producers were removed after the persisted packet → native host →
  canonical receipt path gained replacement coverage. The two retained Phase 7
  documents are marked historical and support legacy data/type interpretation
  only; they are not active instructions or a mutation path.
  ...
  Phase 9C Agent Workplane Autonomy Runner Preflight read-only Web preview
  panel v0.1 adds repo-local non-SSOT pointers for
  `lib/autonomy/read-autonomy-runner-preflight-for-web.ts`,
  `components/autonomy/autonomy-runner-preflight-preview-panel.tsx`, and
  `scripts/smoke-autonomy-runner-preflight-web-preview-v0-1.mjs` (`npm run
  smoke:autonomy-runner-preflight-web-preview-v0-1`). The `/workbench` panel is
  read-only display only, consumes Phase 9A/9B preflight/dry-run preview data
  without inventing readiness or blocker policy, displays readiness,
  blockers/warnings, `dry_run_plan.status: dry_run_only`, planned steps,
  `would_execute: false`, source/route status, public safety status, and the
  all-false no-run authority boundary. Phase 9C adds no API route, App/MCP
  tool, actual runner, scheduler, daemon, background work, DB schema/migration,
  DB write, provider/OpenAI call, GitHub actuation, Codex execution,
  proof/evidence write, memory mutation, durable Perspective apply, handoff
  execution, branch/PR creation from product code, product-write, budget spend,
  auto-apply, file download/export-to-disk, merge/publish/retry/replay/deploy
  behavior, or external side effect. This index pointer is not roadmap
  authority.
  Phase 9D ChatGPT App/MCP Autonomy Runner Preflight read-only preview tool
  v0.1 adds repo-local non-SSOT pointers for
  `apps/augnes_apps/src/server.ts`,
  `apps/augnes_apps/src/lib/state-runtime-types.ts`,
  `apps/augnes_apps/src/adapters/state-runtime-http.ts`,
  `apps/augnes_apps/scripts/mock-state-runtime.ts`, and
  `scripts/smoke-chatgpt-app-autonomy-runner-preflight-tool-v0-1.mjs`
  (`npm run smoke:chatgpt-app-autonomy-runner-preflight-tool-v0-1`). The
  `augnes_get_autonomy_runner_preflight` tool is read-only preview only,
  consumes Phase 9A/9B preflight/dry-run preview data without inventing
  readiness or blocker policy, returns readiness, blockers/warnings,
  `dry_run_plan.status: dry_run_only`, planned steps, `would_execute: false`,
  source refs, public safety status, no-run notes, and the all-false authority
  boundary. Phase 9D adds no API route, UI, actual runner, scheduler, daemon,
  background work, DB schema/migration, DB write, provider/OpenAI call, GitHub
  actuation, Codex execution, proof/evidence write, memory mutation, durable
...
- Codex Result To Dogfooding Record Binding v0.1:
  Historical metadata only. Its prose-result producer, mapper, fixture, smoke,
  and active documentation were removed by R5 PR D. Historical canonical
  records remain readable; no raw report is reconstructed or re-admitted.
...
- Codex Result Report Ingestion v0.1:
  Historical metadata only. The manual textarea, normalizer, fixture, smoke,
  and component were retired by R5 PR D. Native-host results now use the one
  structured normalizer and canonical `RunReceipt` writer.
...
- `AUGNES_START_HERE_FOR_USERS_AND_AI.md`: README front-door companion
  ...
  `AG-DOGFOOD-RESEARCH-001` historical research work loop and authority
  boundaries를 한 곳에 모은다. Its pasted-result return path is retired.
