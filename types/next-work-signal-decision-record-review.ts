...
export const NEXT_WORK_SIGNAL_DECISION_RECORD_REVIEW_VERSION =
  "next_work_signal_decision_record_review.v0.1" as const;

export interface NextWorkSignalDecisionRecordReviewInput {
  records?: unknown[];
  store_result?: NextWorkSignalDecisionStoreResult | null;
...
export type NextWorkSignalDecisionRecordReviewStatus =
  | "no_records"
  | "schema_missing"
  | "records_available"
...
export interface NextWorkSignalDecisionRecordSummary {
  record_id: string;
  idempotency_key: string;
  created_at: string;
  operator_ref: string | null;
  metric_snapshot_source_count: number;
  selected_signal_count: number;
  preserve_context_count: number;
  warn_context_count: number;
  drop_or_deprioritize_count: number;
  verification_focus_count: number;
  expected_observed_followup_count: number;
  handoff_quality_focus_count: number;
  context_diet_count: number;
  review_burden_reduction_count: number;
  record_fingerprint: string | null;
  receipt_no_side_effects_valid: boolean;
  problem_reasons: string[];
}

export interface NextWorkSignalDecisionNoSideEffectsSummary {
  next_work_signal_decision_record_written_count: number;
  next_work_signal_decision_receipt_written_count: number;
  next_work_signal_decision_persisted_count: number;
...
export interface NextWorkSignalDecisionRecordReviewAuthorityBoundary {
...
export interface NextWorkSignalDecisionRecordReview {
  review_version: typeof NEXT_WORK_SIGNAL_DECISION_RECORD_REVIEW_VERSION;
  scope: string;
  as_of: string;
  source_refs: string[];
  review_status: NextWorkSignalDecisionRecordReviewStatus;
  input_summary: {
  ...
  record_summaries: NextWorkSignalDecisionRecordSummary[];
  selected_record_summary: NextWorkSignalDecisionRecordSummary | null;
  latest_record_summary: NextWorkSignalDecisionRecordSummary | null;
  ...
  receipt_no_side_effects_summary: NextWorkSignalDecisionNoSideEffectsSummary;
  blocked_reasons: string[];
  insufficient_data_reasons: string[];
  operator_review_checklist: string[];
  would_not_do: string[];
  non_goals: string[];
  authority_boundary: NextWorkSignalDecisionRecordReviewAuthorityBoundary;
}

export type NextWorkSignalDecisionRecordReviewNoSideEffects =
  NextWorkSignalDecisionNoSideEffects;
