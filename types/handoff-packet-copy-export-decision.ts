...
export const HANDOFF_PACKET_COPY_EXPORT_DECISION_PREVIEW_VERSION =
  "handoff_packet_copy_export_operator_decision_preview.v0.1" as const;

export type HandoffPacketCopyExportDecisionPreviewStatus =
  | "insufficient_data"
  | "blocked"
...
export type HandoffPacketCopyExportOperatorDecisionIntent =
  | "approve_for_handoff_packet_copy_export_record"
  | "keep_preview_only"
  | "reject";

export interface HandoffPacketCopyExportDecisionPreviewInput {
  handoff_packet_copy_export_preview?: unknown;
  requested_operator_ref?: string;
  requested_idempotency_key?: string;
  review_confirmation_ref?: string;
  operator_decision_intent?: HandoffPacketCopyExportOperatorDecisionIntent | string;
  scope?: string;
  as_of?: string;
  source_refs?: string[];
}

export interface HandoffPacketCopyExportDecisionReadiness {
  write_ready: boolean;
...
export interface HandoffPacketCopyExportDecisionAuthorityBoundary {
  read_only: true;
  advisory_only: true;
...
export interface HandoffPacketCopyExportOperatorDecisionPreview {
  preview_version: typeof HANDOFF_PACKET_COPY_EXPORT_DECISION_PREVIEW_VERSION;
  scope: string;
  as_of: string;
  source_refs: string[];
  decision_preview_status: HandoffPacketCopyExportDecisionPreviewStatus;
  recommended_operator_decision:
    | "approve_for_handoff_packet_copy_export_record"
    | "keep_preview_only"
    | "reject";
  available_operator_decisions: HandoffPacketCopyExportOperatorDecisionIntent[];
  input_summary: {
    has_copy_export_preview: boolean;
    copy_export_preview_ready: boolean;
    operator_decision_intent:
      | HandoffPacketCopyExportOperatorDecisionIntent
      | string
      | null;
  ...
  write_readiness: HandoffPacketCopyExportDecisionReadiness;
  approval_requirements: string[];
  blocking_reasons: string[];
  missing_evidence: string[];
  refusal_reasons: string[];
  evidence_summary: {
    has_ready_copy_export_preview: boolean;
  ...
  would_write_handoff_packet_copy_export_decision_preview: {
    operator_decision: "approve_for_handoff_packet_copy_export_record" | null;
    requested_operator_ref: string | null;
    requested_idempotency_key: string | null;
    review_confirmation_ref: string | null;
    source_refs: string[];
    evidence_refs: string[];
    copy_export_preview: HandoffPacketCopyExportPreview | null;
  };
  operator_review_checklist: string[];
  would_not_write: string[];
  non_goals: string[];
  authority_boundary: HandoffPacketCopyExportDecisionAuthorityBoundary;
}
