...
} from "./perspective-next-work-bias-write";

export const PERSPECTIVE_NEXT_WORK_BIAS_RECORD_REVIEW_VERSION =
  "perspective_next_work_bias_record_review.v0.1" as const;

export interface PerspectiveNextWorkBiasRecordReviewInput {
  records?: unknown[];
  store_result?: PerspectiveNextWorkBiasStoreResult | null;
...
export type PerspectiveNextWorkBiasRecordReviewStatus =
  | "no_records"
  | "schema_missing"
  | "records_available"
...
export interface PerspectiveNextWorkBiasRecordSummary {
  record_id: string;
  ...
  preserve_next_time_count: number;
  warn_next_time_count: number;
  drop_or_deprioritize_count: number;
  verification_bias_count: number;
  context_diet_bias_count: number;
  handoff_quality_bias_count: number;
  record_fingerprint: string | null;
  receipt_no_side_effects_valid: boolean;
  problem_reasons: string[];
}

export interface PerspectiveNextWorkBiasNoSideEffectsSummary {
  perspective_next_work_bias_record_written_count: number;
  perspective_next_work_bias_receipt_written_count: number;
  perspective_next_work_bias_persisted_count: number;
...
export interface PerspectiveNextWorkBiasRecordReviewAuthorityBoundary {
...
export interface PerspectiveNextWorkBiasRecordReview {
  review_version: typeof PERSPECTIVE_NEXT_WORK_BIAS_RECORD_REVIEW_VERSION;
  scope: string;
  as_of: string;
  source_refs: string[];
  review_status: PerspectiveNextWorkBiasRecordReviewStatus;
  input_summary: {
  ...
  record_summaries: PerspectiveNextWorkBiasRecordSummary[];
  selected_record_summary: PerspectiveNextWorkBiasRecordSummary | null;
  latest_record_summary: PerspectiveNextWorkBiasRecordSummary | null;
  ...
  next_work_bias_material_summary: {
    selected_next_work_bias_candidate_count: number;
    next_work_bias_entry_count: number;
    preserve_next_time_count: number;
    warn_next_time_count: number;
  ...
  receipt_no_side_effects_summary: PerspectiveNextWorkBiasNoSideEffectsSummary;
  blocked_reasons: string[];
  insufficient_data_reasons: string[];
  operator_review_checklist: string[];
  would_not_do: string[];
  non_goals: string[];
  authority_boundary: PerspectiveNextWorkBiasRecordReviewAuthorityBoundary;
}

export type PerspectiveNextWorkBiasRecordReviewNoSideEffects =
  PerspectiveNextWorkBiasNoSideEffects;
