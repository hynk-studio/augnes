...
export const CURRENT_WORKING_PERSPECTIVE_ROUTE_INTEGRATION_CONTRACT_DECISION_PREVIEW_VERSION =
  "current_working_perspective_route_integration_contract_operator_decision_preview.v0.1" as const;

export type CurrentWorkingPerspectiveRouteIntegrationContractOperatorDecisionIntent =
  | "approve_for_current_working_perspective_route_integration_contract_record"
  | "keep_runtime_only"
  | "keep_preview_only"
  | "reject";

export type CurrentWorkingPerspectiveRouteIntegrationContractDecisionPreviewStatus =
  | "no_current_working_perspective_route_integration_contract_preview"
  | "insufficient_data"
  | "blocked"
...
export interface CurrentWorkingPerspectiveRouteIntegrationContractDecisionPreviewInput {
  current_working_perspective_route_integration_contract_preview?: unknown;
  requested_operator_ref?: string;
  requested_idempotency_key?: string;
  review_confirmation_ref?: string;
  operator_decision_intent?: CurrentWorkingPerspectiveRouteIntegrationContractOperatorDecisionIntent;
  scope?: string;
  as_of?: string;
  source_refs?: string[];
}

export interface CurrentWorkingPerspectiveRouteIntegrationContractDecisionAuthorityBoundary {
  read_only: true;
  advisory_only: true;
...
export interface CurrentWorkingPerspectiveRouteIntegrationContractOperatorDecisionPreview {
  preview_version: typeof CURRENT_WORKING_PERSPECTIVE_ROUTE_INTEGRATION_CONTRACT_DECISION_PREVIEW_VERSION;
  scope: string;
  as_of: string;
  source_refs: string[];
  decision_preview_status: CurrentWorkingPerspectiveRouteIntegrationContractDecisionPreviewStatus;
  ...
  available_operator_decisions: CurrentWorkingPerspectiveRouteIntegrationContractOperatorDecisionIntent[];
  input_summary: {
    has_contract_preview: boolean;
    contract_preview_ready: boolean;
    operator_decision_intent: CurrentWorkingPerspectiveRouteIntegrationContractOperatorDecisionIntent | null;
  ...
  write_readiness: CurrentWorkingPerspectiveRouteIntegrationContractReadiness;
  ...
  would_write_current_working_perspective_route_integration_contract_decision_preview: {
    ...
    source_refs: string[];
    evidence_refs: string[];
    contract_preview: CurrentWorkingPerspectiveRouteIntegrationContractPreview | null;
  };
  operator_review_checklist: string[];
  would_not_write: string[];
  non_goals: string[];
  authority_boundary: CurrentWorkingPerspectiveRouteIntegrationContractDecisionAuthorityBoundary;
}
