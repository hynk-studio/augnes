...
} from "./current-working-perspective-apply-preview";

export const CURRENT_WORKING_PERSPECTIVE_APPLY_DECISION_PREVIEW_VERSION =
  "current_working_perspective_apply_operator_decision_preview.v0.1" as const;

export type CurrentWorkingPerspectiveApplyOperatorDecisionIntent =
  | "approve_for_current_working_perspective_apply_record"
  | "keep_preview_only"
  | "reject";

export type CurrentWorkingPerspectiveApplyDecisionPreviewStatus =
  | "no_current_working_perspective_apply_preview"
  | "insufficient_data"
  | "blocked"
...
export interface CurrentWorkingPerspectiveApplyDecisionPreviewInput {
  current_working_perspective_apply_preview?: unknown;
  requested_operator_ref?: string;
  requested_idempotency_key?: string;
  review_confirmation_ref?: string;
  operator_decision_intent?: CurrentWorkingPerspectiveApplyOperatorDecisionIntent;
  scope?: string;
  as_of?: string;
  source_refs?: string[];
}

export interface CurrentWorkingPerspectiveApplyDecisionAuthorityBoundary {
  read_only: true;
  advisory_only: true;
...
export interface CurrentWorkingPerspectiveApplyOperatorDecisionPreview {
  preview_version: typeof CURRENT_WORKING_PERSPECTIVE_APPLY_DECISION_PREVIEW_VERSION;
  scope: string;
  as_of: string;
  source_refs: string[];
  decision_preview_status: CurrentWorkingPerspectiveApplyDecisionPreviewStatus;
  recommended_operator_decision:
    | "approve_for_current_working_perspective_apply_record"
    | "keep_preview_only"
    | "reject"
    | "resolve_current_working_perspective_apply_blockers";
  available_operator_decisions: CurrentWorkingPerspectiveApplyOperatorDecisionIntent[];
  input_summary: {
    has_contract_preview: boolean;
    contract_preview_ready: boolean;
    operator_decision_intent: CurrentWorkingPerspectiveApplyOperatorDecisionIntent | null;
  ...
  write_readiness: CurrentWorkingPerspectiveApplyReadiness;
  ...
  would_write_current_working_perspective_apply_decision_preview: {
    ...
    source_refs: string[];
    evidence_refs: string[];
    contract_preview: CurrentWorkingPerspectiveApplyPreview | null;
  };
  operator_review_checklist: string[];
  would_not_write: string[];
  non_goals: string[];
  authority_boundary: CurrentWorkingPerspectiveApplyDecisionAuthorityBoundary;
}
