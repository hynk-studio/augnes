...
} from "./perspective-unit-write";

export const PERSPECTIVE_UNIT_RECORD_REVIEW_VERSION =
  "perspective_unit_record_review.v0.1" as const;

export interface PerspectiveUnitRecordReviewInput {
  records?: unknown[];
  store_result?: PerspectiveUnitStoreResult | null;
...
export type PerspectiveUnitRecordReviewStatus =
  | "no_records"
  | "schema_missing"
  | "records_available"
...
export interface PerspectiveUnitRecordSummary {
  record_id: string;
  ...
  weaken_or_warn_count: number;
  retire_or_deprioritize_count: number;
  split_or_review_count: number;
  record_fingerprint: string | null;
  receipt_no_side_effects_valid: boolean;
  problem_reasons: string[];
}

export interface PerspectiveUnitNoSideEffectsSummary {
  perspective_unit_record_written_count: number;
  perspective_unit_receipt_written_count: number;
  perspective_unit_persisted_count: number;
...
export interface PerspectiveUnitRecordReviewAuthorityBoundary {
...
export interface PerspectiveUnitRecordReview {
  review_version: typeof PERSPECTIVE_UNIT_RECORD_REVIEW_VERSION;
  scope: string;
  as_of: string;
  source_refs: string[];
  review_status: PerspectiveUnitRecordReviewStatus;
  input_summary: {
  ...
  record_summaries: PerspectiveUnitRecordSummary[];
  selected_record_summary: PerspectiveUnitRecordSummary | null;
  latest_record_summary: PerspectiveUnitRecordSummary | null;
  ...
  perspective_unit_material_summary: {
    selected_perspective_unit_candidate_count: number;
    perspective_unit_entry_count: number;
    reinforce_count: number;
    weaken_or_warn_count: number;
  ...
  receipt_no_side_effects_summary: PerspectiveUnitNoSideEffectsSummary;
  blocked_reasons: string[];
  insufficient_data_reasons: string[];
  operator_review_checklist: string[];
  would_not_do: string[];
  non_goals: string[];
  authority_boundary: PerspectiveUnitRecordReviewAuthorityBoundary;
}

export type PerspectiveUnitRecordReviewNoSideEffects =
  PerspectiveUnitNoSideEffects;
