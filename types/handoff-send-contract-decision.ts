...
export const HANDOFF_SEND_CONTRACT_DECISION_PREVIEW_VERSION =
  "handoff_send_contract_operator_decision_preview.v0.1" as const;

export type HandoffSendContractOperatorDecisionIntent =
  | "approve_for_handoff_send_contract_record"
  | "keep_preview_only"
  | "reject";

export type HandoffSendContractDecisionPreviewStatus =
  | "insufficient_data"
  | "blocked"
...
export interface HandoffSendContractDecisionPreviewInput {
  handoff_send_contract_preview?: unknown;
  requested_operator_ref?: string;
  requested_idempotency_key?: string;
  review_confirmation_ref?: string;
  operator_decision_intent?: HandoffSendContractOperatorDecisionIntent | string;
  scope?: string;
  as_of?: string;
  source_refs?: string[];
}

export interface HandoffSendContractDecisionReadiness {
  write_ready: boolean;
...
export interface HandoffSendContractDecisionAuthorityBoundary {
  read_only: true;
  advisory_only: true;
...
export interface HandoffSendContractOperatorDecisionPreview {
  preview_version: typeof HANDOFF_SEND_CONTRACT_DECISION_PREVIEW_VERSION;
  scope: string;
  as_of: string;
  source_refs: string[];
  decision_preview_status: HandoffSendContractDecisionPreviewStatus;
  recommended_operator_decision:
    | "approve_for_handoff_send_contract_record"
    | "keep_preview_only"
    | "reject";
  available_operator_decisions: HandoffSendContractOperatorDecisionIntent[];
  input_summary: {
    has_handoff_send_contract_preview: boolean;
    handoff_send_contract_preview_ready: boolean;
    operator_decision_intent:
      | HandoffSendContractOperatorDecisionIntent
      | string
      | null;
  ...
  write_readiness: HandoffSendContractDecisionReadiness;
  approval_requirements: string[];
  blocking_reasons: string[];
  missing_evidence: string[];
  refusal_reasons: string[];
  evidence_summary: {
    has_ready_handoff_send_contract_preview: boolean;
  ...
  would_write_handoff_send_contract_decision_preview: {
    operator_decision: "approve_for_handoff_send_contract_record" | null;
  ...
  non_goals: string[];
  authority_boundary: HandoffSendContractDecisionAuthorityBoundary;
}
